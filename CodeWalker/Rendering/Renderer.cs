...
{
    private readonly Dictionary<Archetype, Renderable> ArchetypeRenderables = new();
    ...
    private float fcelapsed;
    private readonly DXForm Form;
    private int fps;
    private int framecount;
    private readonly GameFileCache gameFileCache;


    private readonly ShaderGlobalLights globalLights = new();
    ...
    private readonly RenderLodManager LodManager = new();

    public float
        lodthreshold = 50.0f / (0.1f + Settings.Default.DetailDist); //to match formula for the DetailTrackBar value
    ...
    private readonly List<YmapEntityDef> RenderEntities = new();
    ...
    private readonly List<RenderSkeletonItem> renderskeletonlist = new();
    ...
    private readonly List<YmapEntityDef> renderworldentities = new(); //used when rendering world view.
    public float renderworldLodDistMult = 1.0f;

    public rage__eLodType renderworldMaxLOD = rage__eLodType.LODTYPES_DEPTH_ORPHANHD;
    private readonly List<RenderableEntity> renderworldrenderables = new();
    private readonly Dictionary<YmapEntityDef, Renderable> RequiredParents = new();


    private readonly YmapEntityDef SelectedCarGenEntity = new(); //placeholder entity object for drawing cars
    ...
    public MapSelectionMode
        SelectionMode = MapSelectionMode.Entity; //to assist in rendering embedded collisions properly...
    ...
    private readonly List<VertexTypePC> skeletonLineVerts = new();
    ...
    private readonly List<YtdFile> tryGetRenderableHDtxds = new();


    private readonly List<YtdFile> tryGetRenderableSDtxds = new();
    ...
    public Renderer(DXForm form, GameFileCache cache)
    {
        Form = form;
        gameFileCache = cache;
        if (gameFileCache == null) gameFileCache = GameFileCacheFactory.Create();
    ...
    public RenderableCache RenderableCache { get; }

    public DXManager DXMan { get; } = new();

    public Device Device { get; private set; }

    public object RenderSyncRoot { get; } = new();
    ...
    {
        var rcItemsPending = RenderableCache.ContentThreadProc();
    ...
    {
        if (!Monitor.TryEnter(RenderSyncRoot, 50)) return; //couldn't get a lock...
    ...
    {
        var rgc = shaders != null ? shaders.RenderedGeometries : 0;
        var crc = RenderableCache.LoadedRenderableCount;
        var ctc = RenderableCache.LoadedTextureCount;
        var tcrc = RenderableCache.MemCachedRenderableCount;
        var tctc = RenderableCache.MemCachedTextureCount;
    ...
    public void UpdateSelectionDrawFlags(DrawableModel model, DrawableGeometry geom, bool rem)
    {
        lock (RenderSyncRoot)
        {
            if (model != null)
            {
                if (rem)
                {
                    if (SelectionModelDrawFlags.ContainsKey(model)) SelectionModelDrawFlags.Remove(model);
                }
                else
                {
            ...
            if (geom != null)
            {
                if (rem)
                {
                    if (SelectionGeometryDrawFlags.ContainsKey(geom)) SelectionGeometryDrawFlags.Remove(geom);
                }
                else
                {
    ...
    private void UpdateTimeOfDay(float elapsed)
    {
        if (timerunning)
        {
            var helapsed = elapsed * timespeed / 60.0f;
            timeofday += helapsed;
    ...
    {
        var lightdir = Vector3.Zero; //will be updated before each frame from X and Y vars
        var lightdircolour = Color4.White;
        var lightdirambcolour = new Color4(0.5f, 0.5f, 0.5f, 1.0f);
        var lightnaturalupcolour = new Color4(0.0f);
        var lightnaturaldowncolour = new Color4(0.0f);
        var lightartificialupcolour = new Color4(0.0f);
        var lightartificialdowncolour = new Color4(0.0f);
        var hdr = shaders != null ? shaders.hdr : false;
        var hdrint = 1.0f;
        var sundir = Vector3.Up;
        var moondir = Vector3.Down;
        var moonax = Vector3.UnitZ;

        if (controllightdir)
        {
            var cryd = (float)Math.Cos(lightdiry);
        ...
        {
            var sunroll = timecycle.sun_roll * (float)Math.PI / 180.0f; //122
            var moonroll = timecycle.moon_roll * (float)Math.PI / 180.0f; //-122
            var moonwobamp = timecycle.moon_wobble_amp; //0.2
            var moonwobfreq = timecycle.moon_wobble_freq; //2
            var moonwoboffs = timecycle.moon_wobble_offset; //0.375
            var dayval = 0.5f + (timeofday - 6.0f) / 14.0f;
            var nightval = (timeofday > 12.0f ? timeofday - 7.0f : timeofday + 17.0f) / 9.0f;
            var daycyc = (float)Math.PI * dayval;
            var nightcyc = (float)Math.PI * nightval;
            ...
            if (swaphemisphere) sundir.Y = -sundir.Y;

            lightdir = sundir;

            //if (lightdir.Z < -0.5f) lightdir.Z = -lightdir.Z; //make sure the lightsource is always above the horizon...

            if (timeofday < 5.0f || timeofday > 21.0f) lightdir = moondir;
            //usemoon = true;
            if (lightdir.Z < 0) lightdir.Z = 0; //don't let the light source go below the horizon...
            ...
            if (weather != null && weather.Inited)
            {
                ...
                var lamult = weather.CurrentValues.lightDirAmbIntensityMult;
                var abounce = weather.CurrentValues.lightDirAmbBounce;
                var minmult = hdr ? 0.0f : 0.5f;
                ...
                lightnaturalupcolour *=
                    lightnaturalupcolour.Alpha * weather.CurrentValues.lightNaturalAmbUpIntensityMult;
                ...
                if (!hdr)
                {
                    var maxdirc = new Color4(1.0f);
                    var maxambc = new Color4(0.5f);
        ...
        globalLights.Params.LightArtificialAmbUp =
            renderartificialambientlight ? lightartificialupcolour : Color4.Black;
        globalLights.Params.LightArtificialAmbDown =
            renderartificialambientlight ? lightartificialdowncolour : Color4.Black;


        if (shaders != null) shaders.SetGlobalLightParams(globalLights);
    }
    ...
    private void UpdateWindVector(float elapsed)
    {
        ...
        var dirval = (float)(currentWindTime * 0.01 * Math.PI);
        var dirval1 = (float)Math.Sin(currentWindTime * 0.100 * Math.PI) * 0.3f;
        var dirval2 = (float)(currentWindTime * 0.333 * Math.PI);
        var dirval3 = (float)(currentWindTime * 0.5 * Math.PI);
        var dirval4 = (float)Math.Sin(currentWindTime * 0.223 * Math.PI) * 0.4f;
        var dirval5 = (float)Math.Sin(currentWindTime * 0.4 * Math.PI) * 5.5f;
        ...
        var strval = (float)(currentWindTime * 0.1 * Math.PI);
        var strval2 = (float)(currentWindTime * 0.825 * Math.PI);
        var strval3 = (float)(currentWindTime * 0.333 * Math.PI);
        var strval4 = (float)(currentWindTime * 0.666 * Math.PI);
        var strbase = 0.1f * (float)Math.Sin(strval * 0.5);
        var strbase2 = 0.02f * (float)Math.Sin(strval2 * 0.1);
    ...
    public void RenderMarkers(List<MapMarker> batch)
    {
        ...
        {
            icon = marker.Icon;
            var texs = new Vector2(icon.TexWidth, icon.TexHeight);
            var size = texs * marker.Distance;
            var offset = (new Vector2(texs.X, -texs.Y) - new Vector2(icon.Center.X, -icon.Center.Y) * 2.0f) *
                         marker.Distance;
    ...
    public void RenderTransformWidget(TransformWidget widget)
    {
        ...
        if (widget.Mode == WidgetMode.Rotation) dsmode = DepthStencilMode.DisableAll;
    ...
    {
        var cred = (uint)new Color4(1.0f, 0.0f, 0.0f, 1.0f).ToRgba();
        var cgrn = (uint)new Color4(0.0f, 1.0f, 0.0f, 1.0f).ToRgba();
        var cblu = (uint)new Color4(0.0f, 0.0f, 1.0f, 1.0f).ToRgba();

        var pos = ent.WidgetPosition;
        var ori = ent.WidgetOrientation;

        var pxsize = 120.0f;
        var sssize = pxsize / camera.Height;
        var dist = (pos - camera.Position).Length();
        var size = sssize * dist;
        if (camera.IsMapView || camera.IsOrthographic) size = sssize * camera.OrthographicSize;
        var rad = size * 0.066f;
    ...
    public void RenderMouseHit(BoundsShaderMode mode, ref Vector3 camrel, ref Vector3 bbmin, ref Vector3 bbmax,
        ref Vector3 scale, ref Quaternion ori, float bsphrad)
    {
    ...
    public void RenderSelectionArrowOutline(Vector3 pos, Vector3 dir, Vector3 up, Quaternion ori, float len, float rad,
        uint colour)
    {
        var ax = Vector3.Cross(dir, up);
        var sx = ax * rad;
        var sy = up * rad;
        var sz = dir * len;
        var c = new VertexTypePC[8];
        var d0 = -sx - sy;
        var d1 = -sx + sy;
        var d2 = +sx - sy;
        var d3 = +sx + sy;
    ...
    {
        var res = (int)(angle * 0.1f);
        const float DegToRad = 0.0174533f;
        var ang = DegToRad * angle / res;
    ...
    public void RenderSelectionCone(Vector3 position, Vector3 ax, Vector3 ay, Vector3 dir, float radius, float height,
        uint col)
    {
        const int Reso = 36;
        const float MaxDeg = 360f;
    ...
    public void RenderSelectionCapsule(Vector3 position, Vector3 ax, Vector3 ay, Vector3 dir, float radius,
        float height, uint col)
    {
    ...
    public void RenderSelectionDrawableLight(LightAttributes light, Bone bone)
    {
        ...
        switch (type)
        {
            case LightType.Point:
                ...
                break;
            case LightType.Spot:
                RenderSelectionCone(pos, tx, ty, dir, (float)Math.Sin(outerAngle) * extent,
                    (float)Math.Cos(outerAngle) * extent, colblu);
                RenderSelectionCone(pos, tx, ty, dir, (float)Math.Sin(innerAngle) * extent,
                    (float)Math.Cos(innerAngle) * extent, colwht);
                break;
    ...
    public void RenderSelectionNavPolyOutline(YnvPoly poly, uint colourval)
    {
        ...
        for (int id = startid; id < endid; id++)
        {
            ...
            if (id == startid)
                ...
                SelectionLineVerts.Add(v);
            if (id == lastid) SelectionLineVerts.Add(v0);
        }
    ...
    public void RenderSelectionCollisionPolyOutline(BoundPolygon poly, uint colourval, YmapEntityDef entity)
    {
        ...
        else if (poly is BoundPolygonSphere psph)
        {
            ...
            RenderSelectionCircle(p1, psph.sphereRadius * 1.03f,
                colourval); //enlarge the circle to make it more visible..
        }
    ...
    public void RenderSelectionGeometry(MapSelectionMode mode)
    {
        ...
        var coloursel = new Vector3(0, 1, 0) * globalLights.HdrIntensity * 5.0f;
        var colourwht = new Vector3(1, 1, 1) * globalLights.HdrIntensity * 10.0f;
        ...
        if (WhiteBoxes.Count > 0 || SelectionBoxes.Count > 0)
        {
            ...
            for (var i = 0; i < WhiteBoxes.Count; i++)
            {
                var mb = WhiteBoxes[i];
                shader.SetBoxVars(context, mb.CamRelPos, mb.BBMin, mb.BBMax, mb.Orientation, mb.Scale);
                shader.DrawBox(context);
        ...
        if (WhiteSpheres.Count > 0 || SelectionSpheres.Count > 0)
        {
            ...
            shader.SetSceneVars(context, camera, null, globalLights);
            shader.SetColourVars(context, new Vector4(coloursel, 1));
            for (var i = 0; i < SelectionSpheres.Count; i++)
            {
                var ms = SelectionSpheres[i];
                shader.SetSphereVars(context, ms.CamRelPos, ms.Radius);
                shader.DrawSphere(context);
            }

            shader.SetColourVars(context, new Vector4(colourwht, 1));
            for (var i = 0; i < WhiteSpheres.Count; i++)
            {
                var ms = WhiteSpheres[i];
                shader.SetSphereVars(context, ms.CamRelPos, ms.Radius);
                shader.DrawSphere(context);
            }
    ...
    public void RenderBounds(MapSelectionMode mode)
    {
        ...
        var colour = new Vector3(0, 0, 1) * globalLights.HdrIntensity;
        var colourhi = new Vector3(0, 1, 1) * globalLights.HdrIntensity;
        ...
        if (BoundingBoxes.Count > 0 || HilightBoxes.Count > 0)
        {
            ...
            {
                var mb = BoundingBoxes[i];
                shader.SetBoxVars(context, mb.CamRelPos, mb.BBMin, mb.BBMax, mb.Orientation, mb.Scale);
                shader.DrawBox(context);
            }

            shader.SetColourVars(context, new Vector4(colourhi, 1));
            for (var i = 0; i < HilightBoxes.Count; i++)
            {
                var mb = HilightBoxes[i];
                shader.SetBoxVars(context, mb.CamRelPos, mb.BBMin, mb.BBMax, mb.Orientation, mb.Scale);
                shader.DrawBox(context);
            }
        }
        ...
        if (BoundingSpheres.Count > 0 || HilightSpheres.Count > 0)
        {
            ...
            for (var i = 0; i < BoundingSpheres.Count; i++)
            {
                var ms = BoundingSpheres[i];
                shader.SetSphereVars(context, ms.CamRelPos, ms.Radius);
                shader.DrawSphere(context);
            }

            shader.SetColourVars(context, new Vector4(colourhi, 1));
            for (var i = 0; i < HilightSpheres.Count; i++)
            {
                var ms = HilightSpheres[i];
                shader.SetSphereVars(context, ms.CamRelPos, ms.Radius);
                shader.DrawSphere(context);
            }
        }
    ...
    {
        var item = new RenderSkeletonItem();
    ...
    private void RenderSkeletons()
    {
        ...
        var vr = new VertexTypePC();
        var vg = new VertexTypePC();
        var vb = new VertexTypePC();
        vr.Colour = cred;
        vg.Colour = cgrn;
        vb.Colour = cblu;
        ...
        foreach (var item in renderskeletonlist)
        {
            ...
            var drawable = item.Renderable.Key;
            var skeleton = drawable?.Skeleton;
            if (skeleton == null) continue;

            var campos = camera.Position - (entity?.Position ?? Vector3.Zero);
            ...
            var cnt = Math.Min(pinds.Length, bones.Length);
            for (var i = 0; i < cnt; i++)
            {
                var pind = pinds[i];
                var bone = bones[i];
                ...
                var lbeg = Vector3.Zero;
                var lend = bone.AnimTranslation; // bone.Rotation.Multiply();

                var starsize = (bone.AnimTransform.TranslationVector - campos).Length() * 0.011f;
                Vector3[] starverts0 = { Vector3.UnitX * starsize, Vector3.UnitY * starsize, Vector3.UnitZ * starsize };
                Vector3[] starverts1 =
                    { Vector3.UnitX * -starsize, Vector3.UnitY * -starsize, Vector3.UnitZ * -starsize };
                for (var j = 0; j < 3; j++) starverts0[j] = bone.AnimTransform.MultiplyW(starverts0[j]);
                for (var j = 0; j < 3; j++) starverts1[j] = bone.AnimTransform.MultiplyW(starverts1[j]);
                ...
                if (entity != null)
                {
                    ...
                    for (var j = 0; j < 3; j++)
                        starverts0[j] = entity.Position + entity.Orientation.Multiply(starverts0[j] * entity.Scale);
                    for (var j = 0; j < 3; j++)
                        starverts1[j] = entity.Position + entity.Orientation.Multiply(starverts1[j] * entity.Scale);
                }

                vr.Position = starverts0[0];
                skeletonLineVerts.Add(vr);
                vr.Position = starverts1[0];
                skeletonLineVerts.Add(vr);
                vg.Position = starverts0[1];
                skeletonLineVerts.Add(vg);
                vg.Position = starverts1[1];
                skeletonLineVerts.Add(vg);
                vb.Position = starverts0[2];
                skeletonLineVerts.Add(vb);
                vb.Position = starverts1[2];
                skeletonLineVerts.Add(vb);
        ...
        if (skeletonLineVerts.Count > 0) RenderLines(skeletonLineVerts, DepthStencilMode.DisableAll);
    }
    ...
    private void RenderSky()
    {
        ...
        Texture moon = null;
        var skydomeytd = gameFileCache.GetYtd(2640562617); //skydome hash
        if (skydomeytd != null && skydomeytd.Loaded && skydomeytd.TextureDict != null &&
            skydomeytd.TextureDict.Dict != null)
        {
            skydomeytd.TextureDict.Dict.TryGetValue(1064311147, out starfield); //starfield hash

            if (rendermoon) skydomeytd.TextureDict.Dict.TryGetValue(234339206, out moon); //moon-new hash
        }

        Renderable sdrnd = null;
        if (skydomeydr != null) sdrnd = RenderableCache.GetRenderable(skydomeydr);

        RenderableTexture sftex = null;
        if (starfield != null) sftex = RenderableCache.GetRenderableTexture(starfield);

        RenderableTexture moontex = null;
        if (moon != null) moontex = RenderableCache.GetRenderableTexture(moon);
        ...
        if (sdrnd != null && sdrnd.IsLoaded && sftex != null && sftex.IsLoaded)
        {
            ...
            var rmod = sdrnd.HDModels != null && sdrnd.HDModels.Length > 0 ? sdrnd.HDModels[0] : null;
            var rgeom = rmod != null && rmod.Geometries != null && rmod.Geometries.Length > 0
                ? rmod.Geometries[0]
                : null;
    ...
    private void RenderClouds()
    {
        ...
        if (!string.IsNullOrEmpty(individualcloudfrag))
        {
            ...
            var frag = clouds.HatManager.FindFrag(individualcloudfrag);
            if (frag == null) return;

            for (var i = 0; i < frag.Layers.Length; i++)
            {
                var layer = frag.Layers[i];
                var dhash = JenkHash.GenHash(layer.Filename.ToLowerInvariant());
                ...
                if (rnd == null || rnd.IsLoaded == false || rnd.AllTexturesLoaded == false) continue;
                ...
                for (var mi = 0; mi < rnd.HDModels.Length; mi++)
                {
                    var model = rnd.HDModels[mi];

                    for (var gi = 0; gi < model.Geometries.Length; gi++)
                    {
    ...
    public void RenderWaterQuads(List<WaterQuad> waterquads)
    {
        ...
        {
            var rquad = RenderableCache.GetRenderableWaterQuad(quad);
    ...
    public void RenderPaths(List<YndFile> ynds)
    {
        ...
        {
            var rnd = RenderableCache.GetRenderablePathBatch(ynd);
            if (rnd != null && rnd.IsLoaded) shaders.Enqueue(rnd);
        }
    }
    ...
    public void RenderTrainTracks(List<TrainTrack> tracks)
    {
        ...
        {
            var rnd = RenderableCache.GetRenderablePathBatch(track);
            if (rnd != null && rnd.IsLoaded) shaders.Enqueue(rnd);
        }
    }
    ...
    public void RenderNavMeshes(List<YnvFile> ynvs)
    {
        ...
        {
            var rnd = RenderableCache.GetRenderablePathBatch(ynv);
            if (rnd != null && rnd.IsLoaded) shaders.Enqueue(rnd);
        }
    }
    ...
    {
        var rnd = RenderableCache.GetRenderablePathBatch(basepath);
        if (rnd != null && rnd.IsLoaded) shaders.Enqueue(rnd);
    }
    ...
    public void RenderScenarios(List<YmtFile> ymts)
    {
        ...
        {
            var rnd = RenderableCache.GetRenderablePathBatch(scenario.ScenarioRegion);
            if (rnd != null && rnd.IsLoaded) shaders.Enqueue(rnd);
        }
    }
    ...
    {
        var rnd = RenderableCache.GetRenderablePathBatch(zones);
        if (rnd != null && rnd.IsLoaded) shaders.Enqueue(rnd);
    }
    ...
    public void RenderWorld(Dictionary<MetaHash, YmapFile> renderworldVisibleYmapDict, IEnumerable<Entity> spaceEnts)
    {
        ...
        foreach (var ymap in renderworldVisibleYmapDict.Values.Where(ymap => RenderWorldYmapIsVisible(ymap)))
        {
        ...
        foreach (var ent in ents.Where(RenderIsEntityFinalRender))
        {
            if (ent.IsMlo)
            {
                if (!renderinteriors || ent.MloInstance == null || MapViewEnabled) continue; //render Mlo child entities...
                VisibleMlos.Add(ent);
            ...
            else
            {
                ...
                if (rndbl != null) RenderEntities.Add(ent);

                var pent = ent.Parent;
                if (!waitforchildrentoload || pent == null) continue;
                if (RequiredParents.ContainsKey(pent)) continue;
                var allok = true;
                ...
                if (!allok)
                {
                    ...
                    if (rndbl != null) RenderEntities.Add(pent);
                }
        ...
        if (renderentities)
        {
            foreach (var ent in renderworldentities)
            {
                var rndbl = GetArchetypeRenderable(ent.Archetype);
                ent.LodManagerRenderable = rndbl;
                if (rndbl != null) RenderEntities.Add(ent);
            ...
            foreach (var ent in RenderEntities)
            {
                if (ent.LodManagerRenderable is not Renderable rndbl) continue;
    ...
    public void RenderWorld_Orig(Dictionary<MetaHash, YmapFile> renderworldVisibleYmapDict,
        IEnumerable<Entity> spaceEnts)
    {
        renderworldentities.Clear();
        renderworldrenderables.Clear();
        VisibleYmaps.Clear();
        ...
        for (var y = 0; y < VisibleYmaps.Count; y++)
        {
            ...
            if (ymap.RootEntities != null)
                for (var i = 0; i < ymap.RootEntities.Length; i++)
                {
                    var ent = ymap.RootEntities[i];
                    var pind = ent._CEntityDef.parentIndex;
        ...
        if (renderentities)
        {
            for (var i = 0; i < renderworldentities.Count; i++)
            {
            ...
            for (var i = 0; i < renderworldrenderables.Count; i++)
            {
    ...
    {
        var dist = (ent.Position - camera.Position).Length();

        if (MapViewEnabled) dist = camera.OrthographicSize / MapViewDetail;
        ...
        {
            if (ent.Archetype != null) loddist = ent.Archetype.LodDist * renderworldLodDistMult;
        }
        ...
        if (cloddist <= 0)
        {
            if (ent.Archetype != null) cloddist = ent.Archetype.LodDist * renderworldLodDistMult;
        }
        else
        {
        ...
        if (renderworldMaxLOD != rage__eLodType.LODTYPES_DEPTH_ORPHANHD)
        {
            ...
            if (ent._CEntityDef.lodLevel == renderworldMaxLOD) ent.ChildrenVisible = false;
        }
    }
        ...
        if (ent.ChildrenVisible)
            if (ent.Children != null)
                for (var i = 0; i < ent.Children.Length; i++)
                {
                    ...
                    if (child.Ymap == ent.Ymap) RenderWorldRecurseCalcEntityVisibility(child);
                }
    }

    private void RenderWorldRecurseAddEntities(YmapEntityDef ent)
    {
        var hide = ent.ChildrenVisible;
        var force = ent.Parent != null && ent.Parent.ChildrenVisible && !hide;
        if (force || (ent.IsVisible && !hide))
            if (ent.Archetype != null)
            {
                ...
                if (!camera.ViewFrustum.ContainsAABBNoClip(ref ent.BBCenter, ref ent.BBExtent)) return;
        ...
        if (ent.IsVisible && ent.ChildrenVisible && ent.Children != null)
            ...
            {
                var child = ent.Children[i];
                if (child.Ymap == ent.Ymap) RenderWorldRecurseAddEntities(ent.Children[i]);
            }
    }
    ...
    private void RenderWorldAddInteriorEntities(YmapEntityDef ent)
    {
        if (ent?.MloInstance?.Entities != null)
            for (var j = 0; j < ent.MloInstance.Entities.Length; j++)
            {
                ...
                if (!camera.ViewFrustum.ContainsAABBNoClip(ref intent.BBCenter,
                        ref intent.BBExtent)) continue; //frustum cull interior ents

                renderworldentities.Add(intent);
            }

        if (ent?.MloInstance?.EntitySets != null)
            for (var e = 0; e < ent.MloInstance.EntitySets.Length; e++)
            {
    ...
    private void RenderWorldAdjustMapViewCamera()
    {
        if (MapViewEnabled)
        {
            ...
            var cvwidth = camera.OrthographicSize * camera.AspectRatio * 0.5f;
            var cvheight = camera.OrthographicSize * 0.5f;
            var cvwmin = camera.Position.X - cvwidth; //TODO:make all these vars global...
            var cvwmax = camera.Position.X + cvwidth;
            var cvhmin = camera.Position.Y - cvheight;
            var cvhmax = camera.Position.Y + cvheight;

            for (var y = 0; y < VisibleYmaps.Count; y++)
            {
                var ymap = VisibleYmaps[y];
                if (ymap.AllEntities != null)
                    for (var i = 0; i < ymap.AllEntities.Length; i++) //this is bad
                    {
                        var ent = ymap.AllEntities[i];
                        if (ent.Position.Z < 1000.0f && ent.BSRadius < 500.0f)
                            if (ent.BBMax.X > cvwmin && ent.BBMin.X < cvwmax && ent.BBMax.Y > cvhmin &&
                                ent.BBMin.Y < cvhmax)
    ...
    private void RenderWorldYmapExtras()
    {
        if (rendercars)
            ...
            {
                var ymap = VisibleYmaps[y];
                if (ymap.CarGenerators != null) RenderYmapCarGenerators(ymap);
            }
        ...
        if (rendergrass && renderworldMaxLOD == rage__eLodType.LODTYPES_DEPTH_ORPHANHD) //hide grass with orphans
            ...
            {
                var ymap = VisibleYmaps[y];
                if (ymap.GrassInstanceBatches != null) RenderYmapGrass(ymap);
            }
        ...
        if (renderdistlodlights && timecycle.IsNightTime)
            ...
            {
                var ymap = VisibleYmaps[y];
                if (ymap.DistantLODLights != null) RenderYmapDistantLODLights(ymap);
            }
        ...
        if (renderlodlights && shaders.deferred)
            ...
            {
                var ymap = VisibleYmaps[y];
                if (ymap.LODLights != null) RenderYmapLODLights(ymap);
            }
    }
    ...
    private bool RenderIsEntityFinalRender(YmapEntityDef ent)
    {
        ...
        var isshadowproxy = false;
        var isreflproxy = false;
        var archflags = arch._BaseArchetypeDef.flags;
        ...
        ////}
        if ((archflags & 2048) > 0) isshadowproxy = true;
        ...
        if (isshadowproxy || isreflproxy) return renderproxies; //filter out proxy entities...
        return true;
    }
    ...
    private Renderable GetArchetypeRenderable(Archetype arch)
    {
        ...
        if (rndbl != null && rndbl.IsLoaded && (rndbl.AllTexturesLoaded || !waitforchildrentoload)) return rndbl;
        return null;
    }
    ...
    public void RenderYmap(YmapFile ymap)
    {
        ...
        if (ymap.AllEntities != null && ymap.RootEntities != null)
        {
            ...
            else
            {
                ...
                for (var i = 0; i < ents.Length; i++)
                {
                    ...
                    if (arch != null)
                    {
                        var timed = arch.Type == MetaName.CTimeArchetypeDef;
        ...
        if (rendercars && ymap.CarGenerators != null) RenderYmapCarGenerators(ymap);
        if (rendergrass && ymap.GrassInstanceBatches != null) RenderYmapGrass(ymap);
    ...
    private bool RenderYmapLOD(YmapFile ymap, YmapEntityDef entity)
    {
        ...
        if (arch != null)
        {
            ...
            {
                var usechild = false;
                var camrel = entity.Position - camera.Position;
                var dist = (camrel + entity.BSCenter).Length();
                entity.Distance = dist;
                var rad = arch.BSRadius;
                var loddist = entity._CEntityDef.lodDist;
                if (loddist < 1.0f) loddist = 200.0f;
                var mindist = Math.Max(dist - rad, 1.0f) * lodthreshold;
                if (mindist < loddist)
                {
                    //recurse...
                        ...
                        for (var i = 0; i < children.Length; i++)
                        {
                            ...
                            if (!RenderYmapLOD(childe.Ymap, childe))
                                if (waitforchildrentoload)
                                    usechild =
                                        false; //might cause some overlapping, but should reduce things disappearing
                        }
                    }

                    if (!entity.ChildrenRendered) entity.ChildrenRendered = usechild;
                }
                else
                {
                ...
                if (!usechild && !entity.ChildrenRendered)
                {
                    if (renderinteriors && entity.IsMlo) //render Mlo child entities...
                        if (entity.MloInstance != null && entity.MloInstance.Entities != null)
                            for (var j = 0; j < entity.MloInstance.Entities.Length; j++)
                            {
    ...
    private void RenderYmapGrass(YmapFile ymap)
    {
        ...
        foreach (var batch in ymap.GrassInstanceBatches)
        {
            ...
            var lodDist = batch.Batch.lodDist * renderworldDetailDistMult; //maybe add grass dist mult
            ...
            var bsrad = batch.Radius;
            if (!camera.ViewFrustum.ContainsSphereNoClipNoOpt(ref bscent,
                    bsrad)) continue; //frustum cull grass batches...
            ...
            var binst = new RenderableInstanceBatchInst();
            binst.Batch = instb;
    ...
    private void RenderYmapLODLights(YmapFile ymap)
    {
        ...
        var lights = RenderableCache.GetRenderableLODLights(ymap);
    ...
    private void RenderYmapDistantLODLights(YmapFile ymap)
    {
        ...
        var lights = RenderableCache.GetRenderableDistantLODLights(ymap.DistantLODLights);
        if (!lights.IsLoaded) return;


        var ytdhash = 3154743001; //"graphics"
        var texhash = 2236244673; //"distant_light"
        var graphicsytd = gameFileCache.GetYtd(ytdhash);
        Texture lighttex = null;
        if (graphicsytd != null && graphicsytd.Loaded && graphicsytd.TextureDict != null &&
            graphicsytd.TextureDict.Dict != null) graphicsytd.TextureDict.Dict.TryGetValue(texhash, out lighttex);

        if (lighttex == null) return;
        RenderableTexture lightrtex = null;
        if (lighttex != null) lightrtex = RenderableCache.GetRenderableTexture(lighttex);
    ...
    private void RenderYmapCarGenerators(YmapFile ymap)
    {
        ...
        for (var i = 0; i < ymap.CarGenerators.Length; i++)
        {
            var cg = ymap.CarGenerators[i];

            var bscent = cg.Position - camera.Position;
            var bsrad = cg._CCarGen.perpendicularLength;
            if (bscent.LengthSquared() > maxdist2) continue; //don't render distant cars..
            if (!camera.ViewFrustum.ContainsSphereNoClipNoOpt(ref bscent, bsrad)) continue; //frustum cull cars...
            ...
            var cgori = Quaternion.Multiply(cg.Orientation, cgtrn);
        ...
        ClipMapEntry animClip = null)
    {
        ...
        if (f.PhysicsLODGroup != null && f.PhysicsLODGroup.PhysicsLOD1 != null)
        {
            ...
            {
                for (var i = 0; i < pl1.Children.data_items.Length; i++)
                {
                        ...
                        if (dwblcopy != null)
                        {
                            if (dwbl != null)
                            {
                                if (dwbl != dwblcopy && dwbl.AllModels.Length == 0)
                                {
                                    ...
                                    dwbl.AllModels =
                                        dwblcopy
                                            .AllModels; //hopefully this is all that's need to render, otherwise drawable is actually getting edited!
        ...
        var isselected = SelectionFlagsTestAll || f.Drawable == SelectedDrawable;
        if (isselected)
        {
        ...
        if (renderfragwindows)
        {
            ...
            if (f.GlassWindows?.data_items != null)
                for (var i = 0; i < f.GlassWindows.data_items.Length; i++)
                {
            ...
            if (f.VehicleGlassWindows?.Windows != null)
                for (var i = 0; i < f.VehicleGlassWindows.Windows.Length; i++)
                {
        ...
        ClipMapEntry animClip = null)
    {
        ...
        var entpos = entity != null ? entity.Position : Vector3.Zero;
        var camrel = entpos - camera.Position;
        ...
        var bsrad = arche.BSRadius; // * scale;
        if (cull)
        ...
        if (rndbl != null)
        {
            ...
            if (fd != null)
            {
                ...
                if (frag != null && frag.DrawableCloth != null) //cloth...
                {
                    ...
                    if (rndbl != null)
                    {
                        var res2 = RenderRenderable(rndbl, arche, entity);
                        res = res || res2;
                    }
                }
            }
    ...
    public bool RenderDrawable(DrawableBase drawable, Archetype arche, YmapEntityDef entity, uint txdHash = 0,
        TextureDictionary txdExtra = null, Texture diffOverride = null, ClipMapEntry animClip = null,
        ClothInstance cloth = null, Expression expr = null)
    {
        ...
        var rndbl = TryGetRenderable(arche, drawable, txdHash, txdExtra, diffOverride);
        if (rndbl == null)
            return false;
    ...
    private bool RenderRenderable(Renderable rndbl, Archetype arche, YmapEntityDef entity)
    {
        ...
        var isselected = SelectionFlagsTestAll || rndbl.Key == SelectedDrawable;
        ...
        var bbmin = arche != null ? arche.BBMin : rndbl.Key.BoundingBoxMin;
        var bbmax = arche != null ? arche.BBMax : rndbl.Key.BoundingBoxMax;
        var bscen = arche != null ? arche.BSCenter : rndbl.Key.BoundingCenter;
        var radius = arche != null ? arche.BSRadius : rndbl.Key.BoundingSphereRadius;
        float distance = 0; // (camrel + bscen).Length();
        var interiorent = false;
        var castshadow = true;
        ...
        if (entity != null)
        {
            ...
            castshadow =
                entity.MloParent == null; //don't cast sun/moon shadows if this is an interior entity - optimisation!
        ...
        if (rndbl.HasAnims) rndbl.UpdateAnims(currentRealTime);
        if (rndbl.Cloth != null) rndbl.Cloth.Update(currentRealTime);
        ...
        if ((rendercollisionmeshes || SelectionMode == MapSelectionMode.Collision) && rendercollisionmeshlayerdrawable)
            ...
            {
                var sdrawable = rndbl.Key as Drawable;
                if (sdrawable != null && sdrawable.Bound != null) RenderCollisionMesh(sdrawable.Bound, entity);
                var fdrawable = rndbl.Key as FragDrawable;
                if (fdrawable != null)
                {
                    if (fdrawable.Bound != null) RenderCollisionMesh(fdrawable.Bound, entity);
        ...
        if (renderskeletons && rndbl.HasSkeleton) RenderSkeleton(rndbl, entity);
        ...
        if (renderlights && shaders.deferred && rndbl.Lights != null)
        {
            ...
            if (lights != null && lights.Length != rndbl.Lights.Length) rndbl.InitLights(lights);


            var linst = new RenderableLightInst();
            for (var i = 0; i < rndbl.Lights.Length; i++)
            {
                var rndlight = rndbl.Lights[i];
                var light = rndlight.OwnerLight;

                if (light.UpdateRenderable)
                {
                    rndlight.Init(light);
        ...
        var retval = true; // false;
        if (rndbl.AllTexturesLoaded || !waitforchildrentoload)
        {
            var rginst = new RenderableGeometryInst();
            ...
            var models = isselected ? rndbl.AllModels : rndbl.HDModels;
            ...
            for (var mi = 0; mi < models.Length; mi++)
            {
                ...
                if (!RenderIsModelFinalRender(model) &&
                    !renderproxies) continue; //filter out reflection proxy models...
    ...
    public void RenderCar(Vector3 pos, Quaternion ori, MetaHash modelHash, MetaHash modelSetHash, bool valign = false)
    {
        ...
        var caryft = gameFileCache.GetYft(carhash);
        if (caryft != null && caryft.Loaded && caryft.Fragment != null)
        {
            if (valign)
            {
                var minz = caryft.Fragment.PhysicsLODGroup?.PhysicsLOD1?.Bound?.BoxMin.Z ?? 0.0f;
                pos.Z -= minz;
            }
    ...
    {
        var yft = ped.Yft; // GameFileCache.GetYft(SelectedModelHash);
        if (yft != null)
        {
            if (yft.Loaded)
    ...
    private void RenderPedComponent(Ped ped, int i)
    {
        ...
        var ac = ped.AnimClip;
        if (ac != null) ac.EnableRootMotion = ped.EnableRootMotion;
        ...
        if (skel != null)
        {
            ...
            {
                var dskel = drawable
                    .Skeleton; //put the bones of the fragment into the drawable. drawable's bones in this case seem messed up!
                if (skel.Bones?.Items != null)
                {
                    for (var b = 0; b < skel.Bones.Items.Length; b++)
                    {
                    ...
                    dskel.BonesSorted =
                        skel.BonesSorted; //this is pretty hacky. TODO: try and fix all this! animate only the frag skeleton!
                }
            }
        }
        ...
        var drawFlag = true;
        if (!SelectionDrawableDrawFlags.TryGetValue(drawable, out drawFlag)) drawFlag = true;

        if (drawFlag) RenderDrawable(drawable, null, ped.RenderEntity, 0, td, texture, ac, cloth, expr);
    }
    ...
    public void RenderCollisionMesh(Bounds bounds, YmapEntityDef entity)
    {
        ...
        var rndbc = RenderableCache.GetRenderableBoundComp(bounds);
        if (rndbc != null && rndbc.IsLoaded)
        {
            var rbginst = new RenderableBoundGeometryInst();
    ...
    private Renderable TryGetRenderable(Archetype arche, DrawableBase drawable, uint txdHash = 0,
        TextureDictionary txdExtra = null, Texture diffOverride = null)
    {
            ...
            if (ycd != null && ycd.Loaded)
            {
                ...
                foreach (var model in rndbl.HDModels)
                {
                    ...
                    foreach (var geom in model.Geometries)
                    {
                        ...
                        {
                            var cmeindex = geom.DrawableGeom.ShaderID + 1u;
                            MetaHash
                                cmehash = ahash + cmeindex; //this goes to at least uv5! (from uv0) - see hw1_09.ycd
        ...
        var cacheSD = rndbl.SDtxds == null;
        var cacheHD = renderhdtextures && rndbl.HDtxds == null;
        if (cacheSD || cacheHD)
        {
            ...
            if (cacheHD && arche != null) //try get HD txd for the asset
            {
                ...
                if (hdtxd != arche._BaseArchetypeDef.assetName)
                {
                    ...
                    if (asshdytd != null) tryGetRenderableHDtxds.Add(asshdytd);
                }
            }
            ...
            if (texDict != 0)
            {
                if (cacheSD)
                {
                    ...
                    if (txdytd != null) tryGetRenderableSDtxds.Add(txdytd);
                }
                ...
                if (cacheHD)
                {
                    ...
                    if (hdtxd != texDict)
                    {
                        ...
                        if (txdhdytd != null) tryGetRenderableHDtxds.Add(txdhdytd);
                    }
                }
                ...
                while (ptxdname != 0) //look for parent HD txds
                {
                    if (cacheSD)
                    {
                        ...
                        if (pytd != null) tryGetRenderableSDtxds.Add(pytd);
                    }
                    ...
                    if (cacheHD)
                    {
                        ...
                        if (phdtxdname != ptxdname)
                        {
                            ...
                            if (phdytd != null) tryGetRenderableHDtxds.Add(phdytd);
                        }
                    }
        ...
        var alltexsloaded = true;


        for (var mi = 0; mi < rndbl.AllModels.Length; mi++)
        {
            var model = rndbl.AllModels[mi];

            if (!RenderIsModelFinalRender(model) && !renderproxies) continue; //filter out reflection proxy models...


            foreach (var geom in model.Geometries)
                if (geom.Textures != null)
                    for (var i = 0; i < geom.Textures.Length; i++)
                    {
                        if (diffOverride != null)
                        {
                            var texParamHash = i < geom.TextureParamHashes?.Length ? geom.TextureParamHashes[i] : 0;
                            if (texParamHash == ShaderParamNames.DiffuseSampler) geom.Textures[i] = diffOverride;
                        }
                        ...
                        if (tex != null && ttex == null)
                        {
                            ...
                            if (dtex == null) //else //if (texDict != 0)
                            {
                                ...
                                if (rndbl.SDtxds != null)
                                {
                                    ...
                                    for (var j = 0; j < rndbl.SDtxds.Length; j++)
                                    {
                                        ...
                                        {
                                            txd = gameFileCache.GetYtd(txd.Key
                                                .Hash); //keep trying to load it - sometimes resuests can get lost (!)
                                            waitingforload = true;
                                        }
                                    ...
                                    if (waitingforload) alltexsloaded = false;
                                }
                                ...
                                if (dtex == null && !waitingforload)
                                {
                                    ...
                                    if (ytd2 != null)
                                    {
                                        if (ytd2.Loaded)
                                        {
                                            if (ytd2.TextureDict != null) dtex = ytd2.TextureDict.Lookup(tex.NameHash);
                                        }
                                        else
                                        {
                                            alltexsloaded = false;
                                    ...
                                    if (dtex == null && ytd2 == null) // rndbl.SDtxds.Length == 0)//texture not found..
                                        if (drawable.ShaderGroup?.TextureDictionary !=
                                            null) //check any embedded texdict
                                        {
                        ...
                        if (renderhdtextures)
                        {
                            var hdtex = geom.TexturesHD[i];
                            if (hdtex == null)
                            {
                                //look for a replacement HD texture...
                                if (rndbl.HDtxds != null)
                                    for (var j = 0; j < rndbl.HDtxds.Length; j++)
                                    {
                                ...
                                if (hdtex != null) geom.TexturesHD[i] = hdtex;
                            }

                            if (hdtex != null) rhdtex = RenderableCache.GetRenderableTexture(hdtex);
                        }
...
public class RenderLodManager
{
    ...
    private readonly List<MetaHash> RemoveYmaps = new();
    ...
    public void Update(Dictionary<MetaHash, YmapFile> ymaps, Camera camera, float elapsed)
    {
        ...
        foreach (var kvp in ymaps)
        {
            ...
            {
                ymaps.TryGetValue(ymap._CMapData.parent, out var pymap);
        ...
        foreach (var kvp in CurrentYmaps)
        {
            ...
            if (!ymaps.TryGetValue(kvp.Key, out ymap) || ymap != kvp.Value || (ymap.IsScripted && !ShowScriptedYmaps) ||
                ymap.LodManagerUpdate) RemoveYmaps.Add(kvp.Key);
        }
        ...
        foreach (var remYmap in RemoveYmaps)
        {
            ...
            if (remEnts != null) // remove this ymap's entities from the tree.....
                for (var i = 0; i < remEnts.Length; i++)
                {
                    var ent = remEnts[i];
                    ...
                    ent.LodManagerRenderable = null;
                    if (ent.Parent != null && ent.Parent.Ymap != ymap) ent.Parent.LodManagerRemoveChild(ent);
                }

            var remLodLights = ymap.LODLights?.LodLights;
            if (remLodLights != null)
                for (var i = 0; i < remLodLights.Length; i++)
        ...
        {
            var ymap = kvp.Value;
            if (ymap.IsScripted && !ShowScriptedYmaps) continue;
            ...
            if (!CurrentYmaps.ContainsKey(kvp.Key))
            {
                ...
                if (ymap.AllEntities != null) // add this ymap's entities to the tree...
                    for (var i = 0; i < ymap.AllEntities.Length; i++)
                    {
                        var ent = ymap.AllEntities[i];
                        if (ent.Parent != null)
                ...
                if (addLodLights != null)
                    for (var i = 0; i < addLodLights.Length; i++)
                    {
        ...
        foreach (var kvp in RootEntities)
        {
            ...
            if (EntityVisibleAtMaxLodLevel(ent))
            {
                ...
                if (ent.Distance <= ent.LodDist * LodDistMult) RecurseAddVisibleLeaves(ent);
            }
        }
        ...
        else
        {
            if (EntityVisible(ent))
            {
                ...
                if (HDLightsEnabled && ent.Lights != null)
                    for (var i = 0; i < ent.Lights.Length; i++)
    ...
    private LinkedList<YmapEntityDef> GetEntityChildren(YmapEntityDef ent)
    {
        ...
        if (clist != null && clist.Count >= ent._CEntityDef.numChildren)
        {
            if (ent.Parent != null) //already calculated root entities distance
                ent.Distance = MapViewEnabled ? MapViewDist : (ent.Position - Position).Length();
            if (ent.Distance <= ent.ChildLodDist * LodDistMult) return clist;
            ...
            while (cnode != null)
            {
                ...
                if (child.Distance <= child.LodDist * LodDistMult) return clist;
                cnode = cnode.Next;
            }
        }
