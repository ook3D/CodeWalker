...
public class UnitSphere
{
    ...
    public UnitSphere(Device device, byte[] vsbytes, int detail, bool invert = false)
    {
        ...
        var vdict = new Dictionary<Vector3, int>();
        var curtris = new List<SphTri>();
        var nxttris = new List<SphTri>();
        ...
        for (var i = 0; i < verts.Count; i++) vdict[verts[i]] = i;
        ...
        for (var i = 0; i < detail; i++)
        {
            ...
            foreach (var tri in curtris)
            {
                ...
                var v2 = verts[tri.v2];
                var v3 = verts[tri.v3];
                var s1 = Vector3.Normalize(v1 + v2);
                var s2 = Vector3.Normalize(v2 + v3);
                var s3 = Vector3.Normalize(v3 + v1);
                int i1, i2, i3;
        ...
        var vdata = new List<Vector4>();
        foreach (var vert in verts) vdata.Add(new Vector4(vert, 1.0f));
