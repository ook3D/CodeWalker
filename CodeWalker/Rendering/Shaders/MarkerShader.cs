...
public class MarkerShader : Shader, IDisposable
{
    ...
    private readonly PixelShader markerps;

    private readonly VertexShader markervs;

    private SamplerState texsampler;
    private readonly GpuVarsBuffer<MarkerShaderVSMarkerVars> VSMarkerVars;

    private readonly GpuVarsBuffer<MarkerShaderVSSceneVars> VSSceneVars;


    public MarkerShader(Device device)
    {
        var vsbytes = PathUtil.ReadAllBytes("Shaders\\MarkerVS.cso");
        var psbytes = PathUtil.ReadAllBytes("Shaders\\MarkerPS.cso");
        ...
        ShaderGlobalLights lights)
    {
        ...
        VSSceneVars.Vars.ScreenScale =
            new Vector4(0.5f / camera.Width, 0.5f / camera.Height, camera.Width, camera.Height);
