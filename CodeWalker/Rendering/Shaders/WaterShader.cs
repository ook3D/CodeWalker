...
public class WaterShader : Shader
{
    ...
    private readonly PixelShader psdef;
    private readonly GpuVarsBuffer<WaterShaderPSGeomVars> PSGeomVars;
    private readonly GpuVarsBuffer<WaterShaderPSSceneVars> PSSceneVars;
    ...
    private readonly SamplerState texsamplerflow;
    private readonly GpuVarsBuffer<WaterShaderVSEntityVars> VSEntityVars;
    private readonly GpuVarsBuffer<WaterShaderVSGeomVars> VSGeomVars;
    private readonly VertexShader vspct;
    private readonly VertexShader vspnct;
    private readonly VertexShader vspnctx;

    private readonly VertexShader vspt;

    private readonly GpuVarsBuffer<WaterShaderVSSceneVars> VSSceneVars;
    ...
    {
        var vsptbytes = PathUtil.ReadAllBytes("Shaders\\WaterVS_PT.cso");
        var vspctbytes = PathUtil.ReadAllBytes("Shaders\\WaterVS_PCT.cso");
        var vspnctbytes = PathUtil.ReadAllBytes("Shaders\\WaterVS_PNCT.cso");
        var vspnctxbytes = PathUtil.ReadAllBytes("Shaders\\WaterVS_PNCTX.cso");
        var psbytes = PathUtil.ReadAllBytes("Shaders\\WaterPS.cso");
        var psdefbytes = PathUtil.ReadAllBytes("Shaders\\WaterPS_Deferred.cso");
        ...
        ShaderGlobalLights lights)
    {
        ...
        var phspd = 4.0f;
        var phspdi = 1.0f / phspd;
        var phspdh = phspd * 0.5f;
        var t = (float)(CurrentRealTime - Math.Floor(CurrentRealTime * 0.001) * 1000.0) * 1.0f;
        var ta = t * 2.0f;
        var tb = ta + phspdh;
        var t1 = (ta * phspdi - (float)Math.Floor(ta * phspdi)) * phspd;
        var t2 = (tb * phspdi - (float)Math.Floor(tb * phspdi)) * phspd;
        var s1 = (t1 < phspdh ? t1 : phspd - t1) * phspdi * 1.0f;
        var s2 = (t2 < phspdh ? t2 : phspd - t2) * phspdi * 1.0f;
        ...
        var fogtexMin = new Vector2(-4000.0f, -4000.0f); //aka water quads min/max
        var fogtexMax = new Vector2(4500.0f, 8000.0f);
        var fogtexInv = 1.0f / (fogtexMax - fogtexMin);


        var usewaterbumps = waterbump != null && waterbump.ShaderResourceView != null && waterbump2 != null &&
                            waterbump2.ShaderResourceView != null;
        var usefogtex = waterfog != null && waterfog.ShaderResourceView != null;
        ...
        if (usefogtex) context.PixelShader.SetShaderResource(6, waterfog.ShaderResourceView);
    }
    ...
    public override void SetGeomVars(DeviceContext context, RenderableGeometry geom)
    {
        ...
        var useflow = flowtex != null && flowtex.ShaderResourceView != null;
        var usefoam = foamtex != null && foamtex.ShaderResourceView != null;
        var usefog = fogtex != null && fogtex.ShaderResourceView != null;
        ...
        if (usefoam) foamtex.SetPSResource(context, 3);
        if (useflow) flowtex.SetVSResource(context, 0);
    }
