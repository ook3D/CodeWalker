...
public class TreesLodShader : Shader, IDisposable
{
    ...
    private readonly GpuVarsBuffer<TreesLodShaderPSEntityVars> PSEntityVars;
    private readonly GpuVarsBuffer<TreesLodShaderPSSceneVars> PSSceneVars;
    private SamplerState texsampler;

    private readonly VertexShader vs;
    private readonly GpuVarsBuffer<TreesLodShaderVSEntityVars> VSEntityVars;
    private readonly GpuVarsBuffer<TreesLodShaderVSGeometryVars> VSGeomVars;
    private readonly GpuVarsBuffer<TreesLodShaderVSModelVars> VSModelVars;
    private readonly GpuVarsBuffer<TreesLodShaderVSSceneVars> VSSceneVars;


    public TreesLodShader(Device device)
    {
        var vsbytes = PathUtil.ReadAllBytes("Shaders\\TreesLodVS.cso");
        var psbytes = PathUtil.ReadAllBytes("Shaders\\TreesLodPS.cso");
        var psdefbytes = PathUtil.ReadAllBytes("Shaders\\TreesLodPS_Deferred.cso");
    ...
    public override void SetGeomVars(DeviceContext context, RenderableGeometry geom)
    {
            ...
            for (var i = 0; i < nparams; i++)
            {
                ...
                switch (h)
                {
                    case ShaderParamNames.AlphaTest:
                        VSGeomVars.Vars.AlphaTest = (Vector4)sparams[i].Data;
                        break;
                    case ShaderParamNames.AlphaScale:
                        VSGeomVars.Vars.AlphaScale = (Vector4)sparams[i].Data;
                        break;
                    case ShaderParamNames.UseTreeNormals:
                        VSGeomVars.Vars.UseTreeNormals = (Vector4)sparams[i].Data;
                        break;
                    case ShaderParamNames.treeLod2Normal:
                        VSGeomVars.Vars.treeLod2Normal = (Vector4)sparams[i].Data;
                        break;
                    case ShaderParamNames.treeLod2Params:
                        VSGeomVars.Vars.treeLod2Params = (Vector4)sparams[i].Data;
                        break;
                }
            }
        }
        ...
        var usediff = texture != null && texture.Texture2D != null && texture.ShaderResourceView != null;
