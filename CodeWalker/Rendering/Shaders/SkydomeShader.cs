...
public class SkydomeShader : Shader, IDisposable
{
    ...
    private readonly InputLayout moonlayout;
    private readonly PixelShader moonps;
    private readonly UnitQuad moonquad;
    private readonly VertexShader moonvs;
    private readonly GpuVarsBuffer<SkydomeShaderPSSceneVars> PSSceneVars;
    private readonly GpuVarsBuffer<SkydomeShaderPSSunMoonVars> PSSunMoonVars;

    private readonly InputLayout skylayout;

    private readonly GpuVarsBuffer<SkydomeShaderSkySystemLocals> SkyLocalVars;
    private readonly PixelShader skyps;

    private readonly VertexShader skyvs;

    private readonly UnitDisc sundisc;
    private readonly InputLayout sunlayout;
    private readonly PixelShader sunps;
    private readonly VertexShader sunvs;


    private readonly SamplerState texsampler;
    private readonly GpuVarsBuffer<SkydomeShaderVSEntityVars> VSEntityVars;
    private readonly GpuVarsBuffer<SkydomeShaderVSModelVars> VSModelVars;
    private readonly GpuVarsBuffer<SkydomeShaderVSSceneVars> VSSceneVars;

    private readonly GpuVarsBuffer<SkydomeShaderVSSunMoonVars> VSSunMoonVars;


    public SkydomeShader(Device device)
    {
        var skyvsbytes = PathUtil.ReadAllBytes("Shaders\\SkydomeVS.cso");
        var skypsbytes = PathUtil.ReadAllBytes("Shaders\\SkydomePS.cso");
        var sunvsbytes = PathUtil.ReadAllBytes("Shaders\\SkySunVS.cso");
        var sunpsbytes = PathUtil.ReadAllBytes("Shaders\\SkySunPS.cso");
        var moonvsbytes = PathUtil.ReadAllBytes("Shaders\\SkyMoonVS.cso");
        var moonpsbytes = PathUtil.ReadAllBytes("Shaders\\SkyMoonPS.cso");
    ...
    {
        if (type != VertexType.PTT) return false;
    ...
    public void UpdateSkyLocals(Weather weather, ShaderGlobalLights lights)
    {
        ...
        var skyhdr = EnableHDR ? wv.skyHdr : Math.Min(wv.skyHdr, 1.8f);
        var sunhdr = EnableHDR ? wv.skySunHdr : Math.Min(wv.skySunHdr, 4.8f); //NO HDR - turn it down!!!
        var scathdr = EnableHDR ? wv.skySunScatterInten : Math.Min(wv.skySunScatterInten, 1.8f);
        var suncolhdr = wv.skySunCol * sunhdr;
        var azecol = wv.skyAzimuthEastCol;
        var azwcol = wv.skyAzimuthWestCol;
        var aztcol = wv.skyAzimuthTransitionCol;
        var zencol = wv.skyZenithCol;
        var zetcol = wv.skyZenithTransitionCol;
        var plncol = wv.skyPlane;
        ...
        SkyLocalVars.Vars.sunConstants.Y =
            wv.skySunMie.Y; // 1.0f /wv.skySunMie.Y;// 1.7f;// wv.skySunMie.Y; //mie scatter
        SkyLocalVars.Vars.sunConstants.Z =
            0.00003f * wv
                .skySunInfluenceRadius; // * wv.skySunMie.X;// wv.skySunMie.Z;///720.0f;// * 0.01f; // 0.0025f;  //mie size/"range"
    ...
    public void RenderSun(DeviceContext context, Camera camera, Weather weather, ShaderGlobalLights lights)
    {
        ...
        PSSunMoonVars.Vars.Colour =
            new Vector4(weather.CurrentValues.skySunDiscCol * weather.CurrentValues.skySunHdr, 1);
    ...
    public void RenderMoon(DeviceContext context, Camera camera, Weather weather, ShaderGlobalLights lights,
        RenderableTexture moontex)
    {
        context.VertexShader.Set(moonvs);
        context.PixelShader.Set(moonps);


        var ori = Quaternion.Invert(Quaternion.LookAtRH(Vector3.Zero, lights.CurrentMoonDir, lights.MoonAxis));
        var omat = ori.ToMatrix();
        ...
        PSSunMoonVars.Vars.Colour = new Vector4(weather.CurrentValues.skyMoonCol * weather.CurrentValues.skyMoonIten,
            weather.CurrentValues.skyMoonIten);
        PSSunMoonVars.Update(context);
        PSSunMoonVars.SetPSCBuffer(context, 0);
