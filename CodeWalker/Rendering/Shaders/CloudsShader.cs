...
public class CloudsShader : Shader, IDisposable
{
    ...
    private readonly GpuVarsBuffer<CloudsShaderCloudsLocals> CloudsLocalVars;
    private bool disposed;
    private readonly InputLayout layout;
    private readonly PixelShader ps;
    private readonly GpuVarsBuffer<CloudsShaderPSSceneVars> PSSceneVars;

    private readonly SamplerState texsampler;
    private readonly SamplerState texsampleranis;

    private readonly VertexShader vs;
    private readonly GpuVarsBuffer<CloudsShaderVSEntityVars> VSEntityVars;
    private readonly GpuVarsBuffer<CloudsShaderVSModelVars> VSModelVars;
    private readonly GpuVarsBuffer<CloudsShaderVSSceneVars> VSSceneVars;

    public CloudsShader(Device device)
    {
        var vsbytes = PathUtil.ReadAllBytes("Shaders\\CloudsVS.cso");
        var psbytes = PathUtil.ReadAllBytes("Shaders\\CloudsPS.cso");
    ...
    public override void SetGeomVars(DeviceContext context, RenderableGeometry geom)
    {
        ...
        var usedens = DensitySampler != null && DensitySampler.ShaderResourceView != null;
        var usenorm = NormalSampler != null && NormalSampler.ShaderResourceView != null;
        var usedden = DetailDensitySampler != null && DetailDensitySampler.ShaderResourceView != null;
        var usednrm = DetailNormalSampler != null && DetailNormalSampler.ShaderResourceView != null;
        var useddn2 = DetailDensity2Sampler != null && DetailDensity2Sampler.ShaderResourceView != null;
        var usednm2 = DetailNormal2Sampler != null && DetailNormal2Sampler.ShaderResourceView != null;
        var usedept = DepthMapTexSampler != null && DepthMapTexSampler.ShaderResourceView != null;

        if (usedens) DensitySampler.SetPSResource(context, 0);
        if (usenorm) NormalSampler.SetPSResource(context, 1);
        if (usedden) DetailDensitySampler.SetPSResource(context, 2);
        if (usednrm) DetailNormalSampler.SetPSResource(context, 3);
        if (useddn2) DetailDensity2Sampler.SetPSResource(context, 4);
        if (usednm2) DetailNormal2Sampler.SetPSResource(context, 5);
        if (usedept) DepthMapTexSampler.SetPSResource(context, 6);
    ...
    {
        if (type != VertexType.DefaultEx) return false;
