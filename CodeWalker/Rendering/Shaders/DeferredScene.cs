...
public class DeferredScene
{
    ...
    private PixelShader DirLightMSPS;
    private PixelShader DirLightPS;

    private VertexShader DirLightVS;


    private VertexShader FinalVS;

    public GpuMultiTexture GBuffers; // diffuse, normals, specular, irradiance
    private int Height;
    private UnitCapsule LightCapsule;
    private LightCone LightCone;
    private GpuVarsBuffer<DeferredLightInstVars> LightInstVars;
    private PixelShader LightMSPS;
    private PixelShader LightPS;
    private GpuVarsBuffer<DeferredLightPSVars> LightPSVars;
    private UnitQuad LightQuad;
    private InputLayout LightQuadLayout;
    private UnitSphere LightSphere;
    private VertexShader LightVS;


    private GpuVarsBuffer<DeferredLightVSVars> LightVSVars;
    private PixelShader LodLightMSPS;
    private PixelShader LodLightPS;
    private VertexShader LodLightVS;
    ...
    private PixelShader SSAAPS;
    private GpuVarsBuffer<DeferredSSAAPSVars> SSAAPSVars;
    ...
    public DeferredScene(DXManager dxman)
    {
        ...
        var bDirLightVS = PathUtil.ReadAllBytes("Shaders\\DirLightVS.cso");
        var bDirLightPS = PathUtil.ReadAllBytes("Shaders\\DirLightPS.cso");
        var bDirLightMSPS = PathUtil.ReadAllBytes("Shaders\\DirLightPS_MS.cso");
        var bLodLightVS = PathUtil.ReadAllBytes("Shaders\\LodLightsVS.cso");
        var bLodLightPS = PathUtil.ReadAllBytes("Shaders\\LodLightsPS.cso");
        var bLodLightMSPS = PathUtil.ReadAllBytes("Shaders\\LodLightsPS_MS.cso");
        var bLightVS = PathUtil.ReadAllBytes("Shaders\\LightVS.cso");
        var bLightPS = PathUtil.ReadAllBytes("Shaders\\LightPS.cso");
        var bLightMSPS = PathUtil.ReadAllBytes("Shaders\\LightPS_MS.cso");
        var bFinalVS = PathUtil.ReadAllBytes("Shaders\\PPFinalPassVS.cso");
        var bSSAAPS = PathUtil.ReadAllBytes("Shaders\\PPSSAAPS.cso");
        ...
        LightCapsule = new UnitCapsule(device, bLodLightVS, 4);
    ...
    public void OnWindowResize(DXManager dxman)
    {
        ...
        var uw = Width = dxman.backbuffer.Description.Width * SSAASampleCount;
        var uh = Height = dxman.backbuffer.Description.Height * SSAASampleCount;
    ...
    public void RenderLights(DeviceContext context, Camera camera, Shadowmap globalShadows,
        ShaderGlobalLights globalLights)
    {
        ...
        if (globalShadows != null) globalShadows.SetFinalRenderResources(context);
