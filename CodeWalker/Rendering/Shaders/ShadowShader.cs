...
public class ShadowShader : Shader, IDisposable
{
    ...
    private readonly GpuVarsBuffer<ShadowShaderGeomVars> GeomVars;


    private readonly Dictionary<VertexType, InputLayout> layouts = new();
    private readonly PixelShader shadowps;

    private readonly VertexShader shadowvs;
    private readonly VertexShader shadowvs_skin;

    private SamplerState texsampler;
    private SamplerState texsamplerc;
    private readonly GpuVarsBuffer<ShadowShaderVSEntityVars> VSEntityVars;
    private readonly GpuVarsBuffer<ShadowShaderVSModelVars> VSModelVars;

    private readonly GpuVarsBuffer<ShadowShaderVSSceneVars> VSSceneVars;

    public ShadowShader(Device device)
    {
        var vsbytes = PathUtil.ReadAllBytes("Shaders\\ShadowVS.cso");
        var vssbytes = PathUtil.ReadAllBytes("Shaders\\ShadowVS_Skin.cso");
        var psbytes = PathUtil.ReadAllBytes("Shaders\\ShadowPS.cso");
        ...
        layouts.Add(VertexType.DefaultEx,
            new InputLayout(device, vsbytes, VertexTypeGTAV.GetLayout(VertexType.DefaultEx)));
        ...
        layouts.Add(VertexType.PNCCTTTT,
            new InputLayout(device, vsbytes, VertexTypeGTAV.GetLayout(VertexType.PNCCTTTT)));
        ...
        layouts.Add(VertexType.PNCTTTX_2,
            new InputLayout(device, vsbytes, VertexTypeGTAV.GetLayout(VertexType.PNCTTTX_2)));
        layouts.Add(VertexType.PNCTTTX_3,
            new InputLayout(device, vsbytes, VertexTypeGTAV.GetLayout(VertexType.PNCTTTX_3)));
        layouts.Add(VertexType.PNCTTTTX,
            new InputLayout(device, vsbytes, VertexTypeGTAV.GetLayout(VertexType.PNCTTTTX)));
        ...
        layouts.Add(VertexType.PNCCTTX_2,
            new InputLayout(device, vsbytes, VertexTypeGTAV.GetLayout(VertexType.PNCCTTX_2)));
        layouts.Add(VertexType.PNCCTTTX,
            new InputLayout(device, vsbytes, VertexTypeGTAV.GetLayout(VertexType.PNCCTTTX)));
        ...
        layouts.Add(VertexType.PNCH2,
            new InputLayout(device, vsbytes, VertexTypeGTAV.GetLayout(VertexType.PNCH2, VertexDeclarationTypes.GTAV3)));
        layouts.Add(VertexType.PCCH2H4,
            new InputLayout(device, vsbytes,
        ...
        layouts.Add(VertexType.PBBNCTX,
            new InputLayout(device, vssbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCTX)));
        layouts.Add(VertexType.PBBNCTT,
            new InputLayout(device, vssbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCTT)));
        layouts.Add(VertexType.PBBNCTTT,
            new InputLayout(device, vssbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCTTT)));
        layouts.Add(VertexType.PBBNCCT,
            new InputLayout(device, vssbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCCT)));
        layouts.Add(VertexType.PBBNCCTT,
            new InputLayout(device, vssbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCCTT))); //TODO
        layouts.Add(VertexType.PBBNCCTX,
            new InputLayout(device, vssbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCCTX)));
        layouts.Add(VertexType.PBBNCTTX,
            new InputLayout(device, vssbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCTTX)));
        layouts.Add(VertexType.PBBNCTTTX,
            new InputLayout(device, vssbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCTTTX))); //TODO
        layouts.Add(VertexType.PBBNCCTTX,
            new InputLayout(device, vssbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCCTTX))); //TODO
        //PBBCCT todo
        //PBBNC todo
    ...
    {
        RenderableTexture
            texture = null; // ((geom.Textures != null) && (geom.Textures.Length > 0)) ? geom.Textures[0] : null;
