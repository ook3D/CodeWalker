...
public class BasicShader : Shader, IDisposable
{
    ...
    private readonly PixelShader basicps;
    private readonly PixelShader basicpsdef;
    private readonly VertexShader basicvsbox;
    private readonly VertexShader basicvscapsule;
    private readonly VertexShader basicvscylinder;
    private readonly VertexShader basicvspbbncct;
    private readonly VertexShader basicvspbbnccttx;
    private readonly VertexShader basicvspbbncctx;
    private readonly VertexShader basicvspbbnct;
    private readonly VertexShader basicvspbbnctt;
    private readonly VertexShader basicvspbbncttt;
    private readonly VertexShader basicvspbbncttx;
    private readonly VertexShader basicvspbbnctx;
    private readonly VertexShader basicvspncct;
    private readonly VertexShader basicvspncctt;
    private readonly VertexShader basicvspnccttt;
    private readonly VertexShader basicvspncctttx;
    private readonly VertexShader basicvspnccttx;
    private readonly VertexShader basicvspncctx;

    private readonly VertexShader basicvspnct;
    private readonly VertexShader basicvspnctt;
    private readonly VertexShader basicvspncttt;
    private readonly VertexShader basicvspnctttx;
    private readonly VertexShader basicvspncttx;
    private readonly VertexShader basicvspnctx;
    private readonly VertexShader basicvssphere;
    private readonly GpuABuffer<Matrix3_s> BoneMatrices;
    private readonly UnitCapsule capsule; //for collision capsule render
    private readonly GpuABuffer<Vector4> ClothVertices;
    private readonly UnitCube cube; //for collision box render
    private readonly UnitCylinder cylinder; //for collision cylinder render
    public bool DecalMode = false;

    private readonly Matrix3_s[] defaultBoneMatrices;
    private bool defaultBoneMatricesBound;
    public bool Deferred = false;
    private bool disposed;
    private readonly GpuVarsBuffer<BasicShaderInstGlobals> InstGlobalVars;
    private readonly GpuVarsBuffer<BasicShaderInstLocals> InstLocalVars;

    private readonly Dictionary<VertexType, InputLayout> layouts = new();
    private readonly GpuVarsBuffer<BasicShaderPSGeomVars> PSGeomVars;
    private readonly GpuVarsBuffer<BasicShaderPSSceneVars> PSSceneVars;
    ...
    private readonly UnitSphere sphere; //for collision sphere render
    private readonly SamplerState texsampler;
    private readonly SamplerState texsampleranis;
    private readonly SamplerState texsamplertnt;
    private readonly SamplerState texsamplertntyft;
    private readonly GpuVarsBuffer<BasicShaderVSEntityVars> VSEntityVars;
    private readonly GpuVarsBuffer<BasicShaderVSGeomVars> VSGeomVars;
    private readonly GpuVarsBuffer<BasicShaderVSModelVars> VSModelVars;
    private readonly GpuVarsBuffer<BasicShaderVSSceneVars> VSSceneVars;
    public Vector4 WindVector = Vector4.Zero;

    public BasicShader(Device device)
    {
        var vspnctbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PNCT.cso");
        var vspncttbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PNCTT.cso");
        var vspnctttbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PNCTTT.cso");
        var vspncctbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PNCCT.cso");
        var vspnccttbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PNCCTT.cso");
        var vspncctttbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PNCCTTT.cso");
        var vspnctxbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PNCTX.cso");
        var vspncctxbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PNCCTX.cso");
        var vspncttxbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PNCTTX.cso");
        var vspnccttxbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PNCCTTX.cso");
        var vspnctttxbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PNCTTTX.cso");
        var vspncctttxbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PNCCTTTX.cso");

        var vspbbnctbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PBBNCT.cso");
        var vspbbnctxbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PBBNCTX.cso");
        var vspbbncttbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PBBNCTT.cso");
        var vspbbnctttbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PBBNCTTT.cso");
        var vspbbncctbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PBBNCCT.cso");
        var vspbbncctxbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PBBNCCTX.cso");
        var vspbbnccttxbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PBBNCCTTX.cso");
        var vspbbncttxbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_PBBNCTTX.cso");

        var vsboxbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_Box.cso");
        var vsspherebytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_Sphere.cso");
        var vscapsulebytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_Capsule.cso");
        var vscylinderbytes = PathUtil.ReadAllBytes("Shaders\\BasicVS_Cylinder.cso");
        var psbytes = PathUtil.ReadAllBytes("Shaders\\BasicPS.cso");
        var psdefbytes = PathUtil.ReadAllBytes("Shaders\\BasicPS_Deferred.cso");
        ...
        layouts.Add(VertexType.Default,
            new InputLayout(device, vspnctbytes, VertexTypeGTAV.GetLayout(VertexType.Default)));
        layouts.Add(VertexType.PNCH2,
            new InputLayout(device, vspnctbytes,
                VertexTypeGTAV.GetLayout(VertexType.PNCH2, VertexDeclarationTypes.GTAV3))); //TODO?
        layouts.Add(VertexType.PNCTT,
            new InputLayout(device, vspncttbytes, VertexTypeGTAV.GetLayout(VertexType.PNCTT)));
        layouts.Add(VertexType.PNCTTT,
            new InputLayout(device, vspnctttbytes, VertexTypeGTAV.GetLayout(VertexType.PNCTTT)));
        layouts.Add(VertexType.PNCCT,
            new InputLayout(device, vspncctbytes, VertexTypeGTAV.GetLayout(VertexType.PNCCT)));
        layouts.Add(VertexType.PNCCTT,
            new InputLayout(device, vspnccttbytes, VertexTypeGTAV.GetLayout(VertexType.PNCCTT)));
        layouts.Add(VertexType.PNCCTTTT,
            new InputLayout(device, vspncctttbytes, VertexTypeGTAV.GetLayout(VertexType.PNCCTTTT))); //TODO..?


        //normalmap layouts - requires Position, Normal, Colour, Texcoord, Tangent (X)
        layouts.Add(VertexType.DefaultEx,
            new InputLayout(device, vspnctxbytes, VertexTypeGTAV.GetLayout(VertexType.DefaultEx)));
        layouts.Add(VertexType.PCCH2H4,
            new InputLayout(device, vspnctxbytes,
                VertexTypeGTAV.GetLayout(VertexType.PCCH2H4, VertexDeclarationTypes.GTAV2)));
        layouts.Add(VertexType.PNCCTX,
            new InputLayout(device, vspncctxbytes, VertexTypeGTAV.GetLayout(VertexType.PNCCTX)));
        layouts.Add(VertexType.PNCTTX,
            new InputLayout(device, vspncttxbytes, VertexTypeGTAV.GetLayout(VertexType.PNCTTX)));
        layouts.Add(VertexType.PNCCTTX,
            new InputLayout(device, vspnccttxbytes, VertexTypeGTAV.GetLayout(VertexType.PNCCTTX)));
        layouts.Add(VertexType.PNCCTTX_2,
            new InputLayout(device, vspnccttxbytes, VertexTypeGTAV.GetLayout(VertexType.PNCCTTX_2)));
        layouts.Add(VertexType.PNCTTTX,
            new InputLayout(device, vspnctttxbytes, VertexTypeGTAV.GetLayout(VertexType.PNCTTTX)));
        layouts.Add(VertexType.PNCTTTX_2,
            new InputLayout(device, vspnctttxbytes, VertexTypeGTAV.GetLayout(VertexType.PNCTTTX_2)));
        layouts.Add(VertexType.PNCTTTX_3,
            new InputLayout(device, vspnctttxbytes, VertexTypeGTAV.GetLayout(VertexType.PNCTTTX_3)));
        layouts.Add(VertexType.PNCTTTTX,
            new InputLayout(device, vspnctttxbytes, VertexTypeGTAV.GetLayout(VertexType.PNCTTTTX))); //TODO
        layouts.Add(VertexType.PNCCTTTX,
            new InputLayout(device, vspncctttxbytes, VertexTypeGTAV.GetLayout(VertexType.PNCCTTTX)));


        //skinned layouts
        layouts.Add(VertexType.PBBNCT,
            new InputLayout(device, vspbbnctbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCT)));
        layouts.Add(VertexType.PBBNCTX,
            new InputLayout(device, vspbbnctxbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCTX)));
        layouts.Add(VertexType.PBBNCTT,
            new InputLayout(device, vspbbncttbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCTT)));
        layouts.Add(VertexType.PBBNCTTT,
            new InputLayout(device, vspbbnctttbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCTTT)));
        layouts.Add(VertexType.PBBNCCT,
            new InputLayout(device, vspbbncctbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCCT)));
        layouts.Add(VertexType.PBBNCCTT,
            new InputLayout(device, vspbbncctbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCCTT))); //TODO
        layouts.Add(VertexType.PBBNCCTX,
            new InputLayout(device, vspbbncctxbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCCTX)));
        layouts.Add(VertexType.PBBNCTTX,
            new InputLayout(device, vspbbncttxbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCTTX)));
        layouts.Add(VertexType.PBBNCTTTX,
            new InputLayout(device, vspbbncttxbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCTTTX))); //TODO
        layouts.Add(VertexType.PBBNCCTTX,
            new InputLayout(device, vspbbnccttxbytes, VertexTypeGTAV.GetLayout(VertexType.PBBNCCTTX)));
        //PBBCCT todo
        //PBBNC todo
    ...
    public void Dispose()
    {
        ...
        foreach (var layout in layouts.Values) layout.Dispose();
        layouts.Clear();
        ...
        ShaderGlobalLights lights)
    {
        ...
        if (shadowmap != null) shadowmap.SetFinalRenderResources(context);
    ...
    public override void SetModelVars(DeviceContext context, RenderableModel model)
    {
        ...
        if (model.Owner.Cloth?.Vertices != null) SetClothVertices(context, model.Owner.Cloth.Vertices);
    ...
    public override void SetGeomVars(DeviceContext context, RenderableGeometry geom)
    {
        ...
        var usediff = texture != null && texture.ShaderResourceView != null;
        var usediff2 = texture2 != null && texture2.ShaderResourceView != null;
        var usebump = bumptex != null && bumptex.ShaderResourceView != null;
        var usespec = spectex != null && spectex.ShaderResourceView != null;
        var usedetl = detltex != null && detltex.ShaderResourceView != null;
        var usetint = tintpal != null && tintpal.ShaderResourceView != null;

        uint tintflag = 0;
        if (usetint) tintflag = 1;

        var textureAlphaMask = Vector4.Zero;
        var decalflag = DecalMode ? 1u : 0u;
        var windflag = geom.EnableWind ? 1u : 0u;
        var emflag = geom.IsEmissive ? 1u : 0u;
        var pstintflag = tintflag;
        ...
        switch (shaderFile.Hash)
        {
            ...
            case 731050667: //weapon_normal_spec_palette.sps
                if (usetint)
                {
                    tintflag = 0;
                    pstintflag = 2;
                } //use diffuse sampler alpha for tint lookup!
        ...
        if (usediff) texture.SetPSResource(context, 0);
        if (usebump) bumptex.SetPSResource(context, 2);
        if (usespec) spectex.SetPSResource(context, 3);
        if (usedetl) detltex.SetPSResource(context, 4);
        if (usediff2) texture2.SetPSResource(context, 5);
        if (usetint) tintpal.SetVSResource(context, 0);
        if (pstintflag == 2) tintpal.SetPSResource(context, 6);
    ...
    public void SetInstanceVars(DeviceContext context, RenderableInstanceBatch batch)
    {
        ...
        InstLocalVars.Vars.gLodFadeInstRange =
            new Vector3(gb.Batch.LodInstFadeRange, gb.Batch.LodFadeStartDist, gb.Batch.lodDist);
