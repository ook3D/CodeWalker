...
{
    public static Buffer CreateBuffer(Device device, int size, ResourceUsage usage, BindFlags bindFlags,
        CpuAccessFlags cpuAccessFlags, ResourceOptionFlags miscFlags, int structByteStride)
    {
    ...
    public static Texture2D CreateTexture2D(Device device, int width, int height, int mipLevels, int arraySize,
        Format format, int sampleCount, int sampleQuality, ResourceUsage usage, BindFlags bindFlags,
        CpuAccessFlags cpuAccessFlags, ResourceOptionFlags miscFlags)
    {
    ...
    public static SamplerState CreateSamplerState(Device device, TextureAddressMode addressU,
        TextureAddressMode addressV, TextureAddressMode addressW, RawColor4 border, Comparison comparisonFunc,
        Filter filter, int maxAnisotropy, float maxLOD, float minLOD, float mipLODBias)
    {
        var smpDesc = new SamplerStateDescription();
        ...
        var smp = new SamplerState(device, smpDesc);
        return smp;
    }

    public static SamplerState CreateSamplerState(Device device, TextureAddressMode address, RawColor4 border,
        Comparison comparisonFunc, Filter filter, int maxAnisotropy, float maxLOD, float minLOD, float mipLODBias)
    {
        return CreateSamplerState(device, address, address, address, border, comparisonFunc, filter, maxAnisotropy,
            maxLOD, minLOD, mipLODBias);
    }

    public static ShaderResourceView CreateShaderResourceView(Device device, Resource resource, Format format,
        ShaderResourceViewDimension viewDimension, int mipLevels, int mostDetailedMip, int arraySize,
        int firstArraySlice)
    {
        ...
        var srv = new ShaderResourceView(device, resource, srvd);
        return srv;
    }

    public static UnorderedAccessView CreateUnorderedAccessView(Device device, Resource resource, Format format,
        UnorderedAccessViewDimension viewDimension, int firstElement, int numElements,
        UnorderedAccessViewBufferFlags flags, int mipSlice)
    {
    ...
    public static RenderTargetView CreateRenderTargetView(Device device, Resource renderTarget, Format format,
        RenderTargetViewDimension viewDimension, int mipSlice, int arraySize, int firstArraySlice)
    {
        ...
        switch (viewDimension)
        {
            case RenderTargetViewDimension.Buffer: // D3D11_RTV_DIMENSION_BUFFER:
                ...
                rtvd.Buffer.ElementWidth =
                    arraySize * ElementSize(format); // arraySize; //assume square buffer... is this the width?
                rtvd.Buffer.FirstElement = 0 * ElementSize(format); //firstArraySlice;
                rtvd.Buffer.ElementCount =
                    arraySize; //*arraySize*ElementSize(format); //does this represent the height??
                break;
    ...
    public static DepthStencilView CreateDepthStencilView(Device device, Texture2D depthStencil, Format format,
        DepthStencilViewDimension viewDimension)
    {
        ...
        var dsv = new DepthStencilView(device, depthStencil, dsvd);
        return dsv;
    }
    ...
    public static DepthStencilState CreateDepthStencilState(Device device, bool depthEnable, DepthWriteMask writeMask,
        Comparison func, bool stencilEnable, byte stencilReadMask, byte stencilWriteMask,
        DepthStencilOperationDescription frontFace, DepthStencilOperationDescription backFace)
    {
        ...
        var s = new DepthStencilState(device, dsd);
        return s;
    }
    ...
    public static DepthStencilState CreateDepthStencilState(Device device, bool depthEnable, DepthWriteMask writeMask)
    {
        ...
        return CreateDepthStencilState(device, depthEnable, writeMask, Comparison.LessEqual, stencil, rm, wm, frontFace,
            backFace);
    }


    public static RasterizerState CreateRasterizerState(Device device, FillMode fillMode, CullMode cullMode,
        bool depthClipEnable, bool scissorEnable, bool multisampleEnable, int depthBias, float depthBiasClamp,
        float slopeScaledDepthBias)
    {
        var drd = new RasterizerStateDescription();
        ...
        var rs = new RasterizerState(device, drd);
        return rs;
    }
    ...
    {
        return CreateRasterizerState(device, fillMode, cullMode, depthClipEnable, scissorEnable, multisampleEnable, 0,
            0.0f, 0.0f);
    }
    ...
    public static BlendState CreateBlendState(Device device, bool blendEnable, BlendOperation op, BlendOption src,
        BlendOption dst, BlendOperation opAlpha, BlendOption srcAlpha, BlendOption dstAlpha,
        ColorWriteMaskFlags writeMask)
    {
        ...
        var bs = new BlendState(device, bsd);
        return bs;
    }
