...
public class ShaderManager
{
    ...
    private readonly BlendState bsAdd;
    private readonly BlendState bsAlpha;
    private readonly BlendState bsDefault;
    ...
    private readonly DepthStencilState dsDisableAll;
    private readonly DepthStencilState dsDisableComp;
    private readonly DepthStencilState dsDisableWrite;
    private readonly DepthStencilState dsDisableWriteRev;
    private readonly DepthStencilState dsEnabled;
    private readonly DXManager DXMan;
    ...
    private readonly RasterizerState rsSolid;
    private readonly RasterizerState rsSolidDblSided;
    private readonly RasterizerState rsWireframe;
    private readonly RasterizerState rsWireframeDblSided;
    private readonly List<RenderableGeometryInst> shadowbatch = new();
    private readonly List<ShaderBatch> shadowbatches = new();
    private int shadowcastercount; //total casters rendered
    private readonly List<RenderableGeometryInst> shadowcasters = new();
    ...
    public ShaderManager(Device device, DXManager dxman)
    {
        ...
        var dsd = new DepthStencilStateDescription
        {
    ...
    public long TotalGraphicsMemoryUse
    {
        ...
        {
            long u = 0;
            if (DefScene != null) u += DefScene.VramUsage;
            if (HDR != null) u += HDR.VramUsage;
            if (Shadowmap != null) u += Shadowmap.VramUsage;
            return u;
        }
    }
    ...
    public void BeginFrame(DeviceContext context, double currentRealTime, float elapsedTime)
    {
        ...
        shadowcasters.Clear();
        if (shadows && Shadowmap == null) Shadowmap = new Shadowmap(Device);
        ...
        foreach (var bucket in RenderBuckets) bucket.Clear();
    ...
    public void EnsureShaderTextures(GameFileCache gameFileCache, RenderableCache renderableCache)
    {
        ...
        var graphics = 3154743001; //JenkHash.GenHash("graphics");
        var waterbump = 2826194296; //JenkHash.GenHash("waterbump");
        uint waterbump2 = 209023451; //JenkHash.GenHash("waterbump2");
        var waterfog = 4047019542;
    ...
    public void RenderQueued(DeviceContext context, Camera camera, Vector4 wind)
    {
        ...
        shadowbatches.Clear();
        for (var i = 0; i < RenderBuckets.Count; i++)
        {
        ...
        if (shadows) RenderShadowmap(context);
        ...
        for (var i = 0; i < RenderBuckets.Count; i++) //"solid" objects pass
        {
            var bucket = RenderBuckets[i];
            ...
            if (bucket.TerrainBatches.Count > 0) RenderGeometryBatches(context, bucket.TerrainBatches, Terrain);
            if (bucket.BasicBatches.Count > 0) RenderGeometryBatches(context, bucket.BasicBatches, Basic);
        ...
        if (RenderInstBatches.Count > 0) //grass pass
        {
            ...
            for (var i = 0; i < RenderInstBatches.Count; i++) RenderInstancedBatch(context, RenderInstBatches[i]);
        ...
        if (RenderWaterQuads.Count > 0) //render water quads
        {
            ...
            for (var i = 0; i < RenderWaterQuads.Count; i++) Water.RenderWaterQuad(context, RenderWaterQuads[i]);
            Water.UnbindResources(context);
        }

        for (var i = 0; i < RenderBuckets.Count; i++) //main water geoms pass
        {
            var bucket = RenderBuckets[i];
            if (bucket.WaterBatches.Count > 0) RenderGeometryBatches(context, bucket.WaterBatches, Water);
        }

        context.OutputMerger.DepthStencilState = dsDisableWrite;
        for (var i = 0; i < RenderBuckets.Count; i++) //water decals pass
        {
            var bucket = RenderBuckets[i];
            if (bucket.Water2Batches.Count > 0) RenderGeometryBatches(context, bucket.Water2Batches, Water);
        }


        //TODO: needs second gbuffer pass?
        Basic.DecalMode = true;
        for (var i = 0; i < RenderBuckets.Count; i++) //alphablended and glass pass
        {
            var bucket = RenderBuckets[i];
            ...
            if (bucket.GlassBatches.Count > 0) RenderGeometryBatches(context, bucket.GlassBatches, Basic);
        }
        ...
        if (RenderDistLODLights.Count > 0) //distant LOD lights pass
        {
            ...
            for (var i = 0; i < RenderDistLODLights.Count; i++) DistLights.RenderBatch(context, RenderDistLODLights[i]);
    ...
    private void RenderShadowmap(DeviceContext context)
    {
        ...
        var maxdist = Shadowmap.maxShadowDistance; // 3000.0f;// cascade.IntervalFar * 5.0f;

        //find the casters within range
        shadowcasters.Clear();
        shadowcastercount = 0;
        for (var b = 0; b < shadowbatches.Count; b++)
        {
            //shadowcasters.AddRange(shadowbatches[b].Geometries);
            var sbgeoms = shadowbatches[b].Geometries;
            for (var g = 0; g < sbgeoms.Count; g++)
            {
                var sbgeom = sbgeoms[g];
                if (sbgeom.Inst.CastShadow)
                {
                    var idist = sbgeom.Inst.Distance - sbgeom.Inst.Radius;
                    if (idist <= maxdist) shadowcasters.Add(sbgeom);
                }
            }
        }
        ...
        for (var i = 0; i < Shadowmap.CascadeCount; i++)
        {
            var cascade = Shadowmap.Cascades[i];

            Shadowmap.BeginDepthRender(context, i);

            var worldtocascade = cascade.WorldUnitsToCascadeUnits * 2.0f;
            var minrad = cascade.WorldUnitsPerTexel * 5.0f;

            shadowbatch.Clear();
            for (var c = 0; c < shadowcasters.Count; c++)
            {
                ...
                var iscenepos = caster.Inst.Position - Shadowmap.SceneOrigin;
                var ilightpos = cascade.Matrix.Multiply(iscenepos + caster.Inst.BSCenter);
                var ilightradf = caster.Inst.Radius * worldtocascade;
                var ilightminx = ilightpos.X - ilightradf;
                var ilightmaxx = ilightpos.X + ilightradf;
                var ilightminy = ilightpos.Y - ilightradf;
                var ilightmaxy = ilightpos.Y + ilightradf;
    ...
    private void RenderGeometryBatches(DeviceContext context, List<ShaderBatch> batches, Shader shader)
    {
        ...
        foreach (var batch in batches) RenderGeometryBatch(context, batch.Geometries, shader);
    ...
    private void RenderBoundGeometryBatch(DeviceContext context, List<RenderableBoundGeometryInst> batch)
    {
        ...
        foreach (var geom in batch) Basic.RenderBoundGeom(context, geom);
    ...
    public ShaderRenderBucket EnsureRenderBucket(int index)
    {
        ...
        while (index >= RenderBuckets.Count) RenderBuckets.Add(new ShaderRenderBucket(RenderBuckets.Count));
        if (index < RenderBuckets.Count) bucket = RenderBuckets[index];
        return bucket;
    }
    ...
    public void OnWindowResize(int w, int h)
    {
        Width = w;
        Height = h;
        if (DefScene != null) DefScene.OnWindowResize(DXMan);
        if (HDR != null) HDR.OnWindowResize(DXMan);
    }
}
