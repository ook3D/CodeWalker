...
public class Renderable : RenderableCacheItem<DrawableBase>
{
    ...
    public double CurrentAnimTime;

    public bool EnableRootMotion; //used to toggle whether or not to include root motion when playing animations
    ...
    public override void Init(DrawableBase drawable)
    {
        ...
        var totmodels = (hd?.Length ?? 0) + (med?.Length ?? 0) + (low?.Length ?? 0) + (vlow?.Length ?? 0);
        var curmodel = hd?.Length ?? 0;
        ...
        var fragoffset = Vector4.Zero;
        var fragtransformid = 0;
        Bone[] bones = null;
        var usepose = false;
        if (skeleton != null)
        {
            ...
            if (fd != null)
            {
                ...
                if (pose != null && pose.Items != null) //seems to be the default pose
                {
                    ...
                    for (var i = 0; i < maxbonecount; i++)
                    {
                        var p = pose.Items[i];
                        var r1 = p.Row1;
                        var r2 = p.Row2;
                        var r3 = p.Row3;
                        modeltransforms[i] = new Matrix(r1.X, r2.X, r3.X, 0.0f, r1.Y, r2.Y, r3.Y, 0.0f, r1.Z, r2.Z,
                            r3.Z, 0.0f, r1.W, r2.W, r3.W, 1.0f);
                    }
        ...
        for (var mi = 0; mi < AllModels.Length; mi++)
        {
            ...
            if (hastransforms)
            {
                ...
                var trans = boneidx < modeltransforms.Length ? modeltransforms[boneidx] : Matrix.Identity;
                var bone = hasbones && boneidx < bones.Length ? bones[boneidx] : null;
                ...
                else if (!usepose) //when using the skeleton's matrices, they need to be transformed by parent
                {
                    ...
                    var parentind = pinds != null && boneidx < pinds.Length ? pinds[boneidx] : (short)-1;
                    while (parentind >= 0 && parentind < pinds.Length)
                    {
                        var ptrans = parentind < modeltransforms.Length ? modeltransforms[parentind] : Matrix.Identity;
    ...
    public override void Load(Device device)
    {
        if (AllModels != null)
            foreach (var model in AllModels)
            {
                ...
                foreach (var geom in model.Geometries) geom.Load(device);
            }
    ...
    public override void Unload()
    {
        ...
        if (AllModels != null)
            ...
            {
                if (model.Geometries == null) continue;
                foreach (var geom in model.Geometries) geom.Unload();
            }
    ...
    private void UpdateBoneTransforms()
    {
        ...
        for (var i = 0; i < AllModels.Length; i++)
        {
            ...
            if (model?.Geometries == null) continue;
            for (var g = 0; g < model.Geometries.Length; g++)
            {
    ...
    {
        if (ClipMapEntry?.OverridePlayTime ?? false) realTime = ClipMapEntry.PlayTime;
        ...
        if (ClipMapEntry != null) UpdateAnim(ClipMapEntry); //animate skeleton/models
        ...
        foreach (var model in HDModels)
        {
            ...
            foreach (var geom in model.Geometries)
            {
                ...
                if (geom.ClipMapEntryUV != null) UpdateAnimUV(geom.ClipMapEntryUV, geom); //animate UVs
            }
        }
    }
    ...
    private void UpdateAnim(ClipMapEntry cme)
    {
        ...
        if (clipanim?.Animation != null) UpdateAnim(clipanim.Animation, clipanim.GetPlaybackTime(CurrentAnimTime));
        ...
        if (bones != null)
        {
            for (var i = 0; i < bones.Length; i++)
            {
                ...
                switch (bone.Tag)
                {
                    case 23639:
                        tag = 58271;
                        break; //RB_L_ThighRoll: SKEL_L_Thigh
                    case 6442:
                        tag = 51826;
                        break; //RB_R_ThighRoll: SKEL_R_Thigh
    ...
    {
        if (anim == null) return;
        if (anim.BoneIds?.data_items == null) return;
        if (anim.Sequences?.data_items == null) return;

        var interpolate = true; //how to know? eg. cs4_14_hickbar_anim shouldn't
        ...
        for (var i = 0; i < anim.BoneIds.data_items.Length; i++)
        {
    ...
    private void UpdateAnimUV(Animation anim, float t, RenderableGeometry rgeom = null)
    {
        ...
        for (var i = 0; i < anim.BoneIds.data_items.Length; i++)
        {
            var boneiditem = anim.BoneIds.data_items[i];
            var track = boneiditem.Track;
            if (track != 17 && track != 18) continue; //17 and 18 would be UV0 and UV1
            ...
            switch (track)
            {
                case 17:
                    globalAnimUV0 = v;
                    break; //could be overwriting values here...
                case 18:
                    globalAnimUV1 = v;
                    break;
            }
...
public class RenderableModel
{
    ...
    public void Init(DrawableModel dmodel)
    {
        ...
        for (var i = 0; i < geomcount; i++)
        {
            ...
            if (dmodel.BoundsData != null && i < dmodel.BoundsData.Length) GeometryBounds[i] = dmodel.BoundsData[i];

            //GeometryBounds[i] = new AABB_s();//what to default to?
            if (Owner.Key is FragDrawable) rgeom.IsFragment = true;
        }
    }
}
...
public class RenderableGeometry
{
    ...
    public bool globalAnimUVEnable;
    ...
    public float HardAlphaBlend { get; set; }
    public float useTessellation { get; set; }
    public float wetnessMultiplier { get; set; }
    ...
    public float specularIntensityMult { get; set; }
    public float specularFalloffMult { get; set; }
    ...
    public Vector4 globalAnimUV0 { get; set; } = new(1.0f, 0.0f, 0.0f, 0.0f);
    public Vector4 globalAnimUV1 { get; set; } = new(0.0f, 1.0f, 0.0f, 0.0f);
    public Vector4 DirtDecalMask { get; set; } = Vector4.Zero;
    public bool SpecOnly { get; set; }
    public float WaveOffset { get; set; } //for terrainfoam
    public float WaterHeight { get; set; } //for terrainfoam
    public float WaveMovement { get; set; } //for terrainfoam
    public float HeightOpacity { get; set; } //for terrainfoam
    ...
    public void Init(DrawableGeometry dgeom)
    {
        ...
        if (shader != null && shader.ParametersList != null)
        {
            ...
            var texs = new List<TextureBase>();
            var phashes = new List<ShaderParamNames>();
            if (pl != null && hl != null)
                for (var i = 0; i < pl.Length && i < hl.Length; i++)
                {
                    var pName = (ShaderParamNames)hl[i];
                    var param = pl[i];
    ...
    public void Load(Device device)
    {
        ...
        //        break;
        //}
        if (VertexBuffer != null) VBBinding = new VertexBufferBinding(VertexBuffer, VertexStride, 0);
    ...
    public void Unload()
    {
        ...
        {
            for (var i = 0; i < RenderableTextures.Length; i++) RenderableTextures[i] = null;
            RenderableTextures = null;
        }

        if (RenderableTexturesHD != null)
        {
            for (var i = 0; i < RenderableTexturesHD.Length; i++) RenderableTexturesHD[i] = null;
...
public class RenderableTexture : RenderableCacheItem<Texture>
{
    ...
    public override void Init(Texture tex)
    {
        ...
        if (Key != null && Key.Data != null && Key.Data.FullData != null) DataSize = Key.Data.FullData.Length;
    }
    ...
    public override void Load(Device device)
    {
        if (Key != null && Key.Data != null && Key.Data.FullData != null)
            using (var stream = DataStream.Create(Key.Data.FullData, true, false))
            {
                ...
                var pxsize =
                    TextureFormats.ByteSize(Key.Format); // SharpDX.DXGI.FormatHelper.SizeOfInBytes(desc.Format);
...
public class RenderableInstanceBatch : RenderableCacheItem<YmapGrassInstanceBatch>
{
    ...
    {
        Key = batch;
        if (batch.Instances == null) return;
...
public class RenderableLODLights : RenderableCacheItem<YmapFile>
{
    ...
    public override void Load(Device device)
    {
        if (Points != null && Points.Length > 0) PointsBuffer = new GpuSBuffer<LODLight>(device, Points);
        if (Spots != null && Spots.Length > 0) SpotsBuffer = new GpuSBuffer<LODLight>(device, Spots);
        if (Caps != null && Caps.Length > 0) CapsBuffer = new GpuSBuffer<LODLight>(device, Caps);
...
public class RenderableDistantLODLights : RenderableCacheItem<YmapDistantLODLights>
{
    ...
    {
        Key = key;
        if (key.positions == null || key.colours == null) return;
...
public class RenderablePathBatch : RenderableCacheItem<BasePathData>
{
    ...
    public override void Init(BasePathData key)
    {
        ...
        if (Nodes != null) DataSize += Nodes.Length * 16; //sizeof(Vector4)
    }
    ...
    public override void Load(Device device)
    {
        if (PathVertices != null && PathVertices.Length > 0)
        {
            ...
            if (PathVertexBuffer != null) PathVBBinding = new VertexBufferBinding(PathVertexBuffer, VertexStride, 0);
        }
        ...
        if (Nodes != null && Nodes.Length > 0) NodeBuffer = new GpuSBuffer<Vector4>(device, Nodes);
...
public class RenderableWaterQuad : RenderableCacheItem<WaterQuad>
{
    ...
    public override void Init(WaterQuad key)
    {
        ...
        var sx = key.maxX - key.minX;
        var sy = key.maxY - key.minY;
    ...
    public override void Load(Device device)
    {
        ...
        if (Indices != null && Indices.Length > 0) IndexBuffer = Buffer.Create(device, BindFlags.IndexBuffer, Indices);
        //LastUseTime = DateTime.Now; //reset usage timer
        IsLoaded = true;
    }
...
public class RenderableBoundComposite : RenderableCacheItem<Bounds>
{
    ...
    public override void Init(Bounds bound)
    {
        ...
        else
        {
            ...
            else if (bound is BoundDisc boundisc) rgeom.Init(boundisc, ref xform);
    ...
    {
        if (bound.Children == null) return;

        var geoms = new RenderableBoundGeometry[bound.Children.data_items.Length];
        long dsize = 0;
        for (var i = 0; i < bound.Children.data_items.Length; i++)
        {
            var rgeom = new RenderableBoundGeometry(this);
...
public class RenderableBoundGeometry
{
    ...
    public void Init(BoundGeometry bgeom)
    {
        ...
        if (bgeom.Polygons == null || bgeom.Vertices == null) return;
        ...
        for (var i = 0; i < bgeom.Polygons.Length; i++)
        {
        ...
        var rverts = rvertcount > 0 ? new VertexTypeDefault[rvertcount] : null;
        var rboxes = rboxcount > 0 ? new RenderableBox[rboxcount] : null;
        var rspheres = rspherecount > 0 ? new RenderableSphere[rspherecount] : null;
        var rcapsules = rcapsulecount > 0 ? new RenderableCapsule[rcapsulecount] : null;
        var rcylinders = rcylindercount > 0 ? new RenderableCylinder[rcylindercount] : null;
        for (var i = 0; i < bgeom.Polygons.Length; i++)
        {
            var poly = bgeom.Polygons[i];
            if (poly == null) continue;
            var mat = poly.Material;
            var color = BoundsMaterialTypes.GetMaterialColour(mat.Type);
            Vector3 p1, p2, p3, p4, a1, n1; //, n2, n3, p5, p7, p8;
            var norm = Vector3.Zero;
            var colour = (uint)color.ToRgba();
            switch (poly.Type)
            {
                case BoundPolygonType.Triangle:
                ...
                case BoundPolygonType.Capsule:
                    ...
                    var q1 = Quaternion.Invert(Quaternion.LookAtRH(Vector3.Zero, p3, n1));
                ...
                case BoundPolygonType.Cylinder:
                    ...
                    p3 = Vector3.Normalize(n1.GetPerpVec());
                    //p4 = Vector3.Normalize(Vector3.Cross(n1, p3));
                    var q2 = Quaternion.Invert(Quaternion.LookAtRH(Vector3.Zero, p3, n1));
    ...
    public void Init(BoundCapsule bcap, ref Matrix xform)
    {
        ...
        var extent = bcap.SphereRadius - bcap.Margin;
    ...
    public void Init(BoundDisc bdisc, ref Matrix xform)
    {
        ...
        BBMax = bdisc.BoxMax;
        BBOffset = xform.TranslationVector;
        BBOrientation = Quaternion.LookAtLH(Vector3.Zero, Vector3.UnitX, Vector3.UnitZ) *
                        Quaternion.RotationMatrix(rmat);
    ...
    public void Load(Device device)
    {
        ...
        if (Boxes != null && Boxes.Length > 0) BoxBuffer = new GpuSBuffer<RenderableBox>(device, Boxes);
        if (Spheres != null && Spheres.Length > 0) SphereBuffer = new GpuSBuffer<RenderableSphere>(device, Spheres);
