...
public partial class AudioExplorerForm : Form
{
    ...
    private readonly Dictionary<string, RelData> NameComboLookup = new();
    ...
    private GameFileCache GameFileCache { get; }
    ...
    private void SelectType()
    {
        ...
        if (typespl.Length == 2)
            switch (typespl[0])
            {
                case "Config":
                {
                    dict = GameFileCache.AudioConfigDict;
                    if (Enum.TryParse(typespl[1], out Dat4ConfigType t)) typeid = (byte)t;
                    break;
                }
                case "Speech":
                {
                    dict = GameFileCache.AudioSpeechDict;
                    if (Enum.TryParse(typespl[1], out Dat4SpeechType t)) typeid = (byte)t;
                    break;
                }
                case "Synths":
                {
                    dict = GameFileCache.AudioSynthsDict;
                    if (Enum.TryParse(typespl[1], out Dat10RelType t)) typeid = (byte)t;
                    break;
                }
                case "Mixers":
                {
                    dict = GameFileCache.AudioMixersDict;
                    if (Enum.TryParse(typespl[1], out Dat15RelType t)) typeid = (byte)t;
                    break;
                }
                case "Curves":
                {
                    dict = GameFileCache.AudioCurvesDict;
                    if (Enum.TryParse(typespl[1], out Dat16RelType t)) typeid = (byte)t;
                    break;
                }
                case "Categories":
                {
                    dict = GameFileCache.AudioCategsDict;
                    if (Enum.TryParse(typespl[1], out Dat22RelType t)) typeid = (byte)t;
                    break;
                }
                case "Sounds":
                {
                    dict = GameFileCache.AudioSoundsDict;
                    if (Enum.TryParse(typespl[1], out Dat54SoundType t)) typeid = (byte)t;
                    break;
                }
                case "Game":
                {
                    dict = GameFileCache.AudioGameDict;
                    if (Enum.TryParse(typespl[1], out Dat151RelType t)) typeid = (byte)t;
                    break;
                }
            }
    ...
    private void LoadItemHierarchy(RelData item, TreeNode parentNode = null)
    {
        ...
        if (item is Dat22Category && parentNode != null &&
            !(parentNode.Tag is Dat22Category)) //don't bother expanding out categories, too spammy!
            return;
        ...
        if (speech != null)
            foreach (var h in speech)
                if (GameFileCache.AudioSpeechDict.TryGetValue(h, out var child))
                    LoadItemHierarchy(child, node);
        if (synths != null)
            foreach (var h in synths)
                if (GameFileCache.AudioSynthsDict.TryGetValue(h, out var child))
                    LoadItemHierarchy(child, node);
        if (mixers != null)
            foreach (var h in mixers)
                if (GameFileCache.AudioMixersDict.TryGetValue(h, out var child))
                    LoadItemHierarchy(child, node);
        if (curves != null)
            foreach (var h in curves)
                if (GameFileCache.AudioCurvesDict.TryGetValue(h, out var child))
                    LoadItemHierarchy(child, node);
        if (categs != null)
            foreach (var h in categs)
                if (GameFileCache.AudioCategsDict.TryGetValue(h, out var child))
                    LoadItemHierarchy(child, node);
        if (sounds != null)
            foreach (var h in sounds)
                if (GameFileCache.AudioSoundsDict.TryGetValue(h, out var child))
                    LoadItemHierarchy(child, node);
        if (games != null)
            foreach (var h in games)
                if (GameFileCache.AudioGameDict.TryGetValue(h, out var child))
                    LoadItemHierarchy(child, node);
    ...
    {
        if (NameComboLookup.TryGetValue(NameComboBox.Text, out var item)) LoadItemHierarchy(item);
    }
