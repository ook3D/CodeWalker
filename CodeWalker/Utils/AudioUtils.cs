...
public class AudioPlayer
{
    ...
    private readonly Stopwatch playtimer = new();
    ...
    public int PlayTimeMS => (int)playtimer.Elapsed.TotalMilliseconds + playBeginMs;

    public int TotalTimeMS => (int)(trackLength * 1000);
    ...
    public void LoadAudio(params AwcStream[] audios)
    {
        ...
        if (voices == null || voices.Length != audios.Length)
        {
            ...
            for (var i = 0; i < audios.Length; i++) voices[i] = new AudioVoice();
        }
        ...
        if (State != newState)
        {
            switch (newState)
            {
                case PlayerState.Playing:
                    if (State == PlayerState.Stopped) playtimer.Reset();
                    playtimer.Start();
                    break;
    ...
    public void SetOutputMatrix(int v, float l, float r)
    {
        ...
        if (State == PlayerState.Playing) voice.sourceVoice.SetOutputMatrix(1, 2, voice.outputMatrix);
    }
    ...
    public void DisposeAudio()
    {
        ...
        foreach (var voice in voices) voice?.audioBuffer?.Stream?.Dispose();
    }
    ...
    public void Play(float playBegin = 0)
    {
        ...
        foreach (var voice in voices) voice.sourceVoice.Start();
    ...
    public void Pause()
    {
        if (State == PlayerState.Playing)
        {
            foreach (var voice in voices) voice.sourceVoice.Stop();
    ...
    public class AudioVoice
    {
        ...
        public float[] outputMatrix = { 1.0f, 1.0f }; //left/right channel output levels
...
public class AudioDatabase
{
    ...
    public void Init(GameFileCache gameFileCache, bool sounds = true, bool game = true)
    {
        ...
        void addRpfDatRels(RpfFile rpffile)
        {
            ...
            foreach (var entry in rpffile.AllEntries)
                if (entry is RpfFileEntry)
                {
                    ...
                    //}
                    if (sounds && entry.NameLower.EndsWith(".dat54.rel")) datrelentries[entry.NameHash] = fentry;
                    if (game && entry.NameLower.EndsWith(".dat151.rel")) datrelentries[entry.NameHash] = fentry;
                }
        }
                ...
                if (entry is RpfFileEntry)
                {
                    ...
                    if (entry.NameLower.EndsWith(".awc"))
                    {
                        ...
                        if (string.IsNullOrEmpty(parentname) && entry.Parent?.File != null)
                        {
                            ...
                            var ind = parentname.LastIndexOf('.');
                            if (ind > 0) parentname = parentname.Substring(0, ind);
                        }
        ...
        if (audrpf != null) addRpfDatRels(audrpf);
        foreach (var baserpf in gameFileCache.BaseRpfs) addRpfAwcs(baserpf);
        if (gameFileCache.EnableDlc)
        {
            var updrpf = rpfman.FindRpfFile("update\\update.rpf");
            if (updrpf != null) addRpfDatRels(updrpf);
        ...
        foreach (var datentry in datrelentries.Values)
        {
            ...
            if (relfile?.RelDatas != null)
                foreach (var rd in relfile.RelDatas)
                    ...
                    else if (rd is Dat151RelData gd) gamedb[gd.NameHash] = gd;
        }
