...
public class Synthesizer : IDisposable
{
    ...
    public void Play(Dat10Synth synth)
    {
        ...
        for (var i = 0; i < synth.BuffersCount; i++)
    ...
    private void SynthesizeFrame()
    {
        ...
        for (var insti = 0; insti < instructions.Length; insti++)
        {
            ...
            switch (inst.Opcode)
            {
                case Dat10Synth.Opcode.COPY_BUFFER:
                    ...
                    for (var i = 0; i < inst.NumberOfOutputs - 1; i++)
                ...
                case Dat10Synth.Opcode.NOISE:
                    a = GetBuffer(param[0]);
                    for (var i = 0; i < BufferSize; i++)
                        a[i] = (float)rnd.NextDouble() * 2.0f -
                               1.0f; // TODO(alexguirre): verify that the game generates numbers between [-1.0f, 1.0f]
                    break;
                ...
                case Dat10Synth.Opcode.BiquadCoefficients_LowPass_2:
                    BiquadCoefficientsLowPass(
                        inst.Opcode == Dat10Synth.Opcode.BiquadCoefficients_LowPass_2,
                        GetScalar(param[5]),
                        GetScalar(param[6]),
                ...
                case Dat10Synth.Opcode.BiquadCoefficients_HighPass_2:
                    BiquadCoefficientsHighPass(
                        inst.Opcode == Dat10Synth.Opcode.BiquadCoefficients_HighPass_2,
                        GetScalar(param[5]),
                        GetScalar(param[6]),
                        out Registers[param[0].Value & 0xFF],
                        out Registers[param[1].Value & 0xFF],
                ...
                case Dat10Synth.Opcode.OnePole_LPF_SCALAR_SCALAR:
                    SetRegister(param[0],
                        OnePoleLPF(GetScalar(param[1]), GetScalar(param[2]), ref StateBlocks[param[3].Value]));
                    break;
                ...
                case Dat10Synth.Opcode.OnePole_HPF_SCALAR_SCALAR:
                    SetRegister(param[0],
                        OnePoleHPF(GetScalar(param[1]), GetScalar(param[2]), ref StateBlocks[param[3].Value]));
                    break;
                ...
                case Dat10Synth.Opcode.ENVELOPE_GEN__R_EXP_T_RETRIGGER:
                {
                    ...
                    var envFinished = EnvelopeGen(inst.Opcode, buffer, ref StateBlocks[stateBlock], predelay, attack,
                        decay, sustain, hold, release, trigger);
                ...
                case Dat10Synth.Opcode.TIMED_TRIGGER__T_RETRIGGER:
                {
                    ...
                    var result = TimedTrigger(inst.Opcode, ref StateBlocks[stateBlock], trigger, predelay, attack,
                        decay, hold, release);
                ...
                case Dat10Synth.Opcode.DECIMATE_BUFFER:
                    Decimate(GetBuffer(param[0]), GetScalar(param[1]), GetScalar(param[2]),
                        ref StateBlocks[param[3].Value]);
                    break;
                ...
                case Dat10Synth.Opcode.SMALL_DELAY_FRAC:
                    SmallDelay(GetBuffer(param[0]), GetScalar(param[1]), 0.0f,
                        ref StateBlocks[param[inst.NumberOfInputs].Value], true);
                    break;
                case Dat10Synth.Opcode.SMALL_DELAY_NON_INTERP:
                    SmallDelay(GetBuffer(param[0]), GetScalar(param[1]), 0.0f,
                        ref StateBlocks[param[inst.NumberOfInputs].Value], false);
                    break;
                case Dat10Synth.Opcode.SMALL_DELAY_FRAC_FEEDBACK:
                    SmallDelay(GetBuffer(param[0]), GetScalar(param[1]), GetScalar(param[2]),
                        ref StateBlocks[param[3].Value], true);
                    break;
                case Dat10Synth.Opcode.SMALL_DELAY_NON_INTERP_FEEDBACK:
                    SmallDelay(GetBuffer(param[0]), GetScalar(param[1]), GetScalar(param[2]),
                        ref StateBlocks[param[3].Value], false);
                    break;
                case Dat10Synth.Opcode.TRIGGER_DIFF:
                    SetRegister(param[0],
                        TriggerDiff(GetScalar(param[1]), GetScalar(param[2]), ref StateBlocks[param[3].Value]));
                    break;
                ...
                case Dat10Synth.Opcode.AWProcess:
                    AWFilter(GetBuffer(param[0]), GetBuffer(param[1]), GetBuffer(param[2]),
                        ref StateBlocks[param[3].Value]);
                    break;
                ...
                case Dat10Synth.Opcode.FINISH:
                    ...
                    break;
                default:
                    throw new NotImplementedException(inst.Opcode + " (line " + (insti + 1) + ")");
            }
        }
    ...
    private float GetScalar(Dat10Synth.Parameter parameter)
    {
        ...
        if ((scalarId & 0xFF00) == 0x100) return Registers[scalarId & 0xFF];
    ...
    private float Counter(bool returnCounter, float setToZeroTrigger, float incrementTrigger, float decrementTrigger,
        float maxCounter, ref StateBlock stateBlock)
    {
        // State block:
        //  X -> counter value
    ...
    {
        var valuePercent = Math.Abs(value) / Math.Max(0.0001f, threshold);
    ...
    private void Compress(float[] buffer, float scalar1_threshold, float scalar2_ratio, float scalar3_time1,
        float scalar4_time2, ref StateBlock stateBlock)
    {
        // State block:
        //  X -> ...
        ...
        var v1_slopeFactor = Math.Max(0.000001f, 1.0f - 1.0f / scalar2_ratio);
        var v2_threshold = (float)Math.Pow(scalar1_threshold, v1_slopeFactor);
        var v3 = (float)Math.Exp(-2.2f / (scalar3_time1 * SampleRate));
        var v4 = (float)Math.Exp(-2.2f / (scalar4_time2 * SampleRate));

        for (var i = 0; i < BufferSize; i++)
        {
            var sample = Math.Max(0.000001f, Math.Abs(buffer[i]));
            var v5 = Math.Max(1.0f, (float)Math.Pow(sample, v1_slopeFactor) / v2_threshold);
            var v6 = v5 - stateX;
            var v7 = v6 < 0.0f ? v4 : v3;
    ...
    private void BiquadCoefficientsLowPass(bool unkFlag, float frequency, float unk, out float b0, out float b1,
        out float b2, out float a1, out float a2)
    {
        frequency = Math.Max(0.01f, Math.Min(20000.16f, frequency));

        var w = (float)(2.0 * Math.PI * frequency / SampleRate);
        var v11 = unkFlag ? 0.5f : 1.0f;
        var cosW = (float)Math.Cos(w);
        var alpha = (float)Math.Sin(w) * (float)Math.Sinh(0.5f / (v11 * unk));
    ...
    private void BiquadCoefficientsHighPass(bool unkFlag, float frequency, float unk, out float b0, out float b1,
        out float b2, out float a1, out float a2)
    {
        frequency = Math.Max(0.01f, Math.Min(20000.16f, frequency));
    ...
    private void BiquadCoefficientsBandPass(float centerFrequency, float freq2, out float b0, out float b1,
        out float b2, out float a1, out float a2)
    {
        ...
        var w = (float)(2.0 * Math.PI * centerFrequency / SampleRate);
        var cosW = (float)Math.Cos(w);
        var v14 = 1.0f / (float)Math.Tan(Math.PI * freq2 / SampleRate);
    ...
    private void BiquadCoefficientsBandStop(float centerFrequency, float freq2, out float b0, out float b1,
        out float b2, out float a1, out float a2)
    {
        centerFrequency = Math.Max(0.01f, Math.Min(24000.0f, centerFrequency));
        freq2 = Math.Max(0.000001f, Math.Min(centerFrequency * 2.0f, freq2));

        var w = (float)(2.0f * Math.PI * centerFrequency / SampleRate);
        var cosW = (float)Math.Cos(w);
        var v15 = (float)Math.Tan(Math.PI * freq2 / SampleRate);
    ...
    private void BiquadCoefficientsPeakingEQ(float centerFrequency, float freq2, float gain, out float b0, out float b1,
        out float b2, out float a1, out float a2)
    {
        ...
        var w = (float)(2.0f * Math.PI * centerFrequency / SampleRate);
        var q = centerFrequency / freq2;
        var alpha = (float)Math.Sin(w) / (2.0f * q);
        var cosW = (float)Math.Cos(w);
        var A = Math.Max(0.000001f,
            gain * (float)Math
                .Sqrt(2)); // TODO(alexguirre): this should be amp = 10.0 ** (gain_db/40.0), where does sqrt(2) come from? maybe gain not in dB?
    ...
    private void BiquadCoefficientsHighShelf(float frequency, float freq2, float A, out float b0, out float b1,
        out float b2, out float a1, out float a2)
    {
        frequency = Math.Max(0.01f, Math.Min(20000.16f, frequency));
        ...
        var w = (float)(2.0f * Math.PI * frequency / SampleRate);
        var q = frequency / freq2;
        var alpha = (float)Math.Sin(w) / (2.0f * q);
        var cosW = (float)Math.Cos(w);
        var sqrtA = 2.0f * (float)Math.Sqrt(A) * alpha;
    ...
    private void BiquadCoefficientsLowShelf(float frequency, float freq2, float A, out float b0, out float b1,
        out float b2, out float a1, out float a2)
    {
        frequency = Math.Max(0.01f, Math.Min(20000.16f, frequency));
        freq2 = Math.Max(0.000001f, Math.Min(frequency * 2.0f, freq2));
    ...
    private void BiquadFilter2Pole(float[] buffer, float b0, float b1, float b2, float a1, float a2,
        ref StateBlock stateBlock)
    {
        var pole1 = stateBlock.X;
        var pole2 = stateBlock.Y;
    ...
    private void BiquadFilter4Pole(float[] buffer, float b0, float b1, float b2, float a1, float a2,
        ref StateBlock stateBlock)
    {
        var pole1 = stateBlock.X;
        var pole2 = stateBlock.Y;
        var pole3 = stateBlock.Z;
        var pole4 = stateBlock.W;
    ...
    {
        for (var i = 0; i < BufferSize; i++) buffer[i] = OnePoleLPF(buffer[i], frequencies[i], ref stateBlock);
    }

    private void OnePoleLPF(float[] buffer, float frequency, ref StateBlock stateBlock)
    {
        for (var i = 0; i < BufferSize; i++) buffer[i] = OnePoleLPF(buffer[i], frequency, ref stateBlock);
    }

    private float OnePoleLPF(float sample, float frequency, ref StateBlock stateBlock)
    {
        var previousSample = stateBlock.X;

        var b1 = (float)Math.Exp(-2.0f * Math.PI * frequency * 256.0f / SampleRate);
        var a0 = 1.0f - b1;

        var s = a0 * sample + b1 * previousSample;
        stateBlock.X = s;
        return s;
    }
    ...
    {
        for (var i = 0; i < BufferSize; i++) buffer[i] = OnePoleHPF(buffer[i], frequencies[i], ref stateBlock);
    }

    private void OnePoleHPF(float[] buffer, float frequency, ref StateBlock stateBlock)
    {
        for (var i = 0; i < BufferSize; i++) buffer[i] = OnePoleHPF(buffer[i], frequency, ref stateBlock);
    }
    ...
    private void EnvelopeFollower(float[] buffer, float a2, float a3, ref StateBlock stateBlock)
    {
        ...
        var prevFollower = stateBlock.X;
        for (var i = 0; i < BufferSize; i++)
        {
            var scalar = Math.Abs(buffer[i]);
            float v12;
            ...
            var v13 = (prevFollower - scalar) * (float)Math.Pow(0.01f, v12) + scalar;

            var follower = Math.Max(0.0f, v13);
            buffer[i] = follower;
    ...
    {
        var triggered = false;
        for (var i = 0; !triggered && i < BufferSize; i++) triggered = buffer[i] >= triggerLimit - 0.000001f;
    ...
    {
        var triggered = false;
        for (var i = 0; !triggered && i < BufferSize; i++) triggered = buffer[i] >= triggerLimitBuffer[i] - 0.000001f;
        return TriggerLatch(triggered, ref stateBlock);
    }
    ...
    private float TriggerLatch(bool triggered, ref StateBlock stateBlock)
    {
        ...
        var set = stateBlock.X != 0.0f;
        var latch = 0.0f;
        if (triggered && !set)
            latch = 1.0f;

        if (triggered != set) stateBlock.X = triggered ? 1.0f : 0.0f;
        return latch;
    }
    ...
    private float TriggerDiff(float value, float triggerLimit, ref StateBlock stateBlock)
    {
        ...
        var diff = Math.Abs(stateBlock.X - value);
        stateBlock.X = value;
        var triggered = diff >= triggerLimit - 0.000001f;
    ...
    private float EnvelopeGen(Dat10Synth.Opcode envelopeGenOpcode, float[] buffer, ref StateBlock stateBlock,
        float predelay, float attack, float decay, float sustain, float hold, float release, float trigger)
    {
        ...
        return EnvelopeGen(releaseType, triggerMode, buffer, ref stateBlock, predelay, attack, decay, sustain, hold,
            release, trigger);
    }

    private float EnvelopeGen(EnvelopeReleaseType releaseType, EnvelopeTriggerMode triggerMode, float[] buffer,
        ref StateBlock stateBlock, float predelay, float attack, float decay, float sustain, float hold, float release,
        float trigger)
    {
        // State block:
        ...
        var stateInfo = BitConverter.ToUInt32(BitConverter.GetBytes(stateBlock.X), 0);
        var stateReleaseStartY = stateBlock.Y;
        var stateRampY = stateBlock.Z;
        var stateRampSlope = stateBlock.W;
        var state = (EnvelopeGenState)(stateInfo & 0xFF);
        var stateRemainingSamples = stateInfo >> 8;
        var stateFinished = stateRemainingSamples < BufferSize;
        var clearBuffer = true;
        var finishedTrigger = 0.0f;
        var triggered = trigger >= 1.0f;
        ...
        switch (state)
        {
            ...
            case EnvelopeGenState.HoldUntilTrigger: // trigger refers to 'hold >= 0.0f'
                ...
                for (var i = 0; i < BufferSize; i++) buffer[i] = sustain;
                clearBuffer = false;
                break;
    ...
    {
        var maxSamples = Math.Max(0.0f, seconds * SampleRate);
        // calculate the multiple of BufferSize closest to maxSamples without exceeding it
        var samples = maxSamples - (maxSamples - (float)Math.Truncate(maxSamples / BufferSize) * BufferSize);
    ...
    private void EnvelopeProcessLinearRelease(float[] buffer, float topY, float bottomY)
    {
        ...
        for (var i = 0; i < BufferSize; i++) buffer[i] = buffer[i] * diff + topY;
    }
    ...
    private TimedTriggerResult TimedTrigger(Dat10Synth.Opcode envelopeGenOpcode, ref StateBlock stateBlock,
        float trigger, float predelay, float attack, float decay, float hold, float release)
    {
        EnvelopeTriggerMode triggerMode;
        switch (envelopeGenOpcode)
        {
    ...
    private TimedTriggerResult TimedTrigger(EnvelopeTriggerMode triggerMode, ref StateBlock stateBlock, float trigger,
        float predelay, float attack, float decay, float hold, float release)
    {
        // State block:
        //  X -> envelope state : 8 bits | envelope state remaining samples : 24 bits
    ...
    {
        var index = (int)(scalars.Length * Math.Max(0.0f, Math.Min(1.0f, value)));
    ...
    {
        var index = (int)value % scalars.Length;
        var t = value - (float)Math.Floor(value);
    ...
    {
        var index = (int)(buffers.Length * Math.Max(0.0f, Math.Min(1.0f, value)));
    ...
    {
        var index = (int)value % buffers.Length;
    ...
    private void SwitchLerp(float value, float[][] buffers, float[] outputBuffer)
    {
        ...
        for (var i = 0; i < BufferSize; i++) outputBuffer[i] = Lerp(t, min[i], max[i]);
    }
    ...
    private void SwitchEqualPower(float value, float[][] buffers, float[] outputBuffer)
    {
        ...
        for (var i = 0; i < BufferSize; i++) outputBuffer[i] = cos * a[i] + sin * b[i];
    }

    private void SmallDelay(float[] buffer, float delaySamples, float feedback, ref StateBlock stateBlock,
        bool interpolate)
    {
        feedback = Math.Max(Math.Min(feedback, 1.0f),
            -1.0f); //just to stop things getting accidentally out of control...
        delaySamples = Math.Abs(delaySamples); //should this be clamped to max 8..?
        var delaySamplesInt = (int)delaySamples;
        var frac = delaySamples - (float)Math.Floor(delaySamples);
        for (var i = 0; i < BufferSize; i++)
        {
            var currSample = buffer[i];
            float delayedSample;
            var delayed = i + delaySamplesInt;
            var s0 = stateBlock[delayed & 7];
            if (interpolate)
            {
                var s1 = stateBlock[(delayed + 1) & 7];
    ...
    {
        var previousUnfilteredSample = stateBlock.X;
        var previousSample = stateBlock.Y;

        var a1 = (float)Math.Tan(Math.PI * breakFrequency / SampleRate);
        a1 = (a1 - 1.0f) / (a1 + 1.0f);

        for (var i = 0; i < BufferSize; i++)
        {
            var sample = a1 * buffer[i] + previousUnfilteredSample - a1 * previousSample;
    ...
    private void AllpassFilter(float[] buffer, float[] breakFrequencies, ref StateBlock stateBlock)
    {
        ...
        for (var i = 0; i < BufferSize; i++)
        {
            var a1 = (float)Math.Tan(Math.PI * breakFrequencies[i] / SampleRate);
            a1 = (a1 - 1.0f) / (a1 + 1.0f);
    ...
    private enum EnvelopeReleaseType
    {
        Linear,
        Exponential
    }

    private enum EnvelopeTriggerMode
    {
        OneShot,
        Retrigger,
        Interruptible
    ...
    public struct StateBlock
    {
        ...
        public float[] Values
        {
            get { return new[] { X, Y, Z, W, A, B, C, D }; }
        }
    }
}