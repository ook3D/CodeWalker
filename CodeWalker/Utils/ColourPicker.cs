...
public partial class ColourPicker : UserControl
{
    ...
    private static readonly Color[] DefaultColours =
    {
    ...
    private readonly int CircleBox = 13;
    private readonly int CircleOffset = 6;
    private readonly float CircleRadius = 5.8f;
    private readonly float CircleThickness = 1.0f;
    ...
    private readonly int SliderBox = 13;
    private Bitmap SliderL;
    private readonly int SliderOffset = 6;
    private Bitmap SliderR;
    private int SVal;
    private bool UpdatingTextboxes;
    private int VVal;
    ...
    private static Vector3 ColourToHSB(Color c)
    {
        ...
        var h = Math.Abs(rng) < 1e-6f ? 0 :
            Math.Abs(max - r) < 1e-6f ? 60 * (g - b) / rng + (g >= b ? 0 : 360) :
            Math.Abs(max - g) < 1e-6f ? 60 * (b - r) / rng + 120 :
            Math.Abs(max - b) < 1e-6f ? 60 * (r - g) / rng + 240 : 0;
    ...
    private Bitmap CreateCircleBitmap(Color colour)
    {
        ...
        for (var y = 0; y < CircleBox; y++)
        for (var x = 0; x < CircleBox; x++)
        {
    ...
    private Bitmap CreateSliderBitmap(bool dirL)
    {
        ...
        for (var y = 0; y < SliderBox; y++)
        for (var x = 0; x < SliderBox; x++)
        {
    ...
    public void SaveRecentColour()
    {
        ...
        foreach (var rc in RecentColours)
            if (rc == Colour)
                return; //it's already in the list, abort
        for (var i = RecentColours.Length - 1; i > 0; i--) RecentColours[i] = RecentColours[i - 1];
    ...
    private void SetColourComponent(ColourComponent c, int v, bool updateTextbox)
    {
        switch (c)
        {
            ...
            case ColourComponent.V:
                VVal = v;
                break;
            case ColourComponent.R:
                RVal = v;
                break;
            case ColourComponent.G:
                GVal = v;
                break;
            case ColourComponent.B:
                BVal = v;
                break;
            case ColourComponent.A:
                AVal = v;
                break;
        }
    ...
    private void SetColour(Color c, bool updateTextbox = true, bool origVal = false)
    {
        ...
        AVal = c.A;
        UpdateGradients();
        if (updateTextbox) UpdateTextboxes(ColourComponent.None);
    }
    ...
    private void UpdateGradientsProc()
    {
        ...
        var mbmp = MainGradient.LockBits(new Rectangle(0, 0, 256, 256), ImageLockMode.WriteOnly,
            PixelFormat.Format32bppArgb);
        var mptr = mbmp.Scan0;
        Marshal.Copy(mg, 0, mptr, mg.Length);
        MainGradient.UnlockBits(mbmp);

        var sbmp = SideGradient.LockBits(new Rectangle(0, 0, 20, 256), ImageLockMode.WriteOnly,
            PixelFormat.Format32bppArgb);
        var sptr = sbmp.Scan0;
    ...
    private Color GetSideColour(int x, int y, bool preview)
    {
        switch (ColourMode)
        {
            case ColourComponent.H:
                return preview
                    ? ColourFromHSB(y * Deg255, 1, 1, AVal)
                    : ColourFromHSB(y * Deg255, SVal * One100, VVal * One100, AVal);
    ...
    private void ColourPicker_MouseUp(object sender, MouseEventArgs e)
    {
        if (MainDrag)
            SetMainColour(e.X, e.Y);
        else if (SideDrag) SetSideColour(e.X, e.Y);
        MainDrag = false;
        SideDrag = false;
    }
