...
public class MapIcon
{
    ...
    public void LoadTexture(Device device, Action<string> errorAction)
    {
        try
        {
            if (device != null)
            {
                Tex = TextureLoader.CreateTexture2DFromBitmap(device,
                    TextureLoader.LoadBitmap(new ImagingFactory2(), Filepath));
...
public class MapMarker
{
    ...
    public void Parse(string s)
    {
        ...
        if (ss.Length > 2) FloatUtil.TryParse(ss[2].Trim(), out p.Z);
        if (ss.Length > 3)
            Name = ss[3].Trim();
        else
            Name = string.Empty;
        for (var i = 4; i < ss.Length; i++)
        {
