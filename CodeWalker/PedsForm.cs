...
public partial class PedsForm : Form, DXForm
{
    ...
    private readonly ComboBox[] ComponentComboBoxes;
    ...
    private readonly Ped SelectedPed = new();
    ...
    private void AddDrawableModelsTreeNodes(DrawableModel[] models, string prefix, bool check,
        TreeNode parentDrawableNode = null, TreeNode parentTextureNode = null)
    {
        ...
        for (var mi = 0; mi < models.Length; mi++)
        {
            ...
            var tmnode = ttnc.Add(mprefix + " " + model);
            tmnode.Tag = model;
    ...
    private void UpdateGlobalPedsUI()
    {
        ...
        else
        {
            ...
            foreach (var ped in peds) PedNameComboBox.Items.Add(ped.Name);
            if (peds.Count > 0)
            {
    ...
    private void UpdateModelsUI()
    {
        ...
        for (var i = 0; i < 12; i++)
        {
            ...
            if (drawable != null) AddDrawableTreeNode(drawable, drawablename, true);
        }
    }
    ...
    public void LoadPed()
    {
        ...
        for (var i = 0; i < 12; i++) ClearCombo(ComponentComboBoxes[i]);
    ...
    private void PopulateCompCombo(ComboBox c, MCPVComponentData compData)
    {
        ...
        foreach (var item in compData.DrawblData3)
            for (var alt = 0; alt <= item.NumAlternatives; alt++)
                if (item.TexData?.Length > 0)
                    for (var tex = 0; tex < item.TexData.Length; tex++)
                        c.Items.Add(new ComponentComboItem(item, alt, tex));
                else
                    c.Items.Add(new ComponentComboItem(item));
        if (compData.DrawblData3.Length > 0) c.SelectedIndex = 1;
    }
