...
public partial class ProjectForm : Form
{
    ...
    private bool hidegtavmap;

    private readonly List<YmapEntityDef> interiorslist = new(); //used for handling interiors ybns

    private readonly Dictionary<uint, YbnFile> projectybns = new(); //used for handling interior ybns


    private bool renderitems = true;

    private bool ShowProjectItemInProcess;
    private readonly Dictionary<uint, RelFile> visibleaudiofiles = new();
    private readonly Dictionary<uint, YmapEntityDef> visiblemloentities = new();
    private readonly Dictionary<string, YmtFile> visiblescenarios = new();
    private readonly Dictionary<string, TrainTrack> visibletrains = new();

    private readonly Dictionary<string, YbnFile> visibleybns = new();
    private readonly Dictionary<int, YndFile> visibleynds = new();
    private readonly Dictionary<int, YnvFile> visibleynvs = new();
    private bool WorldSelectionChangeInProcess;
    ...
    public WorldForm WorldForm { get; }
    ...
    public object ProjectSyncRoot { get; } = new();

    private void UpdateStatus(string text)
    {
        try
        {
            if (InvokeRequired) BeginInvoke(() => { UpdateStatus(text); });
            //TODO: status text
    ...
    private void UpdateError(string text)
    {
        try
        {
            if (InvokeRequired) BeginInvoke(() => { UpdateError(text); });
            //TODO: error text
    ...
    private void SetTheme(string themestr, bool changing = true)
    {
        if (changing && CurrentProjectFile != null)
            if (MessageBox.Show("Project will be closed before changing the theme. Are you sure you want to continue?",
                    "Theme change", MessageBoxButtons.YesNo) != DialogResult.Yes)
                return;
        ...
        foreach (ToolStripMenuItem menu in ViewThemeMenu.DropDownItems) menu.Checked = false;
    ...
    private T FindPanel<T>(Func<T, bool> findFunc) where T : ProjectPanel
    {
        ...
        foreach (var content in pane.Contents)
        {
            ...
            if (test != null && findFunc(test)) return test;
        }
    ...
    public void ShowPanel<T>(bool promote, Func<T> createFunc, Action<T> updateAction, Func<T, bool> findFunc)
        where T : ProjectPanel
    {
        var found = FindPanel(findFunc);
        if (found != null && found != PreviewPanel)
        {
            if (found.IsHidden) found.Show();
    ...
    public void ShowDeleteGrassPanel(bool promote)
    {
        ShowPanel(promote,
            () =>
            {
                DeleteGrassPanel = new DeleteGrassPanel(this);
                return DeleteGrassPanel;
            }, //createFunc
            panel =>
            {
                panel.SetProject(CurrentProjectFile);
                panel.IsFloat = true;
            }, //updateFunc
            panel => { return true; }); //findFunc
    }
    ...
    public void ShowEditAudioStaticEmitterPanel(bool promote)
    {
        ShowPanel(promote,
            ...
            panel =>
            {
                return panel.CurrentEmitter?.StaticEmitter == CurrentAudioStaticEmitter?.StaticEmitter;
            }); //findFunc
    }
    ...
    private void ShowCurrentProjectItem(bool promote)
    {
        ...
        else if (CurrentYndFile != null) ShowEditYndPanel(promote);
        ...
        else if (CurrentYnvFile != null) ShowEditYnvPanel(promote);
        if (CurrentTrainNode != null)
            ShowEditTrainNodePanel(promote);
        else if (CurrentTrainTrack != null) ShowEditTrainTrackPanel(promote);
        if (CurrentScenarioNode != null)
            ShowEditScenarioNodePanel(promote);
        else if (CurrentScenario != null) ShowEditScenarioYmtPanel(promote);
        ...
        else if (CurrentAudioFile != null) ShowEditAudioFilePanel(promote);
    }
    ...
    public void ShowProjectItem(object item, bool promote)
    {
        if (item is object[] arr)
        {
            for (var i = 0; i < arr.Length; i++)
                if (arr[i] is MCEntityDef mcent) //for MLO entities, need to transform this into an actual entity instance
                {
                    ...
                    if (instance != null) arr[i] = instance.TryGetYmapEntity(mcent);
                }
        ...
        if (item is MapSelection[] multi) WorldForm?.SelectMulti(multi, false, false);
    ...
    public void SetProjectItem(object item, bool refreshUI = true)
    {
        ...
        var
            daz = item as Dat151AmbientZone; //need to get these from WorldForm, project tree only contains the Dat151Rel items
        ...
        {
            var instance = TryGetMloInstance(CurrentMloEntity.OwnerMlo);
        ...
        if (CurrentMloRoom != null) CurrentArchetype = CurrentMloRoom.OwnerMlo;
        if (CurrentMloPortal != null) CurrentArchetype = CurrentMloPortal.OwnerMlo;
        if (CurrentMloEntitySet != null) CurrentArchetype = CurrentMloEntitySet.OwnerMlo;
        if (CurrentArchetype != null)
            CurrentYtypFile = CurrentEntity?.MloParent?.Archetype?.Ytyp ?? CurrentArchetype?.Ytyp;
        if (CurrentCollisionVertex != null) CurrentCollisionBounds = CurrentCollisionVertex.Owner;
        if (CurrentCollisionPoly != null) CurrentCollisionBounds = CurrentCollisionPoly.Owner;
        if (CurrentCollisionBounds != null) CurrentYbnFile = CurrentCollisionBounds.GetRootYbn();
        if (CurrentPathNode != null) CurrentYndFile = CurrentPathNode.Ynd;
        if (CurrentNavPoly != null) CurrentYnvFile = CurrentNavPoly.Ynv;
        if (CurrentNavPoint != null) CurrentYnvFile = CurrentNavPoint.Ynv;
        if (CurrentNavPortal != null) CurrentYnvFile = CurrentNavPortal.Ynv;
        if (CurrentTrainNode != null) CurrentTrainTrack = CurrentTrainNode.Track;
        if (CurrentScenario != null &&
            CurrentScenario.ScenarioRegion == null)
            CurrentScenario = null; //incase other types of ymt files make it into the project...
        if (CurrentScenarioNode != null) CurrentScenario = CurrentScenarioNode.Ymt;
        if (CurrentScenarioChainEdge != null) CurrentScenario = CurrentScenarioChainEdge.Region?.Ymt;
        if (CurrentAudioAmbientZone != null) CurrentAudioFile = CurrentAudioAmbientZone.RelFile;
        if (CurrentAudioAmbientRule != null) CurrentAudioFile = CurrentAudioAmbientRule.RelFile;
        if (CurrentAudioStaticEmitter != null) CurrentAudioFile = CurrentAudioStaticEmitter.RelFile;
        if (CurrentAudioAmbientZoneList != null) CurrentAudioFile = CurrentAudioAmbientZoneList.Rel;
        if (CurrentAudioStaticEmitterList != null) CurrentAudioFile = CurrentAudioStaticEmitterList.Rel;
        if (CurrentAudioInterior != null) CurrentAudioFile = CurrentAudioInterior.Rel;
        if (CurrentAudioInteriorRoom != null) CurrentAudioFile = CurrentAudioInteriorRoom.Rel;
    ...
    private void PromoteIfPreviewPanel(IDockContent panel)
    {
        ...
        {
            if (PreviewPanel != null) PreviewPanel.HideOnClose = false;
            PreviewPanel = null;
        }
    }
    ...
    {
        if ((CurrentGrassBatch != null && CurrentGrassBatch.BrushEnabled) || (DeleteGrassPanel != null &&
                                                                              DeleteGrassPanel.Mode !=
                                                                              DeleteGrassPanel.DeleteGrassMode.None))
            return true;
        return false;
    }
    ...
    public float GetInstanceBrushRadius()
    {
        ...
        if (DeleteGrassPanel != null && mode != DeleteGrassPanel.DeleteGrassMode.None)
    ...
    {
        var file = ShowOpenDialog("CodeWalker Projects|*.cwproj", string.Empty);
        ...
        var cpath = new FileInfo(CurrentProjectFile.Filepath).Directory.FullName;

        foreach (var ymap in CurrentProjectFile.YmapFiles)
        {
            var filename = ymap.FilePath;
            if (!File.Exists(filename)) filename = cpath + "\\" + filename;
        ...
        {
            var filename = scenario.FilePath;
            if (!File.Exists(filename)) filename = cpath + "\\" + filename;
            if (File.Exists(filename))
    ...
    public void CloseProject()
    {
        ...
        foreach (var ymap in CurrentProjectFile.YmapFiles)
            if (ymap != null && ymap.HasChanged)
                ...
                if (MessageBox.Show("Would you like to save " + ymap.Name + " before closing?",
                        "Save .ymap before closing?", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
        ...
        foreach (var ytyp in CurrentProjectFile.YtypFiles)
            if (ytyp != null && ytyp.HasChanged)
                ...
                if (MessageBox.Show("Would you like to save " + ytyp.Name + " before closing?",
                        "Save .ytyp before closing?", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
        ...
        foreach (var ybn in CurrentProjectFile.YbnFiles)
            if (ybn != null && ybn.HasChanged)
                ...
                if (MessageBox.Show("Would you like to save " + ybn.Name + " before closing?",
                        "Save .ybn before closing?", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    CurrentYbnFile = ybn;
                    SaveYbn();
                }
        ...
        foreach (var ynd in CurrentProjectFile.YndFiles)
            if (ynd != null && ynd.HasChanged)
                ...
                if (MessageBox.Show("Would you like to save " + ynd.Name + " before closing?",
                        "Save .ynd before closing?", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    CurrentYndFile = ynd;
                    SaveYnd();
                }
        ...
        foreach (var ynv in CurrentProjectFile.YnvFiles)
            if (ynv != null && ynv.HasChanged)
                ...
                if (MessageBox.Show("Would you like to save " + ynv.Name + " before closing?",
                        "Save .ynv before closing?", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    CurrentYnvFile = ynv;
                    SaveYnv();
                }
        ...
        foreach (var trains in CurrentProjectFile.TrainsFiles)
            if (trains != null && trains.HasChanged)
                ...
                if (MessageBox.Show("Would you like to save " + trains.Name + " before closing?",
                        "Save trains file before closing?", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
        ...
        foreach (var scenario in CurrentProjectFile.ScenarioFiles)
            if (scenario != null && scenario.HasChanged)
                ...
                if (MessageBox.Show("Would you like to save " + scenario.Name + " before closing?",
                        "Save scenario file before closing?", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
        ...
        foreach (var datrel in CurrentProjectFile.AudioRelFiles)
            if (datrel != null && datrel.HasChanged)
                //save the current scenario file first?
                if (MessageBox.Show("Would you like to save " + datrel.Name + " before closing?",
                        "Save scenario file before closing?", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
        ...
        if (CurrentProjectFile.HasChanged)
            ...
            if (MessageBox.Show("Would you like to save the current project before closing?",
                    "Save project before closing?", MessageBoxButtons.YesNo) == DialogResult.Yes)
                SaveProject();
        ...
        if (WorldForm != null) WorldForm.SelectItem(); //make sure current selected item isn't still selected...
    ...
    public void SaveProject(bool saveas = false)
    {
        ...
        {
            var fileName = ShowSaveDialog("CodeWalker Projects|*.cwproj", CurrentProjectFile.Filepath);
            if (string.IsNullOrEmpty(fileName)) return; //user cancelled

            var oldpath = CurrentProjectFile.Filepath;
    ...
    public void OpenFolder()
    {
        ...
        if (files.Length > 100)
            if (MessageBox.Show(
                    "This folder contains many files, loading may take a long time!\nAre you sure you want to continue?",
                    "Large folder warning", MessageBoxButtons.YesNo) != DialogResult.Yes)
                return;
    ...
    public void OpenFiles(string[] files = null)
    {
        ...
        if (CurrentProjectFile == null) NewProject();
    ...
    public void Save(bool saveas = false)
    {
        ...
        else if (CurrentProjectFile != null) SaveProject(saveas);
    }
    ...
    public object NewObject(MapSelection sel, bool copyPosition = false, bool selectNew = true)
    {
        ...
        if (sel.MultipleSelectionItems != null)
        {
            ...
            for (var i = 0; i < sel.MultipleSelectionItems.Length; i++)
        ...
        if (sel.CollisionPoly != null)
            return NewCollisionPoly(sel.CollisionPoly.Type, sel.CollisionPoly, copyPosition, selectNew);
        if (sel.CollisionBounds != null)
            return NewCollisionBounds(sel.CollisionBounds.Type, sel.CollisionBounds, copyPosition, selectNew);
        if (sel.EntityDef != null)
            return NewEntity(sel.EntityDef, copyPosition, selectNew);
        if (sel.CarGenerator != null)
            return NewCarGen(sel.CarGenerator, copyPosition, selectNew);
        if (sel.LodLight != null)
            return NewLodLight(sel.LodLight, copyPosition, selectNew);
        if (sel.BoxOccluder != null)
            return NewBoxOccluder(sel.BoxOccluder, copyPosition, selectNew);
        if (sel.OccludeModelTri != null)
            return NewOccludeModelTriangle(sel.OccludeModelTri, copyPosition, selectNew);
        if (sel.PathNode != null)
            return NewPathNode(sel.PathNode, copyPosition, selectNew);
        if (sel.NavPoly != null)
            return NewNavPoly(sel.NavPoly, copyPosition, selectNew);
        if (sel.NavPoint != null)
            return NewNavPoint(sel.NavPoint, copyPosition, selectNew);
        if (sel.NavPortal != null)
            return NewNavPortal(sel.NavPortal, copyPosition, selectNew);
        if (sel.TrainTrackNode != null)
            return NewTrainNode(sel.TrainTrackNode, copyPosition, selectNew);
        if (sel.ScenarioNode != null)
            return NewScenarioNode(sel.ScenarioNode, copyPosition, selectNew);
        if (sel.Audio?.AmbientZone != null)
            return NewAudioAmbientZone(sel.Audio, copyPosition, selectNew);
        if (sel.Audio?.AmbientRule != null)
            return NewAudioAmbientRule(sel.Audio, copyPosition, selectNew);
        if (sel.Audio?.StaticEmitter != null) return NewAudioStaticEmitter(sel.Audio, copyPosition, selectNew);
    ...
    public void SaveYmap(bool saveas = false)
    {
        ...
        var ymapname = CurrentYmapFile.Name;
        var filepath = CurrentYmapFile.FilePath;
        if (string.IsNullOrEmpty(filepath)) filepath = ymapname;
        var origfile = filepath;
        ...
        if (saveas)
        {
            ...
            if (CurrentProjectFile != null)
            {
                var origpath = CurrentProjectFile.GetRelativePath(origfile);
                var newpath = CurrentProjectFile.GetRelativePath(CurrentYmapFile.FilePath);
        ...
        if (CurrentYmapFile.SaveWarnings != null)
        {
            ...
            MessageBox.Show(
                CurrentYmapFile.SaveWarnings.Count + " warnings were generated while saving the ymap:\n" + w);
    ...
    public void AddYmapToProject(YmapFile ymap)
    {
        ...
        else if (CurrentGrassBatch != null) ProjectExplorer?.TrySelectGrassBatchTreeNode(CurrentGrassBatch);
    }
    ...
    {
        if (CurrentYmapFile == null) return;
        if (autoymapextents) CurrentYmapFile.CalcExtents();
        if (autoymapflags) CurrentYmapFile.CalcFlags();
        var panel = FindPanel((EditYmapPanel p) => p.Tag == CurrentYmapFile);
        if (panel != null) panel.SetYmap(CurrentYmapFile);
    }
    ...
    public YmapEntityDef NewEntity(YmapEntityDef copy = null, bool copyPosition = false, bool selectNew = true)
    {
        if (copy != null)
        {
            ...
            if (entdef != null) return NewMloEntity(copy, copyPosition, selectNew);
        }


        if (CurrentYmapFile == null) return null;

        var spawndist = 5.0f; //use archetype BSradius if starting with a copy...
        if (copy != null) spawndist = copy.BSRadius * 2.5f;
        var cp = copyPosition && copy != null;
        var pos = cp ? copy.Position : GetSpawnPos(spawndist);
        ...
        var ent = new YmapEntityDef(CurrentYmapFile, 0, ref cent);
    ...
    private bool DeleteYmapEntity()
    {
        ...
        {
            MessageBox.Show(
                "This entity's numChildren is not 0 - deleting entities with children is not currently supported by CodeWalker.");
            return true;
        }

        var idx = CurrentEntity.Index;
        for (var i = idx + 1; i < CurrentYmapFile.AllEntities.Length; i++)
        {
            var ent = CurrentYmapFile.AllEntities[i];
            if (ent._CEntityDef.numChildren != 0)
            {
                MessageBox.Show(
                    "There are other entities present in this .ymap that have children. Deleting this entity is not currently supported by CodeWalker.");
                return true;
            }
        }
        ...
        if (!res)
            MessageBox.Show(
                "Entity.Index didn't match the index of the entity in the ymap. This shouldn't happen, check LOD linkages!");
        ...
        if (WorldForm != null) WorldForm.SelectItem();
    ...
    public YmapGrassInstanceBatch NewGrassBatch(YmapGrassInstanceBatch copy = null)
    {
        ...
        var fwBatch = new rage__fwGrassInstanceListDef();
        var instances = new rage__fwGrassInstanceListDef__InstanceData[0];
        ...
        var batch = new YmapGrassInstanceBatch
        {
    ...
    public bool DeleteGrassBatch()
    {
        ...
        if (MessageBox.Show(
                "Are you sure you want to delete this grass batch?\n" + CurrentGrassBatch +
                "\n\nThis operation cannot be undone. Continue?", "Confirm delete", MessageBoxButtons.YesNo) !=
        ...
        if (!res) MessageBox.Show("Unable to delete the grass batch. This shouldn't happen!");
    ...
    public void PaintGrass(SpaceRayIntersectResult mouseRay, InputManager input)
    {
        try
        {
            if (InvokeRequired)
            {
                Invoke(() => { PaintGrass(mouseRay, input); });
                return;
            }

            if (!mouseRay.Hit || !mouseRay.TestComplete) return;

            var batchPanel = FindPanel<EditYmapGrassPanel>(x => x.CurrentBatch == CurrentGrassBatch);
            ...
            if (CurrentGrassBatch != null && batchPanel != null && (mode == DeleteGrassPanel.DeleteGrassMode.Batch ||
                                                                    (CurrentGrassBatch.BrushEnabled &&
                                                                     input.ShiftPressed)))
    ...
    public void BulkEraseGrassInstancesAtMouse(SpaceRayIntersectResult mouseRay)
    {
        BulkEraseGrassInstancesAtMouse(mouseRay, y => true);
    }

    public void BulkEraseGrassInstancesAtMouse(SpaceRayIntersectResult mouseRay, Predicate<YmapFile> ymapFilter)
    {
        BulkEraseGrassInstancesAtMouse(mouseRay, ymapFilter, b => true);
    }

    public void BulkEraseGrassInstancesAtMouse(SpaceRayIntersectResult mouseRay, Predicate<YmapFile> ymapFilter,
        Predicate<YmapGrassInstanceBatch> batchFilter)
    {
        if (!mouseRay.Hit) return;

        var radius = DeleteGrassPanel?.BrushRadius ?? 5f;
        ...
        lock (WorldForm.RenderSyncRoot)
        {
            foreach (var ymap in WorldForm.Renderer.VisibleYmaps)
            {
                ...
                for (var i = 0; i < ymap.GrassInstanceBatches.Length; i++)
                {
    ...
    public YmapCarGen NewCarGen(YmapCarGen copy = null, bool copyPosition = false, bool selectNew = true)
    {
        ...
        if (!copyPosition || copy == null) ccg.position = pos;


        var cg = new YmapCarGen(CurrentYmapFile, ccg);
    ...
    public bool DeleteCarGen()
    {
        ...
        if (!res) MessageBox.Show("Unable to delete the car generator. This shouldn't happen!");
    ...
    public YmapLODLight NewLodLight(YmapLODLight copy = null, bool copyPosition = false, bool selectNew = true)
    {
        ...
        else
        {
            ...
            yll.Colour = new Color(255, 255, 255, 127);
            yll.Direction = Vector3.ForwardRH;
            yll.Falloff = 10.0f;
            //...
        }

        if (!copyPosition || copy == null) yll.Position = pos;
    ...
    public bool DeleteLodLight()
    {
        ...
        if (!res) MessageBox.Show("Unable to delete the LOD light. This shouldn't happen!");
    ...
    public YmapBoxOccluder NewBoxOccluder(YmapBoxOccluder copy = null, bool copyPosition = false, bool selectNew = true)
    {
        ...
        //...
        if (!copyPosition || copy == null) bo.Position = pos;
    ...
    public bool DeleteBoxOccluder()
    {
        ...
        if (!res) MessageBox.Show("Unable to delete the box occluder. This shouldn't happen!");
    ...
    public YmapOccludeModel NewOccludeModel(YmapOccludeModel copy = null, bool copyPosition = false,
        bool selectNew = true)
    {
        if (CurrentYmapFile == null) return null;
    ...
    public bool DeleteOccludeModel()
    {
        ...
        if (!res) MessageBox.Show("Unable to delete the occlude model. This shouldn't happen!");
    ...
    public YmapOccludeModelTriangle NewOccludeModelTriangle(YmapOccludeModelTriangle copy = null,
        bool copyPosition = false, bool selectNew = true)
    {
        if (CurrentYmapFile == null) return null;
        ...
        if (copy != null)
            ot = new YmapOccludeModelTriangle(copy.Model, copy.Corner1, copy.Corner2, copy.Corner3,
                copy.Model.Triangles?.Length ?? 0);
        else
            ot = new YmapOccludeModelTriangle(CurrentOccludeModel, pos, pos + Vector3.UnitY, pos + Vector3.UnitX,
                CurrentOccludeModel?.Triangles?.Length ?? 0);
        //...
        if (!copyPosition || copy == null)
        {
            //om.Center = pos;
    ...
    public bool DeleteOccludeModelTriangle()
    {
        ...
        if (!res) MessageBox.Show("Unable to delete the occlude model triangle. This shouldn't happen!");
    ...
    private void ImportMenyooXml()
    {
        ...
        foreach (var placement in menyooXml.Placements)
            ...
            else if (placement.Type == 2)
            {
                ...
                var cdir = rotq.Multiply(new Vector3(0, 5, 0));
                ccg.flags = 3680;
                ccg.orientX = cdir.X;
                ccg.orientY = cdir.Y;
                ccg.perpendicularLength = 2.6f;
                ...
                if (sbyte.TryParse(placement.VehicleProperties.FirstOrDefault(p => p.Name == "Livery")?.Value,
                        out var livery)) ccg.livery = livery;
            ...
            else if (placement.Type == 3) //standard entity
            {
                ...
                cent.ambientOcclusionMultiplier = 255;
                cent.artificialAmbientOcclusion = 255;
                if (uint.TryParse(placement.ObjectProperties.FirstOrDefault(p => p.Name == "TextureVariation")?.Value,
                        out var tint)) cent.tintValue = tint;
        ...
        MessageBox.Show(entcount + " entities imported. \n" + carcount + " car generators imported. \n" + pedcount +
                        " peds ignored. \n" + unkcount + " others ignored.");
    }
    ...
    public void SaveYtyp(bool saveas = false)
    {
        ...
        var ytypname = CurrentYtypFile.Name;
        var filepath = CurrentYtypFile.FilePath;
        if (string.IsNullOrEmpty(filepath)) filepath = ytypname;
        var origfile = filepath;
        if (!File.Exists(filepath)) saveas = true;
        ...
        if (saveas)
        {
            ...
            {
                var origpath = CurrentProjectFile.GetRelativePath(origfile);
                var newpath = CurrentProjectFile.GetRelativePath(CurrentYtypFile.FilePath);
        ...
        if (CurrentYtypFile.SaveWarnings != null)
        {
            ...
            MessageBox.Show(
                CurrentYtypFile.SaveWarnings.Count + " warnings were generated while saving the ytyp:\n" + w);
    ...
    public Archetype NewArchetype(Archetype copy = null)
    {
        ...
        if (copy != null) archetype.Init(CurrentYtypFile, ref copy._BaseArchetypeDef);
    ...
    public void NewArchetypesFromYdrs()
    {
        ...
        var files = ShowOpenDialogMulti("Ydr files|*.ydr", string.Empty);
        if (files == null) return;
        if (files.Length == 0) return;
    ...
    public YmapEntityDef NewMloEntity(YmapEntityDef copy = null, bool copyTransform = false, bool selectNew = true)
    {
        ...
        {
            mloArch = CurrentEntity?.MloParent.Archetype as MloArchetype ?? CurrentMloRoom?.OwnerMlo ??
                CurrentMloPortal?.OwnerMlo ?? CurrentMloEntitySet?.OwnerMlo;
        ...
        {
            MessageBox.Show(
                "Unable to find MLO instance for this interior! Try adding an MLO instance ymap to the project.");
            return null;
        }
        ...
        var roomIndex = CurrentMloRoom?.Index ?? -1;
        if (roomIndex >= (mloArch.rooms?.Length ?? 0))
        {
            MessageBox.Show(
                $@"Room at index {roomIndex} does not exist in {mloArch.Name}! {mloArch.Name} only has {mloArch.rooms?.Length ?? 0} rooms.");
            return null;
        }

        var portalIndex = CurrentMloPortal?.Index ?? -1;
        if (portalIndex >= (mloArch.portals?.Length ?? 0))
        {
            MessageBox.Show(
                $@"Portal at index {portalIndex} does not exist in {mloArch.Name}! {mloArch.Name} only has {mloArch.portals?.Length ?? 0} portals.");
            return null;
        }

        var entsetIndex = CurrentMloEntitySet?.Index ?? -1;
        if (entsetIndex >= (mloArch.entitySets?.Length ?? 0))
        {
            MessageBox.Show(
                $@"EntitySet at index {entsetIndex} does not exist in {mloArch.Name}! {mloArch.Name} only has {mloArch.entitySets?.Length ?? 0} entitySets.");
            return null;
        }
        ...
        var cp = copyTransform && copy != null;
        var pos = cp
            ? copy.CEntityDef.position
            : GetSpawnPosRel(spawndist, mloInstance.Owner.Position, mloInstance.Owner.Orientation);
        var rot = cp ? copy.CEntityDef.rotation.ToQuaternion() : Quaternion.Identity;
    ...
    {
        var mlo = CurrentMloRoom?.OwnerMlo ?? CurrentMloPortal?.OwnerMlo ?? CurrentMloEntitySet?.OwnerMlo ??
            CurrentEntity?.MloParent.Archetype as MloArchetype ?? CurrentArchetype as MloArchetype;
        if (mlo == null) return null;

        if (copy == null) copy = CurrentMloRoom;
    ...
    public MCMloPortalDef NewMloPortal(MCMloPortalDef copy = null)
    {
        ...
        if (copy == null) copy = CurrentMloPortal;
        ...
        if (portal.Corners == null)
            portal.Corners = new[]
            {
                new Vector4(0, 0, 0, float.NaN), new Vector4(0, 0, 1, float.NaN), new Vector4(0, 1, 1, float.NaN),
                new Vector4(0, 1, 0, float.NaN)
            };
    ...
    public MCMloEntitySet NewMloEntitySet(MCMloEntitySet copy = null)
    {
        ...
        if (copy == null) copy = CurrentMloEntitySet;
        ...
        var mloInstance = TryGetMloInstance(mlo);
        if (mloInstance != null) mloInstance.AddEntitySet(set);
    ...
    public bool DeleteMloEntity()
    {
        ...
        var mloInstance = CurrentEntity.MloParent.MloInstance;
    ...
    public bool DeleteMloRoom()
    {
        ...
        if (MessageBox.Show(
                ...
                "Confirm delete", MessageBoxButtons.YesNo) != DialogResult.Yes) return true;
    ...
    public bool DeleteMloEntitySet()
    {
        ...
        if (MessageBox.Show(
                "Are you sure you want to delete this entity set?\n" + CurrentMloEntitySet.Name +
                "\n\nThis operation cannot be undone. Continue?", "Confirm delete", MessageBoxButtons.YesNo) !=
            DialogResult.Yes) return true;
        ...
        var mloInstance = TryGetMloInstance(mlo);
        if (mloInstance != null) mloInstance.DeleteEntitySet(CurrentMloEntitySet);
    ...
    private void AddProjectArchetypes(YtypFile ytyp)
    {
        ...
        foreach (var arch in ytyp.AllArchetypes) AddProjectArchetype(arch);
    }
    ...
    {
        if (ytyp?.AllArchetypes == null) return;
        foreach (var arch in ytyp.AllArchetypes) RemoveProjectArchetype(arch);
    }
    ...
    public void SaveYbn(bool saveas = false)
    {
        ...
        var ybnname = CurrentYbnFile.Name;
        var filepath = CurrentYbnFile.FilePath;
        if (string.IsNullOrEmpty(filepath)) filepath = ybnname;
        var origfile = filepath;
        if (!File.Exists(filepath)) saveas = true;
        ...
        if (saveas)
        {
            ...
            {
                var origpath = CurrentProjectFile.GetRelativePath(origfile);
                var newpath = CurrentProjectFile.GetRelativePath(CurrentYbnFile.FilePath);
    ...
    public Bounds NewCollisionBounds(BoundsType type, Bounds copy = null, bool copyPosition = false,
        bool selectNew = true)
    {
        ...
        else
        {
            ...
            if (b is BoundComposite bcmp) bcmp.Position = Vector3.Zero; //start composite transform at 0
        }
    ...
    public void AddCollisionBoundsToProject()
    {
        try
        {
            ...
            {
                MessageBox.Show(
                    "Sorry, only YBN collisions can currently be added to the project. Embedded collisions TODO!");
                return;
            }
    ...
    public bool DeleteCollisionBounds()
    {
        ...
        if (MessageBox.Show(
                "Are you sure you want to delete this collision bounds?\n" + CurrentCollisionBounds +
                "\n\nThis operation cannot be undone. Continue?", "Confirm delete", MessageBoxButtons.YesNo) !=
            DialogResult.Yes) return true;
        ...
        if (!res) MessageBox.Show("Unable to delete the collision bounds. This shouldn't happen!");
        ...
        if (WorldForm != null)
        {
            if (parent != null) WorldForm.UpdateCollisionBoundsGraphics(parent);
            WorldForm.SelectItem();
        }
    ...
    public BoundPolygon NewCollisionPoly(BoundPolygonType type, BoundPolygon copy = null, bool copyPosition = false,
        bool selectNew = true)
    {
        ...
        if (copy != null)
        {
            ...
            switch (type)
            {
                ...
                case BoundPolygonType.Sphere:
                    if (psph != null && csph != null) psph.sphereRadius = csph.sphereRadius;
                    break;
                case BoundPolygonType.Capsule:
                    if (pcap != null && ccap != null) pcap.capsuleRadius = ccap.capsuleRadius;
                    break;
                ...
                case BoundPolygonType.Cylinder:
                    if (pcyl != null && ccyl != null) pcyl.cylinderRadius = ccyl.cylinderRadius;
                    break;
            }
        }
        else
        {
            ...
            if (ptri != null) ptri.VertexPositions = new[] { pos, pos + x, pos + y };
            if (psph != null)
            {
        ...
        if (WorldForm != null) WorldForm.UpdateCollisionBoundsGraphics(bgeom);
    ...
    public bool DeleteCollisionPoly()
    {
        ...
        if (!res) MessageBox.Show("Unable to delete the collision poly. This shouldn't happen!");
    ...
    public bool DeleteCollisionVertex()
    {
        ...
        if (!res) MessageBox.Show("Unable to delete the collision vertex. This shouldn't happen!");
    ...
    public void SaveYnd(bool saveas = false)
    {
        ...
        var yndname = CurrentYndFile.Name;
        var filepath = CurrentYndFile.FilePath;
        if (string.IsNullOrEmpty(filepath)) filepath = yndname;
        var origfile = filepath;
        if (!File.Exists(filepath)) saveas = true;
        ...
        if (saveas)
        {
            ...
            {
                var origpath = CurrentProjectFile.GetRelativePath(origfile);
                var newpath = CurrentProjectFile.GetRelativePath(CurrentYndFile.FilePath);
    ...
    public void AddYndToProject(YndFile ynd)
    {
        ...
        CurrentYndFile = ynd;
        RefreshUI();
        if (CurrentPathNode != null) ProjectExplorer?.TrySelectPathNodeTreeNode(CurrentPathNode);
    }
    ...
    public YndNode NewPathNode(YndNode copy = null, bool copyPosition = false, bool selectNew = true)
    {
        ...
        if (copy == null) copy = CurrentPathNode;
        if (copy != null)
        {
        ...
        var cp = copyPosition && copy != null;
        var pos = cp ? copy.Position : GetSpawnPos(10.0f);
        ...
        if (copy != null)
        {
            ...
            if (copy.Links != null && copy.Links.Length > 0)
            {
                ...
                if (clnode.Links != null)
                    for (var i = 0; i < clnode.Links.Length; i++)
                    {
        ...
        if (WorldForm != null) WorldForm.UpdatePathYndGraphics(CurrentYndFile, false);
    ...
    public bool DeletePathNode()
    {
        ...
        var res = false;
        var affectedFiles = new YndFile[0];
        if (WorldForm != null)
            lock (WorldForm.RenderSyncRoot) //don't try to do this while rendering...
            {
    ...
    public void SaveYnv(bool saveas = false)
    {
        ...
        var ynvname = CurrentYnvFile.Name;
        var filepath = CurrentYnvFile.FilePath;
        if (string.IsNullOrEmpty(filepath)) filepath = ynvname;
        var origfile = filepath;
        if (!File.Exists(filepath)) saveas = true;
        ...
        if (saveas)
        {
            ...
            {
                var origpath = CurrentProjectFile.GetRelativePath(origfile);
                var newpath = CurrentProjectFile.GetRelativePath(CurrentYnvFile.FilePath);
    ...
    public void AddYnvToProject(YnvFile ynv)
    {
        ...
        CurrentYnvFile = ynv;
        RefreshUI();
        if (CurrentNavPoly != null) ProjectExplorer?.TrySelectNavPolyTreeNode(CurrentNavPoly);
    }
    ...
    public void SaveTrainTrack(bool saveas = false)
    {
        ...
        var trackname = CurrentTrainTrack.Name;
        var filepath = CurrentTrainTrack.FilePath;
        if (string.IsNullOrEmpty(filepath)) filepath = trackname;
        var origfile = filepath;
        if (!File.Exists(filepath)) saveas = true;
        ...
        if (saveas)
        {
            ...
            {
                var origpath = CurrentProjectFile.GetRelativePath(origfile);
                var newpath = CurrentProjectFile.GetRelativePath(CurrentTrainTrack.FilePath);
    ...
    public void AddTrainTrackToProject(TrainTrack track)
    {
        ...
        CurrentTrainTrack = track;
        RefreshUI();
        if (CurrentTrainNode != null) ProjectExplorer?.TrySelectTrainNodeTreeNode(CurrentTrainNode);
    }
    ...
    public TrainTrackNode NewTrainNode(TrainTrackNode copy = null, bool copyPosition = false, bool selectNew = true)
    {
        ...
        if (copy == null) copy = CurrentTrainNode;
        if (copy != null) n.NodeType = copy.NodeType;
        ...
        if (WorldForm != null) WorldForm.UpdateTrainTrackGraphics(CurrentTrainTrack, false);
    ...
    public bool DeleteTrainNode()
    {
        ...
        if (!res) MessageBox.Show("Unable to delete the train track node. This shouldn't happen!");
    ...
    public void SaveScenario(bool saveas = false)
    {
        ...
        var ymtname = CurrentScenario.Name;
        var filepath = CurrentScenario.FilePath;
        if (string.IsNullOrEmpty(filepath)) filepath = ymtname;
        var origfile = filepath;
        if (!File.Exists(filepath)) saveas = true;
        ...
        if (saveas)
        {
            ...
            {
                var origpath = CurrentProjectFile.GetRelativePath(origfile);
                var newpath = CurrentProjectFile.GetRelativePath(CurrentScenario.FilePath);
    ...
    public void AddScenarioToProject(YmtFile ymt)
    {
        ...
        CurrentScenario = ymt;
        RefreshUI();
        if (CurrentScenarioNode != null) ProjectExplorer?.TrySelectScenarioNodeTreeNode(CurrentScenarioNode);
    }
    ...
    public ScenarioNode NewScenarioNode(ScenarioNode copy = null, bool copyPosition = false, bool selectNew = true)
    {
        ...
        if (copy == null) copy = CurrentScenarioNode;

        var n = CurrentScenario.ScenarioRegion.AddNode(copy);

        var cp = copyPosition && copy != null;
        var pos = cp ? copy.Position : GetSpawnPos(10.0f);
        var ori = cp ? copy.Orientation : Quaternion.Identity;
        n.SetPosition(pos);
    ...
    public bool DeleteScenarioNode()
    {
        ...
        if (!res) MessageBox.Show("Unable to delete the scenario node. This shouldn't happen!");
    ...
    public void AddScenarioChain()
    {
        ...
        if (copycl != null)
        {
            var copyclnode = new ScenarioNode(CurrentScenario);
        ...
        if (copye != null) ed.Data = copye.Data;
    ...
    public void AddScenarioCluster() //TODO: add defualt cluster points to new cluster
    {
        ...
        var cluster = new MCScenarioPointCluster(rgn, copy);
        var clusterpoints = new List<MCScenarioPoint>();
        ...
        var ncopy = new ScenarioNode(
            CurrentScenario); //copy an empty node to start with, to avoid creating default MyPoint
    ...
    public void AddScenarioEntity() //TODO: add default entity point(s) to entity
    {
        ...
        var entity = new MCScenarioEntityOverride(rgn, copy);
        var entpoints = new List<MCExtensionDefSpawnPoint>();
    ...
    public void DeleteScenarioChain()
    {
        ...
        if (MessageBox.Show(
                "Are you sure you want to delete this scenario chain?\n" + chain +
                "\n\nThis operation cannot be undone. Continue?", "Confirm delete", MessageBoxButtons.YesNo) !=
            DialogResult.Yes) return;

        var delpoints = false;
        if (MessageBox.Show("Delete all Scenario Points for this chain as well?", "Confirm delete points",
                MessageBoxButtons.YesNo) == DialogResult.Yes) delpoints = true;
        ...
        if (WorldForm != null)
        {
            ...
            if (delpoints) WorldForm.SelectItem();
        }
    ...
    public void DeleteScenarioCluster()
    {
        ...
        if (MessageBox.Show(
                "Are you sure you want to delete this scenario cluster?\n" + cluster +
                "\n\nThis operation cannot be undone. Continue?", "Confirm delete", MessageBoxButtons.YesNo) !=
            DialogResult.Yes) return;

        var delpoints = false;
        if (MessageBox.Show("Delete all Scenario Points for this cluster as well?", "Confirm delete points",
                MessageBoxButtons.YesNo) == DialogResult.Yes) delpoints = true;
        ...
        {
            WorldForm.UpdateScenarioGraphics(cs, false);
            if (delpoints || (cn != null && cn.MyPoint == null)) WorldForm.SelectItem();
        }
        else if (cs?.ScenarioRegion != null)
        {
            cs.ScenarioRegion.BuildBVH();
    ...
    public void DeleteScenarioEntity()
    {
        ...
        if (MessageBox.Show(
                "Are you sure you want to delete this scenario entity override, and all its override points?\n" +
                entity + "\n\nThis operation cannot be undone. Continue?", "Confirm delete", MessageBoxButtons.YesNo) !=
            DialogResult.Yes) return;
    ...
    public void ImportScenarioChain()
    {
        ...
        var f = new TextInputForm();
        f.TitleText = "Import scenario chain points";
        f.PromptText =
            "Input chain points in CSV (or TSV) format. Direction is in radians. NavSpeed is from 0 to 15. NavMode can be either Direct, NavMesh, or Roads. ScenarioType is the name of the scenario type to use.";
        ...
        if (stypes != null) defaulttype = stypes.GetScenarioTypeRef(1194480618); //"drive";
        ...
        if (lines.Length < 2) return; //need at least 2 lines (1 point) to work with...
        ...
        for (var i = 0; i < lines.Length; i++)
        {
            ...
            if (vals.Length > 5)
                ...
                {
                    case "Direct":
                        navMode = CScenarioChainingEdge__eNavMode.Direct;
                        break;
                    case "NavMesh":
                        navMode = CScenarioChainingEdge__eNavMode.NavMesh;
                        break;
                    case "Roads":
                        navMode = CScenarioChainingEdge__eNavMode.Roads;
                        break;
                }
            ...
            if (vals.Length > 6)
            {
                ...
                if (stype == null) stype = defaulttype;
            }
            ...
            if (vals.Length > 8)
                if (!uint.TryParse(vals[8].Trim(), out flags))
                    flags = defaultflags;
    ...
    public void NewAudioFile()
    {
        ...
        lock (ProjectSyncRoot)
        {
            ...
            if (rel != null) rel.RelType = RelDatFileType.Dat151; //TODO: different types
        }
    ...
    public void SaveAudioFile(bool saveas = false)
    {
        ...
        var relname = CurrentAudioFile.Name;
        var filepath = CurrentAudioFile.FilePath;
        if (string.IsNullOrEmpty(filepath)) filepath = relname;
        var origfile = filepath;
        if (!File.Exists(filepath)) saveas = true;
        ...
        if (saveas)
        {
            ...
            {
                var origpath = CurrentProjectFile.GetRelativePath(origfile);
                var newpath = CurrentProjectFile.GetRelativePath(CurrentAudioFile.FilePath);
    ...
    public AudioPlacement NewAudioAmbientZone(AudioPlacement copy = null, bool copyPosition = false,
        bool selectNew = true)
    {
        if (CurrentAudioFile == null) return null;

        if (copy == null) copy = CurrentAudioAmbientZone;
        ...
        var pos = cp ? copy.Position : GetSpawnPos(20.0f);
        var ori = cp ? copy.Orientation : Quaternion.Identity;
        ap.SetPosition(pos);
        ...
        if (WorldForm != null) WorldForm.UpdateAudioPlacementGraphics(CurrentAudioFile);
    ...
    public bool DeleteAudioAmbientZone()
    {
        ...
        if (!res) MessageBox.Show("Unspecified error occurred when removing the audio zone from the file!");
    ...
    public AudioPlacement NewAudioAmbientRule(AudioPlacement copy = null, bool copyPosition = false,
        bool selectNew = true)
    {
        if (CurrentAudioFile == null) return null;

        if (copy == null) copy = CurrentAudioAmbientRule;
    ...
    public bool DeleteAudioAmbientRule()
    {
        ...
        if (!res) MessageBox.Show("Unspecified error occurred when removing the audio rule from the file!");
    ...
    public AudioPlacement NewAudioStaticEmitter(AudioPlacement copy = null, bool copyPosition = false,
        bool selectNew = true)
    {
        if (CurrentAudioFile == null) return null;

        if (copy == null) copy = CurrentAudioStaticEmitter;
    ...
    public bool DeleteAudioStaticEmitter()
    {
        ...
        if (!res) MessageBox.Show("Unspecified error occurred when removing the static emitter from the file!");
        ...
        ClosePanel((EditAudioStaticEmitterPanel p) =>
        {
            return p.CurrentEmitter.StaticEmitter == delem.StaticEmitter;
        });
    ...
    public bool DeleteAudioAmbientZoneList()
    {
        ...
        if (!res) MessageBox.Show("Unspecified error occurred when removing the audio zone list from the file!");
    ...
    public bool DeleteAudioStaticEmitterList()
    {
        ...
        if (!res) MessageBox.Show("Unspecified error occurred when removing the audio emitter list from the file!");
    ...
    public bool DeleteAudioInterior()
    {
        ...
        if (MessageBox.Show(
                "Are you sure you want to delete this audio interior?\n" + CurrentAudioInterior.GetNameString() +
                "\n\nThis operation cannot be undone. Continue?", "Confirm delete", MessageBoxButtons.YesNo) !=
            DialogResult.Yes) return true;
        ...
        if (!res) MessageBox.Show("Unspecified error occurred when removing the audio interior from the file!");
    ...
    public bool DeleteAudioInteriorRoom()
    {
        ...
        if (MessageBox.Show(
                "Are you sure you want to delete this audio interior room?\n" +
    ...
    {
        if (hidegtavmap) ymaps.Clear(); //remove all the gtav ymaps.
        ...
        lock (ProjectSyncRoot)
        {
            ...
            {
                for (var i = 0; i < CurrentProjectFile.YmapFiles.Count; i++)
                {
                ...
                foreach (var kvp in ymaps) //TODO: improve performance
                {
                    ...
                    if (ymap.AllEntities != null) //THIS IS TERRIBLE! EATING ALL FPS
                        foreach (var ent in ymap.AllEntities) //WHYYYY - maybe only do this after loading/editing ytyp!
                        {
                            ...
                            if (arch != null && ent.Archetype != arch)
                            {
                                ...
                                if (ent.IsMlo) ent.MloInstance.InitYmapEntityArchetypes(GameFileCache);
                            }
                        }
                    ...
                    if (ymap.MloEntities != null)
                        ...
                        {
                            if (mloDef.Archetype ==
                                null) continue; // archetype was changed from an mlo to a regular archetype
    ...
    public void GetVisibleYbns(Camera camera, List<YbnFile> ybns, Dictionary<YmapEntityDef, YbnFile> interiors)
    {
        ...
        lock (ProjectSyncRoot)
        {
            ...
            visibleybns.Clear();
            for (var i = 0; i < ybns.Count; i++)
            {
                var ybn = ybns[i];
                visibleybns[ybn.Name] = ybn;
            }

            for (var i = 0; i < CurrentProjectFile.YbnFiles.Count; i++)
            {
            ...
            ybns.Clear();
            foreach (var ybn in visibleybns.Values) ybns.Add(ybn);
            ...
            {
                var ybn = CurrentProjectFile.YbnFiles[i];
                if (ybn.Loaded) projectybns[ybn.RpfFileEntry?.ShortNameHash ?? JenkHash.GenHash(ybn.Name)] = ybn;
            }

            interiorslist.Clear();
            interiorslist.AddRange(interiors.Keys);
            for (var i = 0; i < interiorslist.Count; i++)
            {
    ...
    public void GetVisibleYnds(Camera camera, List<YndFile> ynds)
    {
        ...
        lock (ProjectSyncRoot)
        {
            ...
            visibleynds.Clear();
            for (var i = 0; i < ynds.Count; i++)
            {
                var ynd = ynds[i];
                visibleynds[ynd.AreaID] = ynd;
            }

            for (var i = 0; i < CurrentProjectFile.YndFiles.Count; i++)
            {
                var ynd = CurrentProjectFile.YndFiles[i];
                if (ynd.Loaded) visibleynds[ynd.AreaID] = ynd;
            }

            ynds.Clear();
            foreach (var ynd in visibleynds.Values) ynds.Add(ynd);
        }
    }

    public void GetVisibleYnvs(Camera camera, List<YnvFile> ynvs)
    {
        if (hidegtavmap) ynvs.Clear();
        ...
        lock (ProjectSyncRoot)
        {
            ...
            visibleynvs.Clear();
            for (var i = 0; i < ynvs.Count; i++)
            {
                var ynv = ynvs[i];
                visibleynvs[ynv.AreaID] = ynv;
            }

            for (var i = 0; i < CurrentProjectFile.YnvFiles.Count; i++)
            {
                var ynv = CurrentProjectFile.YnvFiles[i];
                if (ynv.Loaded) visibleynvs[ynv.AreaID] = ynv;
            }

            ynvs.Clear();
            foreach (var ynv in visibleynvs.Values) ynvs.Add(ynv);
        }
    }

    public void GetVisibleTrainTracks(Camera camera, List<TrainTrack> tracks)
    {
        if (hidegtavmap) tracks.Clear();
        ...
        lock (ProjectSyncRoot)
        {
            ...
            for (var i = 0; i < CurrentProjectFile.TrainsFiles.Count; i++)
            {
            ...
            tracks.Clear();
            foreach (var track in visibletrains.Values) tracks.Add(track);
        }
    }

    public void GetVisibleScenarios(Camera camera, List<YmtFile> ymts)
    {
        if (hidegtavmap) ymts.Clear();
        ...
        lock (ProjectSyncRoot)
        {
            ...
            for (var i = 0; i < ymts.Count; i++)
            {
                var ymt = ymts[i];
                visiblescenarios[ymt.Name] = ymt;
            }

            for (var i = 0; i < CurrentProjectFile.ScenarioFiles.Count; i++)
            {
                var scenario = CurrentProjectFile.ScenarioFiles[i];
                if (scenario.Loaded) visiblescenarios[scenario.Name] = scenario;
            }

            ymts.Clear();
            foreach (var ymt in visiblescenarios.Values) ymts.Add(ymt);
        }
    }

    public void GetVisibleAudioFiles(Camera camera, List<RelFile> rels)
    {
        if (hidegtavmap) rels.Clear();
        ...
        lock (ProjectSyncRoot)
        {
            ...
            for (var i = 0; i < CurrentProjectFile.AudioRelFiles.Count; i++)
            {
                var rel = CurrentProjectFile.AudioRelFiles[i];
                if (rel.Loaded) visibleaudiofiles[rel.RpfFileEntry.NameHash] = rel;
            }

            rels.Clear();
            foreach (var rel in visibleaudiofiles.Values) rels.Add(rel);
        }
    }

    public void GetVisibleWaterQuads<T>(Camera camera, List<T> quads) where T : BaseWaterQuad
    {
        if (hidegtavmap) quads.Clear();
    }
    ...
    public void GetMouseCollision(Camera camera, ref MapSelection curHit)
    {
        ...
        if (hidegtavmap && curybn != null) curHit.Clear();
        ...
        lock (ProjectSyncRoot)
        {
            if (renderitems && CurrentProjectFile != null)
            {
                for (var i = 0; i < CurrentProjectFile.YbnFiles.Count; i++)
                {
                    ...
                    if (ybn.Loaded)
                    {
                        if (ybn.Name == curybn?.Name) curHit.Clear();
    ...
    public MloInstanceData TryGetMloInstance(MloArchetype arch)
    {
        lock (ProjectSyncRoot)
        {
            ...
            var name = arch._BaseArchetypeDef.name;
    ...
    public void OnWorldSelectionChanged(MapSelection sel)
    {
        ...
        try
        {
            ...
            var arch = mlo?.Archetype ?? ent?.MloParent?.Archetype ?? ent?.Archetype;
            var ytyp = mlo?.Archetype?.Ytyp ??
                       ent?.MloParent?.Archetype?.Ytyp ?? ent?.Archetype?.Ytyp ?? room?.OwnerMlo?.Ytyp;
            var ymap = ent?.Ymap ?? cargen?.Ymap ??
                lodlight?.Ymap ?? boxoccluder?.Ymap ?? occludetri?.Ymap ?? grassbatch?.Ymap ?? mlo?.Ymap;
            var ybn = collbound?.GetRootYbn();
            var ynd = pathnode?.Ynd;
            var ynv = navpoly?.Ynv ?? navpoint?.Ynv ?? navportal?.Ynv;
            var traintrack = trainnode?.Track;
            var scenario = scenariond?.Ymt ?? scenarioedge?.Region?.Ymt;
            var audiofile = audiopl?.RelFile;
            var showcurrent = false;

            if (YmapExistsInProject(ymap) && ybn == null)
            {
                if (wasmult || ent != CurrentEntity) ProjectExplorer?.TrySelectEntityTreeNode(ent);
                if (wasmult || cargen != CurrentCarGen) ProjectExplorer?.TrySelectCarGenTreeNode(cargen);
                if (wasmult || lodlight != CurrentLodLight) ProjectExplorer?.TrySelectLodLightTreeNode(lodlight);
            ...
            {
                if (wasmult || arch != CurrentArchetype) ProjectExplorer?.TrySelectArchetypeTreeNode(mlo?.Archetype);
                if (wasmult || ent != CurrentEntity)
                {
                    var mloInstance = ent.MloParent?.MloInstance;
                    if (mloInstance != null)
                    {
                        var entityDef = mloInstance.TryGetArchetypeEntity(ent);
                        ProjectExplorer?.TrySelectMloEntityTreeNode(entityDef);
                    }
                }

                if (wasmult || room != CurrentMloRoom) ProjectExplorer?.TrySelectMloRoomTreeNode(room);
            }
            ...
            {
                if (wasmult || pathnode != CurrentPathNode) ProjectExplorer?.TrySelectPathNodeTreeNode(pathnode);
            }
            else if (YnvExistsInProject(ynv))
            {
                if (wasmult || navpoly != CurrentNavPoly) ProjectExplorer?.TrySelectNavPolyTreeNode(navpoly);
                if (wasmult || navpoint != CurrentNavPoint) ProjectExplorer?.TrySelectNavPointTreeNode(navpoint);
                if (wasmult || navportal != CurrentNavPortal) ProjectExplorer?.TrySelectNavPortalTreeNode(navportal);
            }
            else if (TrainTrackExistsInProject(traintrack))
            {
                if (wasmult || trainnode != CurrentTrainNode) ProjectExplorer?.TrySelectTrainNodeTreeNode(trainnode);
            }
    ...
    public void OnWorldSelectionModified(MapSelection sel)
    {
        try
        {
            ...
            else
            {
                ...
                else if (sel.Audio != null) OnWorldAudioPlacementModified(sel.Audio);
            }
        }
        catch
        {
    ...
    {
        if (items == CurrentMulti) ShowEditMultiPanel(false);
    }

    private void OnWorldEntityModified(YmapEntityDef ent)
    {
        if (ent.Ymap == null && ent.MloParent == null) return; //TODO: properly handle interior entities!
        ...
        if (ent.MloParent == null && ent.Ymap != null)
        {
            ...
            if (ent == CurrentEntity)
            {
                ...
                if (ent.Ymap != null) SetYmapHasChanged(true);
            }
        }
        else if (ent.MloParent != null && ent.Ymap == null)
        {
            ...
            if (ent == CurrentEntity)
            {
                ...
                if (ent.MloParent.Archetype.Ytyp != null) SetYtypHasChanged(true);
            }
        }
    }
    ...
    private void OnWorldCarGenModified(YmapCarGen cargen)
    {
        ...
        if (cargen == CurrentCarGen)
        {
            ...
            if (cargen.Ymap != null) SetYmapHasChanged(true);
        }
    }
    ...
    private void OnWorldLodLightModified(YmapLODLight lodlight)
    {
        ...
        if (lodlight == CurrentLodLight)
        {
            ...
            if (lodlight.Ymap != null) SetYmapHasChanged(true);
        }
    }
    ...
    private void OnWorldBoxOccluderModified(YmapBoxOccluder box)
    {
        ...
        if (box == CurrentBoxOccluder)
        {
            ...
            if (box.Ymap != null) SetYmapHasChanged(true);
        }
    }
    ...
    private void OnWorldOccludeModelTriModified(YmapOccludeModelTriangle tri)
    {
        ...
        if (tri == CurrentOccludeModelTri)
        {
            ...
            if (tri.Ymap != null) SetYmapHasChanged(true);
        }
    }
    ...
    private void OnWorldCollisionVertexModified(BoundVertex vert)
    {
        ...
        if (vert == CurrentCollisionVertex)
        {
            ...
            if (ybn != null) SetYbnHasChanged(true);
        }
    }
    ...
    private void OnWorldPathNodeModified(YndNode node, YndLink link)
    {
        ...
        if (node == CurrentPathNode)
        {
            ...
            if (node.Ynd != null) SetYndHasChanged(true);
        }
    }
    ...
    private void OnWorldNavPolyModified(YnvPoly poly)
    {
        ...
        if (poly == CurrentNavPoly)
        {
            ...
            if (poly.Ynv != null) SetYnvHasChanged(true);
        }
    }
    ...
    private void OnWorldNavPointModified(YnvPoint point)
    {
        ...
        if (point == CurrentNavPoint)
        {
            ...
            if (point.Ynv != null) SetYnvHasChanged(true);
        }
    }
    ...
    private void OnWorldNavPortalModified(YnvPortal portal)
    {
        ...
        if (portal == CurrentNavPortal)
        {
            ...
            if (portal.Ynv != null) SetYnvHasChanged(true);
        }
    }
    ...
    private void OnWorldTrainNodeModified(TrainTrackNode node)
    {
        ...
        if (node == CurrentTrainNode)
        {
            ...
            if (node.Track != null) SetTrainTrackHasChanged(true);
        }
    }
    ...
    private void OnWorldScenarioNodeModified(ScenarioNode node)
    {
        ...
        if (node == CurrentScenarioNode)
        {
            ...
            if (node?.Ymt != null) SetScenarioHasChanged(true);
        }
    }
    ...
    private void OnWorldAudioPlacementModified(AudioPlacement audio)
    {
        ...
        if (!AudioFileExistsInProject(audio.RelFile))
        {
            ...
            if (audio.AmbientZone != null) ProjectExplorer?.TrySelectAudioAmbientZoneTreeNode(audio);
            if (audio.AmbientRule != null) ProjectExplorer?.TrySelectAudioAmbientRuleTreeNode(audio);
            if (audio.StaticEmitter != null) ProjectExplorer?.TrySelectAudioStaticEmitterTreeNode(audio);
        }
        ...
        {
            ShowEditAudioAmbientZonePanel(false);
            if (audio.RelFile != null) SetAudioFileHasChanged(true);
        }
    ...
    public void SetYmapHasChanged(bool changed)
    {
        ...
        var changechange = changed != CurrentYmapFile.HasChanged;
    ...
    public void SetYtypHasChanged(bool changed)
    {
        ...
        var changechange = changed != CurrentYtypFile.HasChanged;
        if (!changechange) return;
    ...
    public void SetYbnHasChanged(bool changed)
    {
        ...
        var changechange = changed != CurrentYbnFile.HasChanged;
        if (!changechange) return;
    ...
    public void SetYndHasChanged(YndFile yndFile, bool changed, bool force = false)
    {
        ...
        var changechange = changed != yndFile.HasChanged;
    ...
    public void SetYnvHasChanged(bool changed)
    {
        ...
        var changechange = changed != CurrentYnvFile.HasChanged;
        if (!changechange) return;
    ...
    public void SetTrainTrackHasChanged(bool changed)
    {
        ...
        var changechange = changed != CurrentTrainTrack.HasChanged;
        if (!changechange) return;
    ...
    public void SetScenarioHasChanged(bool changed)
    {
        ...
        var changechange = changed != CurrentScenario.HasChanged;
        if (!changechange) return;
    ...
    public void SetAudioFileHasChanged(bool changed)
    {
        ...
        var changechange = changed != CurrentAudioFile.HasChanged;
        if (!changechange) return;
    ...
    public void SetGrassBatchHasChanged(bool changed)
    {
        ...
        var changechange = changed != CurrentGrassBatch.HasChanged;
        if (!changechange) return;
    ...
    public Vector3 GetSpawnPos(float dist)
    {
        ...
        if (WorldForm != null)
        {
            var campos = WorldForm.GetCameraPosition();
            var camdir = WorldForm.GetCameraViewDir();
    ...
    public Vector3 GetSpawnPosRel(float dist, Vector3 relPos, Quaternion relRot)
    {
        ...
        if (WorldForm != null)
        {
            ...
            var rot = Quaternion.Invert(relRot);
            var delta = pos - relPos;
            var relativePos = rot.Multiply(delta);
            pos = relativePos;
        }
    ...
    public RpfFileEntry FindParentYmapEntry(uint hash)
    {
        if (CurrentProjectFile != null)
            foreach (var ymap in CurrentProjectFile.YmapFiles)
                if (ymap._CMapData.name.Hash == hash ||
                    JenkHash.GenHash(Path.GetFileNameWithoutExtension(ymap.Name)) == hash)
                    return ymap.RpfFileEntry;

        if (GameFileCache != null && GameFileCache.IsInited) return GameFileCache.GetYmapEntry(hash);
    ...
    private void LoadYndFromFile(YndFile ynd, string filename)
    {
        ...
        if (WorldForm != null)
            ...
            foreach (var file in CurrentProjectFile.YndFiles)
            {
                ...
                {
                    if (CurrentProjectFile.ContainsYnd(affected)) continue;
    ...
    private void LoadTrainTrackFromFile(TrainTrack track, string filename)
    {
        ...
        var fname = new FileInfo(filename).Name;
    ...
    private void LoadYdrFromFile(YdrFile ydr, string filename)
    {
        ...
        if (GameFileCache != null) GameFileCache.AddProjectFile(ydr);
    }
    ...
    private void LoadYddFromFile(YddFile ydd, string filename)
    {
        ...
        if (GameFileCache != null) GameFileCache.AddProjectFile(ydd);
    }
    ...
    private void LoadYftFromFile(YftFile yft, string filename)
    {
        ...
        if (GameFileCache != null) GameFileCache.AddProjectFile(yft);
    }
    ...
    private void LoadYtdFromFile(YtdFile ytd, string filename)
    {
        ...
        if (GameFileCache != null) GameFileCache.AddProjectFile(ytd);
    }
    ...
    {
        var enable = CurrentProjectFile != null;
    ...
    {
        var enable = CurrentYmapFile != null;
        var inproj = YmapExistsInProject(CurrentYmapFile);
        ...
        if (WorldForm != null) WorldForm.EnableYmapUI(enable, CurrentYmapFile?.Name ?? "");
    }
    ...
    {
        var enable = CurrentYtypFile != null;
        var inproj = YtypExistsInProject(CurrentYtypFile);
        var ismlo = (CurrentEntity != null && CurrentEntity.MloParent != null) || CurrentMloRoom != null ||
                    CurrentMloPortal != null || CurrentMloEntitySet != null || CurrentArchetype is MloArchetype;
    ...
    {
        var enable = CurrentYbnFile != null;
        var inproj = YbnExistsInProject(CurrentYbnFile);
        ...
        if (WorldForm != null) WorldForm.EnableYbnUI(enable, CurrentYbnFile?.Name ?? "");
    }

    private void RefreshYndUI()
    {
        var enable = CurrentYndFile != null;
        var inproj = YndExistsInProject(CurrentYndFile);
        ...
        if (WorldForm != null) WorldForm.EnableYndUI(enable, CurrentYndFile?.Name ?? "");
    }

    private void RefreshYnvUI()
    {
        var enable = CurrentYnvFile != null;
        var inproj = YnvExistsInProject(CurrentYnvFile);
        ...
        if (WorldForm != null) WorldForm.EnableYnvUI(enable, CurrentYnvFile?.Name ?? "");
    }

    private void RefreshTrainTrackUI()
    {
        var enable = CurrentTrainTrack != null;
        var inproj = TrainTrackExistsInProject(CurrentTrainTrack);
        ...
        if (WorldForm != null) WorldForm.EnableTrainsUI(enable, CurrentTrainTrack?.Name ?? "");
    }

    private void RefreshScenarioUI()
    {
        var enable = CurrentScenario != null;
        var inproj = ScenarioExistsInProject(CurrentScenario);
        ...
        if (WorldForm != null) WorldForm.EnableScenarioUI(enable, CurrentScenario?.Name ?? "");
    }

    private void RefreshAudioUI()
    {
        var enable = CurrentAudioFile != null;
        var inproj = AudioFileExistsInProject(CurrentAudioFile);
        ...
        if (WorldForm != null) WorldForm.EnableAudioUI(enable, CurrentAudioFile?.Name ?? "");
    }

    private void RefreshYdrUI()
    {
        var enable = CurrentYdrFile != null;
        var inproj = YdrExistsInProject(CurrentYdrFile);
    ...
    {
        var enable = CurrentYddFile != null;
        var inproj = YddExistsInProject(CurrentYddFile);
    ...
    {
        var enable = CurrentYftFile != null;
        var inproj = YftExistsInProject(CurrentYftFile);
    ...
    {
        var enable = CurrentYtdFile != null;
        var inproj = YtdExistsInProject(CurrentYtdFile);
    ...
    private void SetCurrentSaveItem()
    {
        ...
        else if (CurrentAudioFile != null) filename = CurrentAudioFile.RpfFileEntry?.Name;
        ...
        if (WorldForm != null) WorldForm.SetCurrentSaveItem(filename);
    }
    ...
    private string ShowSaveDialog(string filter, string filename)
    {
        ...
        if (SaveFileDialog.ShowDialog(this) != DialogResult.OK) return string.Empty;
    ...
    private string ShowOpenDialog(string filter, string filename)
    {
        ...
        if (OpenFileDialog.ShowDialog(this) != DialogResult.OK) return string.Empty;
    ...
    private string[] ShowOpenDialogMulti(string filter, string filename)
    {
        ...
        if (OpenFileDialog.ShowDialog(this) != DialogResult.OK) return null;
    ...
    {
        if (WorldForm != null) WorldForm.OnProjectFormClosed();
    }
    ...
    private void MainDockPanel_ActiveContentChanged(object sender, EventArgs e)
    {
        if (!ShowProjectItemInProcess)
        {
            ...
            if (panel != null) MainDockPanel.DefaultFloatWindowSize = panel.Size;
            if (panel?.Tag != null)
            {
    ...
    private void OptionsDisplayEntityIndexesMenu_Click(object sender, EventArgs e)
    {
        ...
        if (CurrentEntity != null)
        {
            ...
            {
                var entityDef = mloInstance.TryGetArchetypeEntity(CurrentEntity);
                ProjectExplorer?.TrySelectMloEntityTreeNode(entityDef);
            }
            else
            {
