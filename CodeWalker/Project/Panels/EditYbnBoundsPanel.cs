...
public partial class EditYbnBoundsPanel : ProjectPanel
{
    ...
    private void LoadDropDowns()
    {
        ...
        foreach (var mat in BoundsMaterialTypes.Materials) MaterialCombo.Items.Add(mat);
    }
    ...
    {
        var changed = CollisionBounds?.HasChanged ?? false;
    ...
    {
        var fn = CollisionBounds?.GetTitle() ?? "untitled";
    ...
    public void UpdateUI()
    {
        ...
        else
        {
            ...
            if (b is BoundGeometry bg)
            {
                ...
                VertexCountLabel.Text = bg.VerticesCount + (bg.VerticesCount == 1 ? " vertex" : " vertices");
                PolyCountLabel.Text = bg.PolygonsCount + (bg.PolygonsCount == 1 ? " polygon" : " polygons");
            }
            else
            {
                BoundsTabControl.TabPages.Remove(GeometryTabPage);
    ...
    private uint GetCheckedListBoxValues(CheckedListBox clb, ItemCheckEventArgs e)
    {
        ...
        for (var i = 0; i < clb.Items.Count; i++)
            if (e != null && e.Index == i)
            {
                if (e.NewValue == CheckState.Checked) r += (uint)(1 << i);
            }
            else
            {
