...
public partial class EditYmapEntityPanel : ProjectPanel
{
    ...
    public void SetEntity(YmapEntityDef entity)
    {
        ...
        var instance = entity?.MloParent?.MloInstance;
    ...
    private void LoadEntity(bool sameEntity)
    {
        ...
        else
        {
            ...
            EntityAddToProjectButton.Enabled = CurrentEntity.Ymap != null
                ? !ProjectForm.YmapExistsInProject(CurrentEntity.Ymap)
                : !ProjectForm.YtypExistsInProject(CurrentEntity.MloParent?.Archetype?.Ytyp);
            EntityDeleteButton.Enabled = !EntityAddToProjectButton.Enabled;
            EntityArchetypeTextBox.Text = e.archetypeName.ToString();
            EntityArchetypeHashLabel.Text = "# " + e.archetypeName.Hash;
            ...
            EntityLodLevelComboBox.SelectedIndex =
                Math.Max(EntityLodLevelComboBox.FindString(e.lodLevel.ToString()), 0);
            EntityNumChildrenTextBox.Text = e.numChildren.ToString();
            EntityPriorityLevelComboBox.SelectedIndex =
                Math.Max(EntityPriorityLevelComboBox.FindString(e.priorityLevel.ToString()), 0);
            ...
            for (var i = 0; i < EntityFlagsCheckedListBox.Items.Count; i++)
            {
            ...
            if (rotupd && sameEntity) UpdateRotationFlag();


            var ms = ProjectForm.WorldForm?.CurrentMapSelection;
            if (ms?.EntityDef != CurrentEntity && ms?.MloEntityDef != CurrentEntity)
                ProjectForm.WorldForm
                    ?.SelectObject(
                        CurrentEntity); //hopefully the drawable is already loaded - this will try get from cache
    ...
    private void UpdateTabVisibility()
    {
        ...
        var ok = true;
        var miloTabVis = false;
        foreach (var tab in EntityTabControl.TabPages)
            if (tab == EntityMiloTabPage)
                miloTabVis = true;
        ...
        if (EntityTabControl.TabPages.Contains(seltab)) EntityTabControl.SelectedTab = seltab;
    }
    ...
    private void EntityArchetypeTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentEntity == null) return;
        uint hash = 0;
        var name = EntityArchetypeTextBox.Text;
        ...
        EntityArchetypeHashLabel.Text = "# " + hash;

        var arch = ProjectForm.GameFileCache.GetArchetype(hash);
        if (arch == null) EntityArchetypeHashLabel.Text += " (not found)";
        ...
        if (CurrentEntity != null)
            lock (ProjectForm.ProjectSyncRoot)
            {
                ...
                if (CurrentMCEntity != null) CurrentMCEntity._Data.archetypeName = new MetaHash(hash);
    ...
    private void ProjectItemChanged()
    {
        ...
        else if (CurrentEntity.MloParent?.Archetype?.Ytyp != null) ProjectForm.SetYtypHasChanged(true);
    }
    ...
    private void EntityFlagsCheckedListBox_ItemCheck(object sender, ItemCheckEventArgs e)
    {
        ...
        for (var i = 0; i < EntityFlagsCheckedListBox.Items.Count; i++)
            ...
            {
                if (EntityFlagsCheckedListBox.GetItemChecked(i)) flags += (uint)(1 << i);
            }
    ...
    private void EntityPositionTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentEntity == null) return;
        var v = FloatUtil.ParseVector3String(EntityPositionTextBox.Text);
        lock (ProjectForm.ProjectSyncRoot)
        {
            if (CurrentEntity._CEntityDef.position != v)
            {
                ...
                if (wf != null)
                    wf.BeginInvoke(() => { wf.SetWidgetPosition(CurrentEntity.WidgetPosition, true); });
            }
        }
    }
    ...
    private void EntityRotationQuatBox_ValueChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentEntity == null) return;
        var q = EntityRotationQuatBox.Value;
        var v = q.ToVector4();
        lock (ProjectForm.ProjectSyncRoot)
        {
            if (CurrentEntity._CEntityDef.rotation != v)
            {
                ...
                ProjectItemChanged();
                var wf = ProjectForm.WorldForm;
                wf?.BeginInvoke(() => { wf.SetWidgetRotation(CurrentEntity.WidgetOrientation, true); });
            }
        }
    ...
    private void EntityScaleXYTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        lock (ProjectForm.ProjectSyncRoot)
        {
            ...
            {
                var newscale = new Vector3(sxy, sxy, CurrentEntity.Scale.Z);
                CurrentEntity.SetScale(newscale);
                ProjectItemChanged();
                var wf = ProjectForm.WorldForm;
                if (wf != null)
                    wf.BeginInvoke(() => { wf.SetWidgetScale(newscale, true); });
            }
        }
    }
    ...
    private void EntityScaleZTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        lock (ProjectForm.ProjectSyncRoot)
        {
            ...
            {
                var newscale = new Vector3(CurrentEntity.Scale.X, CurrentEntity.Scale.Y, sz);
                CurrentEntity.SetScale(newscale);
                ProjectItemChanged();
                var wf = ProjectForm.WorldForm;
                if (wf != null)
    ...
    private void EntityLodLevelComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentEntity == null) return;
        var lodLevel = (rage__eLodType)EntityLodLevelComboBox.SelectedItem;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void EntityPriorityLevelComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentEntity == null) return;
        var priorityLevel = (rage__ePriorityLevel)EntityPriorityLevelComboBox.SelectedItem;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    {
        if (ProjectForm.WorldForm != null) ProjectForm.WorldForm.EditEntityPivot = EntityPivotEditCheckBox.Checked;
    }
    ...
    private void EntityPivotPositionTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentEntity == null) return;
        var v = FloatUtil.ParseVector3String(EntityPivotPositionTextBox.Text);
        lock (ProjectForm.ProjectSyncRoot)
        {
            if (CurrentEntity.PivotPosition != v)
            {
                ...
                if (wf != null)
                    wf.BeginInvoke(() =>
                    {
                        var editpivot = wf.EditEntityPivot;
    ...
    private void EntityPivotRotationQuatBox_ValueChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentEntity == null) return;
        var q = EntityPivotRotationQuatBox.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
