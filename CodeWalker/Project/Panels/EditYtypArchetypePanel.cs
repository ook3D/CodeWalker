...
public partial class EditYtypArchetypePanel : ProjectPanel
{
    ...
    private void UpdateControls()
    {
        if (CurrentArchetype != null)
        {
            ...
            {
                if (!TabControl.TabPages.Contains(MloArchetypeTabPage)) TabControl.TabPages.Add(MloArchetypeTabPage);
            ...
            {
                if (!TabControl.TabPages.Contains(TimeArchetypeTabPage)) TabControl.TabPages.Add(TimeArchetypeTabPage);
    ...
    private void ArchetypeNameTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentArchetype._BaseArchetypeDef.name != hash)
        {
            ...
            var tn = ProjectForm.ProjectExplorer?.FindArchetypeTreeNode(CurrentArchetype);
            if (tn != null)
                tn.Text = ArchetypeNameTextBox.Text ??
                          "0"; // using the text box text because the name may not be in the gfc.
    ...
    private void TextureDictTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        lock (ProjectForm.ProjectSyncRoot)
        {
            ...
            if (CurrentArchetype._BaseArchetypeDef.textureDictionary != hash)
            {
                ...
                if (ytd == null)
                {
                    TextureDictHashLabel.Text = "# " + hash + " (invalid)";
                    ProjectForm.SetYtypHasChanged(true);
                    return;
                }
            ...
            TextureDictHashLabel.Text = "# " + hash;
        }
    }
    ...
    private void DrawableDictionaryTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        lock (ProjectForm.ProjectSyncRoot)
        {
            ...
            if (CurrentArchetype._BaseArchetypeDef.drawableDictionary != hash)
            {
                ...
                if (ydd == null)
                {
                    DrawableDictHashLabel.Text = "# " + hash + " (invalid)";
                    ProjectForm.SetYtypHasChanged(true);
                    return;
                }
            ...
            DrawableDictHashLabel.Text = "# " + hash;
        }
    }
    ...
    private void PhysicsDictionaryTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        lock (ProjectForm.ProjectSyncRoot)
        {
            ...
            if (CurrentArchetype._BaseArchetypeDef.physicsDictionary != hash)
            {
                ...
                if (ybn == null)
                {
                    PhysicsDictHashLabel.Text = "# " + hash + " (invalid)";
                    ProjectForm.SetYtypHasChanged(true);
                    return;
                }
            ...
            PhysicsDictHashLabel.Text = "# " + hash;
        }
    }
    ...
    {
        var min = FloatUtil.ParseVector3String(BBMinTextBox.Text);
    ...
    {
        var max = FloatUtil.ParseVector3String(BBMaxTextBox.Text);
    ...
    {
        var c = FloatUtil.ParseVector3String(BSCenterTextBox.Text);
    ...
    {
        if (FloatUtil.TryParse(BSRadiusTextBox.Text, out var f))
        {
    ...
    private void TimeFlagsTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentArchetype is TimeArchetype TimeArchetype)
        {
            ...
            populatingui = true;
            for (var i = 0; i < TimeFlagsCheckedListBox.Items.Count; i++)
            {
                var c = (flags & (1u << i)) > 0;
    ...
    private void TimeFlagsCheckedListBox_ItemCheck(object sender, ItemCheckEventArgs e)
    {
        ...
        if (CurrentArchetype is TimeArchetype TimeArchetype)
        {
            ...
            for (var i = 0; i < TimeFlagsCheckedListBox.Items.Count; i++)
                ...
                {
                    if (TimeFlagsCheckedListBox.GetItemChecked(i)) flags += (uint)(1 << i);
                }
