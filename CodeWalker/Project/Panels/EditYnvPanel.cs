...
public partial class EditYnvPanel : ProjectPanel
{
    ...
    {
        var fn = Ynv.RpfFileEntry?.Name ?? Ynv.Name;
    ...
    public void UpdateYnvUI()
    {
        ...
        else
        {
            ...
            YnvProjectPathTextBox.Text =
                Ynv != null ? ProjectForm.CurrentProjectFile.GetRelativePath(Ynv.FilePath) : Ynv.FilePath;
            YnvAreaIDXUpDown.Value = Ynv.CellX;
            YnvAreaIDYUpDown.Value = Ynv.CellY;
            YnvAreaIDInfoLabel.Text = "ID: " + Ynv.AreaID;
            ...
            YnvVertexCountLabel.Text = "Vertex count: " + nv.VerticesCount;
            YnvPolyCountLabel.Text = "Poly count: " + nv.PolysCount;
            YnvPortalCountLabel.Text = "Portal count: " + nv.PortalsCount;
            YnvPortalLinkCountLabel.Text = "Portal link count: " + nv.PortalLinksCount;
            YnvPointCountLabel.Text = "Point count: " + nv.PointsCount;
            YnvByteCountLabel.Text = "Byte count: " + nv.TotalBytes;
    ...
    private uint[] GetAdjAreaIdsArray(string text)
    {
        var rsplit = new[] { '\n' };
        var csplit = new[] { ' ' };
        var rowstrs = text.Split(rsplit, StringSplitOptions.RemoveEmptyEntries);
        var vals = new uint[rowstrs.Length];
        for (var i = 0; i < rowstrs.Length; i++)
        {
            var rowstr = rowstrs[i].Trim();
    ...
    private void YnvAreaIDUpDownChange()
    {
        ...
        var x = (int)YnvAreaIDXUpDown.Value;
        var y = (int)YnvAreaIDYUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
            ...
            if (Ynv.AreaID != areaid)
            {
                ...
                YnvAreaIDInfoLabel.Text = "ID: " + areaid;
    ...
    private void YnvAABBSizeTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (Ynv?.Nav == null) return;
        var v = FloatUtil.ParseVector3String(YnvAABBSizeTextBox.Text);
        lock (ProjectForm.ProjectSyncRoot)
        {
