    ...
    private void GenerateProjectManifest()
    {
        ...
        var getYtypName = new Func<YtypFile, string>(ytyp =>
        {
            ...
            if (ytyp != null)
            {
                ...
                if (ytypname.EndsWith(".ytyp")) ytypname = ytypname.Substring(0, ytypname.Length - 5);
            }
        ...
        if (CurrentProjectFile != null)
        {
            if (CurrentProjectFile.YmapFiles.Count > 0)
            {
                ...
                foreach (var ymap in CurrentProjectFile.YmapFiles)
                {
                    ...
                    if (string.IsNullOrEmpty(ymapname)) ymapname = ymap.Name.ToLowerInvariant();
                    if (ymapname.EndsWith(".ymap")) ymapname = ymapname.Substring(0, ymapname.Length - 5);
                    ...
                    if (ymap.AllEntities != null)
                        foreach (var ent in ymap.AllEntities)
                        {
                            ...
                            if (ytyp != null) mapdeps[ytypname] = ytyp;
                            ...
                            if (ent.IsMlo)
                            {
                                ...
                                if (ent.MloInstance?.Entities != null)
                                {
                                    ...
                                    foreach (var ient in ent.MloInstance.Entities)
                                    {
                                        ...
                                        if (iytyp != null && iytypname != ytypname) typdepdict[iytypname] = iytyp;
                                    }
                                }
                            }
                        }
                    ...
                    foreach (var kvp in mapdeps) sb.AppendLine("        <Item>" + kvp.Key + "</Item>");
                    sb.AppendLine("      </itypDepArray>");
                    sb.AppendLine("    </Item>");
                }
        ...
        if (typdeps.Count > 0)
        {
            ...
            foreach (var kvp1 in typdeps)
            {
                ...
                sb.AppendLine("      <manifestFlags>INTERIOR_DATA</manifestFlags>");
                sb.AppendLine("      <itypDepArray>");
                foreach (var kvp2 in kvp1.Value) sb.AppendLine("        <Item>" + kvp2.Key + "</Item>");
                sb.AppendLine("      </itypDepArray>");
                sb.AppendLine("    </Item>");
            }
