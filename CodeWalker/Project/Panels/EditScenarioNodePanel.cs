...
public partial class EditScenarioNodePanel : ProjectPanel
{
    ...
    private void UpdateTabVisibility()
    {
        ...
        var ok = true;
        var pointTabVis = false;
        var entTabVis = false;
        var entPntTabVis = false;
        var chainTabVis = false;
        var chainNodeTabVis = false;
        var clusterTabVis = false;
        var clusterNodeTabVis = false;
        ...
        if (ScenarioTabControl.TabPages.Contains(seltab)) ScenarioTabControl.SelectedTab = seltab;
    }
    ...
    private void LoadScenarioPointTabPage()
    {
        ...
        else
        {
            ...
            ScenarioPointInteriorHashLabel.Text = "Hash: " + p.InteriorName.Hash;
            ScenarioPointGroupTextBox.Text = p.GroupName.ToString();
            ScenarioPointGroupHashLabel.Text = "Hash: " + p.GroupName.Hash;
            ScenarioPointImapTextBox.Text = p.IMapName.ToString();
            ScenarioPointImapHashLabel.Text = "Hash: " + p.IMapName.Hash;
            ...
            for (var i = 0; i < ScenarioPointFlagsCheckedListBox.Items.Count; i++)
            {
                var c = (iflags & (1 << i)) > 0;
    ...
    private void LoadScenarioEntityTabPage()
    {
        ...
        else
        {
            ...
            ScenarioEntityTypeHashLabel.Text = "Hash: " + e.TypeName.Hash;
            ...
            ScenarioEntityInfoLabel.Text = pc + " override point" + (pc != 1 ? "s" : "");
            ScenarioEntityPointsListBox.Items.Clear();
            ScenarioEntityAddPointButton.Enabled = true;

            if (e.ScenarioPoints != null)
            {
                foreach (var point in e.ScenarioPoints) ScenarioEntityPointsListBox.Items.Add(point);
    ...
    private void LoadScenarioEntityPointTabPage()
    {
        ...
        else
        {
            ...
            ScenarioEntityPointNameHashLabel.Text = "Hash: " + p.NameHash.Hash;
            ...
            ScenarioEntityPointSpawnTypeHashLabel.Text = "Hash: " + p.SpawnType.Hash;
            ScenarioEntityPointPedTypeTextBox.Text = p.PedType.ToString();
            ScenarioEntityPointPedTypeHashLabel.Text = "Hash: " + p.PedType.Hash;
            ScenarioEntityPointGroupTextBox.Text = p.Group.ToString();
            ScenarioEntityPointGroupHashLabel.Text = "Hash: " + p.Group.Hash;
            ScenarioEntityPointInteriorTextBox.Text = p.Interior.ToString();
            ScenarioEntityPointInteriorHashLabel.Text = "Hash: " + p.Interior.Hash;
            ScenarioEntityPointRequiredImapTextBox.Text = p.RequiredImap.ToString();
            ScenarioEntityPointRequiredImapHashLabel.Text = "Hash: " + p.RequiredImap.Hash;
            ...
            var iflags = (int)p.Flags;
            ScenarioEntityPointFlagsUpDown.Value = 0;
            for (var i = 0; i < ScenarioEntityPointFlagsCheckedListBox.Items.Count; i++)
            {
                var c = (iflags & (1 << i)) > 0;
                ScenarioEntityPointFlagsCheckedListBox.SetItemCheckState(i,
                    c ? CheckState.Checked : CheckState.Unchecked);
            }
        }
    }
    ...
    private void LoadScenarioChainTabPage()
    {
        ...
        {
            ScenarioChainDeleteButton.Enabled =
            ...
            ScenarioChainEdgesListBox.Items.Clear();
            ScenarioChainEdgeCountLabel.Text = "Edge Count: " + (n.Chain?.EdgeIds?.Length ?? 0);
            ScenarioChainUnk1UpDown.Value = n.Chain?.Unk1 ?? 0;

            if (n.Chain != null && n.Chain.Edges != null)
            {
                foreach (var edge in n.Chain.Edges) ScenarioChainEdgesListBox.Items.Add(edge);
            }
        }
    }
    ...
    private void LoadScenarioChainEdgeTabPage()
    {
        ...
        else
        {
            ...
            if (ProjectForm.WorldForm != null) ProjectForm.WorldForm.SelectObject(e, CurrentScenarioNode);
        }
    }
    ...
    private void LoadScenarioChainNodeTabPage()
    {
        ...
        else
        {
            ...
            ScenarioChainNodePropHashLabel.Text = "Hash: " + n.PropHash.Hash;
    ...
    private void LoadScenarioClusterTabPage()
    {
        ...
        else
        {
            ...
            if (c.Points != null)
            {
                ...
                {
                    foreach (var point in c.Points.MyPoints) ScenarioClusterPointsListBox.Items.Add(point);
                ...
                {
                    foreach (var point in c.Points.LoadSavePoints) ScenarioClusterPointsListBox.Items.Add(point);
    ...
    private void LoadScenarioClusterPointTabPage()
    {
        ...
        else
        {
            ...
            ScenarioClusterPointInteriorHashLabel.Text = "Hash: " + p.InteriorName.Hash;
            ScenarioClusterPointGroupTextBox.Text = p.GroupName.ToString();
            ScenarioClusterPointGroupHashLabel.Text = "Hash: " + p.GroupName.Hash;
            ScenarioClusterPointImapTextBox.Text = p.IMapName.ToString();
            ScenarioClusterPointImapHashLabel.Text = "Hash: " + p.IMapName.Hash;
            ...
            for (var i = 0; i < ScenarioClusterPointFlagsCheckedListBox.Items.Count; i++)
            {
                var c = (iflags & (1 << i)) > 0;
                ScenarioClusterPointFlagsCheckedListBox.SetItemCheckState(i,
                    c ? CheckState.Checked : CheckState.Unchecked);
            }
        }
    }
    ...
    private void AddScenarioChainEdge()
    {
        ...
        if (CurrentScenarioChainEdge != null) edge.Data = CurrentScenarioChainEdge.Data;
    ...
    private void RemoveScenarioChainEdge()
    {
        ...
        if (MessageBox.Show(
                "Are you sure you want to delete this scenario chain edge?\n" + CurrentScenarioChainEdge +
    ...
    private void MoveScenarioChainEdge(bool moveDown)
    {
        ...
        var lasti = (chain.Edges?.Length ?? 0) - 1;

        var edgeid = 0;
        for (var i = 0; i < chain.Edges.Length; i++)
    ...
    private void UpdateScenarioChainEdgeLinkage()
    {
        ...
        var nifrom = CurrentScenarioChainEdge.NodeIndexFrom;
        var nito = CurrentScenarioChainEdge.NodeIndexTo;
        ...
        if (ProjectForm.WorldForm != null) ProjectForm.WorldForm.UpdateScenarioGraphics(CurrentScenario, false);
    }
    ...
    private void ScenarioPointPositionTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        var v = FloatUtil.ParseVector3String(ScenarioPointPositionTextBox.Text);
        var change = false;
    ...
    private void ScenarioPointDirectionTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        var dir = FloatUtil.Parse(ScenarioPointDirectionTextBox.Text);
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioPointTypeComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.MyPoint == null) return;
        var stype = ScenarioPointTypeComboBox.SelectedItem as ScenarioTypeRef;
        lock (ProjectForm.ProjectSyncRoot)
        {
        ...
        if (CurrentScenarioNode.ChainingNode != null) ScenarioChainNodeTypeComboBox.SelectedItem = stype;
    }
    ...
    private void ScenarioPointModelSetComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.MyPoint == null) return;
        var mset = ScenarioPointModelSetComboBox.SelectedItem as AmbientModelSet;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioPointInteriorTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode.MyPoint == null) return;
        uint hash = 0;
        var name = ScenarioPointInteriorTextBox.Text;
        ...
        ScenarioPointInteriorHashLabel.Text = "Hash: " + hash;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioPointGroupTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode.MyPoint == null) return;
        uint hash = 0;
        var name = ScenarioPointGroupTextBox.Text;
        if (!uint.TryParse(name, out hash)) //don't re-hash hashes
        {
            hash = JenkHash.GenHash(name);
            JenkIndex.Ensure(name);
        ...
        ScenarioPointGroupHashLabel.Text = "Hash: " + hash;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioPointImapTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode.MyPoint == null) return;
        uint hash = 0;
        var name = ScenarioPointImapTextBox.Text;
        if (!uint.TryParse(name, out hash)) //don't re-hash hashes
        {
            hash = JenkHash.GenHash(name);
            JenkIndex.Ensure(name);
        ...
        ScenarioPointImapHashLabel.Text = "Hash: " + hash;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioPointTimeStartUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.MyPoint == null) return;
        var t = (byte)ScenarioPointTimeStartUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioPointTimeEndUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.MyPoint == null) return;
        var t = (byte)ScenarioPointTimeEndUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioPointProbabilityUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.MyPoint == null) return;
        var v = (byte)ScenarioPointProbabilityUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioPointSpOnlyFlagUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.MyPoint == null) return;
        var v = (byte)ScenarioPointSpOnlyFlagUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioPointRadiusUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.MyPoint == null) return;
        var v = (byte)ScenarioPointRadiusUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioPointWaitTimeUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.MyPoint == null) return;
        var v = (byte)ScenarioPointWaitTimeUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioPointFlagsValueUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        populatingui = false;
        var f = (CScenarioPointFlags__Flags)iflags;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioPointFlagsCheckedListBox_ItemCheck(object sender, ItemCheckEventArgs e)
    {
        ...
        for (var i = 0; i < ScenarioPointFlagsCheckedListBox.Items.Count; i++)
            if (e.Index == i)
            {
                if (e.NewValue == CheckState.Checked) iflags += (uint)(1 << i);
            }
            else
            {
                if (ScenarioPointFlagsCheckedListBox.GetItemChecked(i)) iflags += (uint)(1 << i);
            }
    ...
    private void ScenarioEntityPositionTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        var v = FloatUtil.ParseVector3String(ScenarioEntityPositionTextBox.Text);
        var change = false;
        lock (ProjectForm.ProjectSyncRoot)
        {
            if (CurrentScenarioNode.Position != v)
    ...
    private void ScenarioEntityTypeTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode.Entity == null) return;
        uint hash = 0;
        var name = ScenarioEntityTypeTextBox.Text;
        if (!uint.TryParse(name, out hash)) //don't re-hash hashes
        {
            hash = JenkHash.GenHash(name);
            JenkIndex.Ensure(name);
        ...
        ScenarioEntityTypeHashLabel.Text = "Hash: " + hash;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityMayNotAlwaysExistCheckBox_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.Entity == null) return;
        var v = ScenarioEntityMayNotAlwaysExistCheckBox.Checked;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntitySpecificallyPreventArtPointsCheckBox_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.Entity == null) return;
        var v = ScenarioEntitySpecificallyPreventArtPointsCheckBox.Checked;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointPositionTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        var v = FloatUtil.ParseVector3String(ScenarioEntityPointPositionTextBox.Text);
    ...
    private void ScenarioEntityPointRotationQuatBox_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.EntityPoint == null) return;
        var q = ScenarioEntityPointRotationQuatBox.Value;
        var v = q.ToVector4();
        var change = false;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointNameTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode.EntityPoint == null) return;
        uint hash = 0;
        var name = ScenarioEntityPointNameTextBox.Text;
        if (!uint.TryParse(name, out hash)) //don't re-hash hashes
        {
            hash = JenkHash.GenHash(name);
            JenkIndex.Ensure(name);
        ...
        ScenarioEntityPointNameHashLabel.Text = "Hash: " + hash;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointSpawnTypeTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode.EntityPoint == null) return;
        uint hash = 0;
        var name = ScenarioEntityPointSpawnTypeTextBox.Text;
        if (!uint.TryParse(name, out hash)) //don't re-hash hashes
        {
            hash = JenkHash.GenHash(name);
            JenkIndex.Ensure(name);
        ...
        ScenarioEntityPointSpawnTypeHashLabel.Text = "Hash: " + hash;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointPedTypeTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode.EntityPoint == null) return;
        uint hash = 0;
        var name = ScenarioEntityPointPedTypeTextBox.Text;
        if (!uint.TryParse(name, out hash)) //don't re-hash hashes
        {
            hash = JenkHash.GenHash(name);
            JenkIndex.Ensure(name);
        ...
        ScenarioEntityPointPedTypeHashLabel.Text = "Hash: " + hash;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointGroupTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode.EntityPoint == null) return;
        uint hash = 0;
        var name = ScenarioEntityPointGroupTextBox.Text;
        if (!uint.TryParse(name, out hash)) //don't re-hash hashes
        {
            hash = JenkHash.GenHash(name);
            JenkIndex.Ensure(name);
        ...
        ScenarioEntityPointGroupHashLabel.Text = "Hash: " + hash;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointInteriorTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode.EntityPoint == null) return;
        uint hash = 0;
        var name = ScenarioEntityPointInteriorTextBox.Text;
        if (!uint.TryParse(name, out hash)) //don't re-hash hashes
        {
            hash = JenkHash.GenHash(name);
            JenkIndex.Ensure(name);
        ...
        ScenarioEntityPointInteriorHashLabel.Text = "Hash: " + hash;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointRequiredImapTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode.EntityPoint == null) return;
        uint hash = 0;
        var name = ScenarioEntityPointRequiredImapTextBox.Text;
        if (!uint.TryParse(name, out hash)) //don't re-hash hashes
        {
            hash = JenkHash.GenHash(name);
            JenkIndex.Ensure(name);
        ...
        ScenarioEntityPointRequiredImapHashLabel.Text = "Hash: " + hash;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointProbabilityTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.EntityPoint == null) return;
        var v = FloatUtil.Parse(ScenarioEntityPointProbabilityTextBox.Text);
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointTimeTillPedLeavesTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.EntityPoint == null) return;
        var v = FloatUtil.Parse(ScenarioEntityPointTimeTillPedLeavesTextBox.Text);
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointRadiusTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.EntityPoint == null) return;
        var v = FloatUtil.Parse(ScenarioEntityPointRadiusTextBox.Text);
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointStartUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.EntityPoint == null) return;
        var v = (byte)ScenarioEntityPointStartUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointEndUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.EntityPoint == null) return;
        var v = (byte)ScenarioEntityPointEndUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointExtendedRangeCheckBox_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.EntityPoint == null) return;
        var v = ScenarioEntityPointExtendedRangeCheckBox.Checked;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointShortRangeCheckBox_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.EntityPoint == null) return;
        var v = ScenarioEntityPointShortRangeCheckBox.Checked;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointHighPriCheckBox_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.EntityPoint == null) return;
        var v = ScenarioEntityPointHighPriCheckBox.Checked;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioEntityPointFlagsCheckedListBox_ItemCheck(object sender, ItemCheckEventArgs e)
    {
        ...
        for (var i = 0; i < ScenarioEntityPointFlagsCheckedListBox.Items.Count; i++)
            ...
            {
                if (ScenarioEntityPointFlagsCheckedListBox.GetItemChecked(i)) iflags += (uint)(1 << i);
            }
    ...
    private void ScenarioChainNodePositionTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentScenarioNode == null) return;
        var v = FloatUtil.ParseVector3String(ScenarioChainNodePositionTextBox.Text);
        var change = false;
        lock (ProjectForm.ProjectSyncRoot)
        {
            if (CurrentScenarioNode.Position != v)
    ...
    private void ScenarioChainNodePropTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        uint hash = 0;
        var name = ScenarioChainNodePropTextBox.Text;
        if (!uint.TryParse(name, out hash)) //don't re-hash hashes
        {
            hash = JenkHash.GenHash(name);
            JenkIndex.Ensure(name);
        ...
        ScenarioChainNodePropHashLabel.Text = "Hash: " + hash;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioChainNodeTypeComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.ChainingNode == null) return;
        var stype = ScenarioChainNodeTypeComboBox.SelectedItem as ScenarioTypeRef;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioChainNodeHasIncomingCheckBox_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.ChainingNode == null) return;
        var v = ScenarioChainNodeHasIncomingCheckBox.Checked;
        var change = false;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioChainNodeHasOutgoingCheckBox_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.ChainingNode == null) return;
        var v = ScenarioChainNodeHasOutgoingCheckBox.Checked;
        var change = false;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioChainUnk1UpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        var v = (byte)ScenarioChainUnk1UpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioChainEdgeNodeIndexFromUpDown_ValueChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentScenarioChainEdge == null) return;
        var nodeid = (ushort)ScenarioChainEdgeNodeIndexFromUpDown.Value;
        var change = false;
        lock (ProjectForm.ProjectSyncRoot)
        {
        ...
        if (change)
        {
            ...
            ScenarioChainEdgesListBox.Items[ScenarioChainEdgesListBox.SelectedIndex] =
    ...
    private void ScenarioChainEdgeNodeIndexToUpDown_ValueChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentScenarioChainEdge == null) return;
        var nodeid = (ushort)ScenarioChainEdgeNodeIndexToUpDown.Value;
        var change = false;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioChainEdgeActionComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentScenarioChainEdge == null) return;
        var v = (CScenarioChainingEdge__eAction)ScenarioChainEdgeActionComboBox.SelectedItem;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioChainEdgeNavModeComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentScenarioChainEdge == null) return;
        var v = (CScenarioChainingEdge__eNavMode)ScenarioChainEdgeNavModeComboBox.SelectedItem;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioChainEdgeNavSpeedComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentScenarioChainEdge == null) return;
        var v = (CScenarioChainingEdge__eNavSpeed)ScenarioChainEdgeNavSpeedComboBox.SelectedItem;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterCenterTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentScenarioNode == null) return;
        var v = FloatUtil.ParseVector3String(ScenarioClusterCenterTextBox.Text);
        var change = false;
        lock (ProjectForm.ProjectSyncRoot)
        {
            if (CurrentScenarioNode.Position != v)
    ...
    private void ScenarioClusterRadiusTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentScenarioNode == null) return;
        var r = FloatUtil.Parse(ScenarioClusterRadiusTextBox.Text);
        var change = false;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterUnk1TextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        var v = FloatUtil.Parse(ScenarioClusterUnk1TextBox.Text);
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterUnk2CheckBox_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.Cluster == null) return;
        var v = ScenarioClusterUnk2CheckBox.Checked;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterPointPositionTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentScenarioNode == null) return;
        var v = FloatUtil.ParseVector3String(ScenarioClusterPointPositionTextBox.Text);
        var change = false;
        lock (ProjectForm.ProjectSyncRoot)
        {
            if (CurrentScenarioNode.Position != v)
    ...
    private void ScenarioClusterPointDirectionTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        var dir = FloatUtil.Parse(ScenarioClusterPointDirectionTextBox.Text);
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterPointTypeComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.ClusterMyPoint == null) return;
        var stype = ScenarioClusterPointTypeComboBox.SelectedItem as ScenarioTypeRef;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterPointModelSetComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.ClusterMyPoint == null) return;
        var mset = ScenarioClusterPointModelSetComboBox.SelectedItem as AmbientModelSet;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterPointInteriorTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode.ClusterMyPoint == null) return;
        uint hash = 0;
        var name = ScenarioClusterPointInteriorTextBox.Text;
        if (!uint.TryParse(name, out hash)) //don't re-hash hashes
        {
            hash = JenkHash.GenHash(name);
            JenkIndex.Ensure(name);
        ...
        ScenarioClusterPointInteriorHashLabel.Text = "Hash: " + hash;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterPointGroupTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode.ClusterMyPoint == null) return;
        uint hash = 0;
        var name = ScenarioClusterPointGroupTextBox.Text;
        if (!uint.TryParse(name, out hash)) //don't re-hash hashes
        {
            hash = JenkHash.GenHash(name);
            JenkIndex.Ensure(name);
        ...
        ScenarioClusterPointGroupHashLabel.Text = "Hash: " + hash;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterPointImapTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode.ClusterMyPoint == null) return;
        uint hash = 0;
        var name = ScenarioClusterPointImapTextBox.Text;
        if (!uint.TryParse(name, out hash)) //don't re-hash hashes
        {
            hash = JenkHash.GenHash(name);
            JenkIndex.Ensure(name);
        ...
        ScenarioClusterPointImapHashLabel.Text = "Hash: " + hash;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterPointTimeStartUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.ClusterMyPoint == null) return;
        var t = (byte)ScenarioClusterPointTimeStartUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterPointTimeEndUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.ClusterMyPoint == null) return;
        var t = (byte)ScenarioClusterPointTimeEndUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterPointProbabilityUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.ClusterMyPoint == null) return;
        var v = (byte)ScenarioClusterPointProbabilityUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterPointSpOnlyFlagUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.ClusterMyPoint == null) return;
        var v = (byte)ScenarioClusterPointSpOnlyFlagUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterPointRadiusUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.ClusterMyPoint == null) return;
        var v = (byte)ScenarioClusterPointRadiusUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterPointWaitTimeUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentScenarioNode == null) return;
        if (CurrentScenarioNode.ClusterMyPoint == null) return;
        var v = (byte)ScenarioClusterPointWaitTimeUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void ScenarioClusterPointFlagsCheckedListBox_ItemCheck(object sender, ItemCheckEventArgs e)
    {
        ...
        for (var i = 0; i < ScenarioClusterPointFlagsCheckedListBox.Items.Count; i++)
            ...
            {
                if (ScenarioClusterPointFlagsCheckedListBox.GetItemChecked(i)) iflags += (uint)(1 << i);
            }
