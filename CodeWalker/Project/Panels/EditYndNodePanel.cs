...
public partial class EditYndNodePanel : ProjectPanel
{
    ...
    {
        var sn = CurrentPathNode.StreetName.Hash == 0
            ? "Path node"
            : CurrentPathNode?.StreetName.ToString() ?? string.Empty;
        Text = sn + " " + CurrentPathNode.NodeID;
    }
    ...
    private void LoadPathNodeTabPage()
    {
        ...
        else
        {
            ...
            PathNodeStreetNameLabel.Text = "Name: " + (n.StreetName.Hash == 0 ? "[None]" :
                string.IsNullOrEmpty(streetname) ? "[Not found]" : streetname);

            UpdatePathNodeFlagsUI(true, true);

            PathNodeLinkCountLabel.Text = "Link Count: " + CurrentPathNode.LinkCount;
            PathNodeLinksListBox.Items.Clear();
            ...
            if (ProjectForm.WorldForm != null) ProjectForm.WorldForm.SelectObject(CurrentPathNode);
            ...
            if (PathNodeSpecialTypeComboBox.Items.Count == 0)
                PathNodeSpecialTypeComboBox.Items.AddRange(Enum.GetValues(typeof(YndNodeSpecialType)).Cast<object>()
                    .ToArray());
    ...
    private void LoadPathNodeLinkPage()
    {
        ...
        else
        {
            ...
            if (ProjectForm.WorldForm != null) ProjectForm.WorldForm.SelectObject(CurrentPathLink);
        }
    }
    ...
    private void SetPathNodeFlagsFromUpDowns()
    {
        ...
        var flags0 = (uint)PathNodeFlags0UpDown.Value;
        var flags1 = (uint)PathNodeFlags1UpDown.Value;
        var flags2 = (uint)PathNodeFlags2UpDown.Value;
        var flags3 = (uint)PathNodeFlags3UpDown.Value;
        var flags4 = (uint)PathNodeFlags4UpDown.Value;
        var flags5 = (uint)PathNodeFlags5UpDown.Value;
    ...
    private void SetPathNodeLinkFlagsFromCheckBoxes()
    {
        ...
        lock (ProjectForm.ProjectSyncRoot)
        {
            ...
            var forwardLanes = (int)PathNodeLinkFwdLanesUpDown.Value;
            ...
            var backwardLanes = (int)PathNodeLinkBackLanesUpDown.Value;
    ...
    private void SetPathNodeLinkFlagsFromUpDowns()
    {
        ...
        var flags0 = (uint)PathNodeLinkFlags0UpDown.Value;
        var flags1 = (uint)PathNodeLinkFlags1UpDown.Value;
        var flags2 = (uint)PathNodeLinkFlags2UpDown.Value;
    ...
    private void AddPathLink()
    {
        ...
        if (ProjectForm.WorldForm != null) ProjectForm.WorldForm.UpdatePathNodeGraphics(CurrentPathNode, false);
    }
    ...
    private void RemovePathLink()
    {
        ...
        foreach (var partner in partners) partner.Node1.RemoveLink(partner);
    ...
    private void UpdatePathNodeLinkage()
    {
        ...
        var areaid = CurrentPathLink._RawData.AreaID;
        var nodeid = CurrentPathLink._RawData.NodeID;
        ...
        else
        {
            ...
            if (ProjectForm.WorldForm != null) linknode = ProjectForm.WorldForm.GetPathNodeFromSpace(areaid, nodeid);
        }

        if (linknode == null)
            PathNodeLinkageStatusLabel.Text = "Unable to find node " + areaid + ":" + nodeid + ".";
        else
            PathNodeLinkageStatusLabel.Text = "";
        ...
        if (l2 != null && partner != null) l2.CopyFlags(partner);

        ////need to rebuild the link verts.. updating the graphics should do it...
        if (ProjectForm.WorldForm != null) ProjectForm.WorldForm.UpdatePathYndGraphics(CurrentYndFile, false);
    }
    ...
    private void PathNodeAreaIDUpDown_ValueChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentPathNode == null) return;
        var areaid = (ushort)PathNodeAreaIDUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void PathNodeNodeIDUpDown_ValueChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentPathNode == null) return;
        var nodeid = (ushort)PathNodeNodeIDUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void PathNodePositionTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentPathNode == null) return;
        var v = FloatUtil.ParseVector3String(PathNodePositionTextBox.Text);
        var change = false;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    {
        if (CurrentPathNode?.Ynd != null) ProjectForm.AddYndToProject(CurrentPathNode.Ynd);
    }
    ...
    private void PathNodeStreetHashTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        PathNodeStreetNameLabel.Text =
            "Name: " + (hash == 0 ? "[None]" : string.IsNullOrEmpty(streetname) ? "[Not found]" : streetname);
    ...
    private void PathNodeLinkAreaIDUpDown_ValueChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentPathLink == null) return;
        var areaid = (ushort)PathNodeLinkAreaIDUpDown.Value;
        var change = false;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void PathNodeLinkNodeIDUpDown_ValueChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentPathLink == null) return;
        var nodeid = (ushort)PathNodeLinkNodeIDUpDown.Value;
        var change = false;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void PathNodeLinkLengthUpDown_ValueChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (CurrentPathLink == null) return;
        var length = (byte)PathNodeLinkLengthUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
        ...
        lock (ProjectForm.ProjectSyncRoot)
        {
            if (CurrentPathNode.HasJunction != PathNodeJunctionEnableCheckBox.Checked)
            {
                ...
                if (CurrentPathNode.HasJunction && CurrentPathNode.Junction == null)
                {
                    ...
                    j.Heightmap = new YndJunctionHeightmap(
                        Enumerable.Repeat((byte)255, j._RawData.HeightmapDimX * j._RawData.HeightmapDimY).ToArray(), j);
                    j.RefData = new NodeJunctionRef
                        { AreaID = CurrentPathNode.AreaID, NodeID = CurrentPathNode.NodeID };
    ...
    private void PathNodeJunctionMaxZUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        var val = (short)(PathNodeJunctionMaxZUpDown.Value * 32);
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void PathNodeJunctionMinZUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentPathNode == null) return;
        if (CurrentPathNode.Junction == null) return;
        var val = (short)(PathNodeJunctionMinZUpDown.Value * 32);
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void PathNodeJunctionPosXUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentPathNode == null) return;
        if (CurrentPathNode.Junction == null) return;
        var val = (short)(PathNodeJunctionPosXUpDown.Value * 32);
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void PathNodeJunctionPosYUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentPathNode == null) return;
        if (CurrentPathNode.Junction == null) return;
        var val = (short)(PathNodeJunctionPosYUpDown.Value * 32);
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void PathNodeJunctionHeightmapDimXUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentPathNode == null) return;
        if (CurrentPathNode.Junction == null) return;
        var val = (byte)PathNodeJunctionHeightmapDimXUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void PathNodeJunctionHeightmapDimYUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (CurrentPathNode == null) return;
        if (CurrentPathNode.Junction == null) return;
        var val = (byte)PathNodeJunctionHeightmapDimYUpDown.Value;
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void PathNodeSpecialTypeComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        lock (ProjectForm.ProjectSyncRoot)
        {
            ...
            if (CurrentPathNode.Special != special)
            {
                ...
                var specialIsPedNode = YndNode.IsSpecialTypeAPedNode(special);
