            ...
            foreach (var ymap in projectYmaps)
            {
                ...
                foreach (var ent in ymap.AllEntities)
                {
                    ...
                    if (dwbl != null)
                    {
                        ...
                        if (elights != null)
                            for (var li = 0; li < elights.Length; li++)
                            {
                                ...
                                uint g = la.ColorG;
                                uint b = la.ColorB;
                                uint i = (byte)Math.Max(Math.Min(Math.Round(la.Intensity * 5.3125f), 255),
                                    0); //5.1=255/48
                                var c = (i << 24) + (r << 16) + (g << 8) + b;
                                var h = elight.Hash;
                                ...
                                var isStreetLight =
                                    ((flags >> 10) & 1u) ==
                                    1; // (name != null) && (name.Contains("street") || name.Contains("traffic"));
                                ...
                                //2 = spot
                                //4 = capsule
                                var type = (uint)la.Type;
                                var unk = isStreetLight ? 1u : 0; //2 bits - isStreetLight low bit, unk high bit
                                var t = la.TimeFlags | (type << 26) | (unk << 24);

                                var inner = (byte)Math.Round(la.ConeInnerAngle * 1.4117647f);
                                var outer = (byte)Math.Round(la.ConeOuterAngle * 1.4117647f);
                                if (type == 4) outer = (byte)Math.Round(la.Extent.X * 1.82f);
