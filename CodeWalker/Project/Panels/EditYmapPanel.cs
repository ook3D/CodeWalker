...
public partial class EditYmapPanel : ProjectPanel
{
    ...
    {
        var fn = Ymap.RpfFileEntry?.Name ?? Ymap.Name;
    ...
    public void UpdateYmapUI()
    {
        ...
        else
        {
            ...
            if (md.name.Hash == 0)
            {
                var name = Path.GetFileNameWithoutExtension(Ymap.Name);
            ...
            YmapNameHashLabel.Text = "Hash: " + md.name.Hash;
            YmapParentTextBox.Text = md.parent.ToString();
            YmapParentHashLabel.Text = "Hash: " + md.parent.Hash;
    ...
    private void UpdateYmapPhysicsDictionariesUI()
    {
        ...
        else
        {
            ...
            foreach (var hash in Ymap.physicsDictionaries) sb.AppendLine(hash.ToString());
    ...
    private void CalcYmapFlags()
    {
        ...
        lock (ProjectForm.ProjectSyncRoot)
        {
            if (Ymap.CalcFlags()) SetYmapHasChanged(true);
        }
    ...
    private void CalcYmapExtents()
    {
        ...
        lock (ProjectForm.ProjectSyncRoot)
        {
            if (Ymap.CalcExtents()) SetYmapHasChanged(true);
        }
    ...
    private void YmapNameTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        YmapNameHashLabel.Text = "Hash: " + hash;

        if (Ymap != null)
            lock (ProjectForm.ProjectSyncRoot)
            {
                Ymap.SetName(name);
                if (!File.Exists(Ymap.FilePath)) Ymap.FilePath = name;
                SetYmapHasChanged(true);
                UpdateFormTitle();
            }
    }
    ...
    private void YmapParentTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        uint hash = 0;
        var name = YmapParentTextBox.Text;
        if (!uint.TryParse(name, out hash)) //don't re-hash hashes
        {
            hash = JenkHash.GenHash(name);
            JenkIndex.Ensure(name);
        ...
        YmapParentHashLabel.Text = "Hash: " + hash;

        if (hash != 0)
        {
            var entry = ProjectForm.FindParentYmapEntry(hash);
            if (entry == null) YmapParentHashLabel.Text += " (not found!)";
        }
    ...
    private void YmapEntitiesExtentsMinTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (Ymap == null) return;
        var v = FloatUtil.ParseVector3String(YmapEntitiesExtentsMinTextBox.Text);
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void YmapEntitiesExtentsMaxTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (Ymap == null) return;
        var v = FloatUtil.ParseVector3String(YmapEntitiesExtentsMaxTextBox.Text);
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void YmapStreamingExtentsMinTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (Ymap == null) return;
        var v = FloatUtil.ParseVector3String(YmapStreamingExtentsMinTextBox.Text);
        lock (ProjectForm.ProjectSyncRoot)
        {
    ...
    private void YmapStreamingExtentsMaxTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (Ymap == null) return;
        var v = FloatUtil.ParseVector3String(YmapStreamingExtentsMaxTextBox.Text);
        lock (ProjectForm.ProjectSyncRoot)
        {
