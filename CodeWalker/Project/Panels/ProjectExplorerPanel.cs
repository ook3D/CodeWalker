...
{
    private bool inDoubleClick; //used in disabling double-click to expand tree nodes
    ...
    public void LoadProjectTree(ProjectFile projectFile)
    {
        ...
        if (CurrentProjectFile.YmapFiles.Count > 0)
        {
            ...
            foreach (var ymapfile in CurrentProjectFile.YmapFiles)
            {
                ...
                var name = ymapfile.Name;
                if (ymapfile.RpfFileEntry != null) name = ymapfile.RpfFileEntry.Name;
        ...
        if (CurrentProjectFile.YtypFiles.Count > 0)
        {
            ...
            foreach (var ytypfile in CurrentProjectFile.YtypFiles)
            {
                ...
                var name = ytypfile.Name;
                if (ytypfile.RpfFileEntry != null) name = ytypfile.RpfFileEntry.Name;
        ...
        if (CurrentProjectFile.YbnFiles.Count > 0)
        {
            ...
            foreach (var ybnfile in CurrentProjectFile.YbnFiles)
            {
                ...
                var name = ybnfile.Name;
                if (ybnfile.RpfFileEntry != null) name = ybnfile.RpfFileEntry.Name;
        ...
        if (CurrentProjectFile.YndFiles.Count > 0)
        {
            ...
            foreach (var yndfile in CurrentProjectFile.YndFiles)
            {
                ...
                var name = yndfile.Name;
                if (yndfile.RpfFileEntry != null) name = yndfile.RpfFileEntry.Name;
        ...
        if (CurrentProjectFile.YnvFiles.Count > 0)
        {
            ...
            foreach (var ynvfile in CurrentProjectFile.YnvFiles)
            {
                ...
                var name = ynvfile.Name;
                if (ynvfile.RpfFileEntry != null) name = ynvfile.RpfFileEntry.Name;
        ...
        if (CurrentProjectFile.TrainsFiles.Count > 0)
        {
            ...
            foreach (var trainfile in CurrentProjectFile.TrainsFiles)
            {
                ...
                var name = trainfile.Name;
                if (trainfile.RpfFileEntry != null) name = trainfile.RpfFileEntry.Name;
        ...
        if (CurrentProjectFile.ScenarioFiles.Count > 0)
        {
            ...
            foreach (var scenariofile in CurrentProjectFile.ScenarioFiles)
            {
                ...
                var name = scenariofile.Name;
                if (scenariofile.RpfFileEntry != null) name = scenariofile.RpfFileEntry.Name;
        ...
        if (CurrentProjectFile.AudioRelFiles.Count > 0)
        {
            ...
            foreach (var audiorelfile in CurrentProjectFile.AudioRelFiles)
            {
                ...
                var name = audiorelfile.Name;
                if (audiorelfile.RpfFileEntry != null) name = audiorelfile.RpfFileEntry.Name;
        ...
        if (CurrentProjectFile.YdrFiles.Count > 0)
        {
            ...
            foreach (var ydrfile in CurrentProjectFile.YdrFiles)
            {
                ...
                var name = ydrfile.Name;
                if (ydrfile.RpfFileEntry != null) name = ydrfile.RpfFileEntry.Name;
        ...
        if (CurrentProjectFile.YddFiles.Count > 0)
        {
            ...
            foreach (var yddfile in CurrentProjectFile.YddFiles)
            {
                ...
                var name = yddfile.Name;
                if (yddfile.RpfFileEntry != null) name = yddfile.RpfFileEntry.Name;
        ...
        if (CurrentProjectFile.YftFiles.Count > 0)
        {
            ...
            foreach (var yftfile in CurrentProjectFile.YftFiles)
            {
                ...
                var name = yftfile.Name;
                if (yftfile.RpfFileEntry != null) name = yftfile.RpfFileEntry.Name;
        ...
        if (CurrentProjectFile.YtdFiles.Count > 0)
        {
            ...
            foreach (var ytdfile in CurrentProjectFile.YtdFiles)
            {
                ...
                var name = ytdfile.Name;
                if (ytdfile.RpfFileEntry != null) name = ytdfile.RpfFileEntry.Name;
    ...
    private void LoadYmapTreeNodes(YmapFile ymap, TreeNode node)
    {
        ...
        {
            var entsnode = node.Nodes.Add("Entities (" + ymap.AllEntities.Length + ")");
        ...
        {
            var cargensnode = node.Nodes.Add("Car Generators (" + ymap.CarGenerators.Length + ")");
            cargensnode.Name = "CarGens";
            cargensnode.Tag = ymap;
            var cargens = ymap.CarGenerators;
            for (var i = 0; i < cargens.Length; i++)
            {
        ...
        {
            var lodlightsnode = node.Nodes.Add("LOD Lights (" + ymap.LODLights.LodLights.Length + ")");
            lodlightsnode.Name = "LodLights";
            lodlightsnode.Tag = ymap;
            var lodlights = ymap.LODLights.LodLights;
            for (var i = 0; i < lodlights.Length; i++)
            {
        ...
        {
            var boxocclsnode = node.Nodes.Add("Box Occluders (" + ymap.BoxOccluders.Length + ")");
        ...
        {
            var occlmodsnode = node.Nodes.Add("Occlude Models (" + ymap.OccludeModels.Length + ")");
        ...
        {
            var grassbatchesnodes = node.Nodes.Add("Grass Batches (" + ymap.GrassInstanceBatches.Length + ")");
            grassbatchesnodes.Name = "GrassBatches";
            grassbatchesnodes.Tag = ymap;
            var grassbatches = ymap.GrassInstanceBatches;
            for (var i = 0; i < grassbatches.Length; i++)
            {
    ...
    private void LoadYtypTreeNodes(YtypFile ytyp, TreeNode node)
    {
        ...
        {
            var archetypesnode = node.Nodes.Add("Archetypes (" + ytyp.AllArchetypes.Length + ")");
            ...
            for (var i = 0; i < archetypes.Length; i++)
            {
                ...
                if (yarch is MloArchetype mlo)
                {
                    ...
                    {
                        var roomsnode = tarch.Nodes.Add("Rooms (" + rooms.Length + ")");
                        roomsnode.Name = "Rooms";
                        for (var j = 0; j < rooms.Length; j++)
                        {
                            var room = rooms[j];
                            var roomnode = roomsnode.Nodes.Add(room.Index + ": " + room.RoomName);
                            roomnode.Tag = room;
                    ...
                    {
                        var portalsnode = tarch.Nodes.Add("Portals (" + portals.Length + ")");
                        ...
                        for (var j = 0; j < portals.Length; j++)
                        {
                            ...
                            if (portalentities != null && entities != null)
                                for (var k = 0; k < portalentities.Length; k++)
                                {
                    ...
                    {
                        var setsnode = tarch.Nodes.Add("Entity Sets (" + entsets.Length + ")");
                        setsnode.Name = "EntitySets";
                        for (var j = 0; j < entsets.Length; j++)
                        {
                            ...
                            if (setents != null && setlocs != null)
                                for (var k = 0; k < setents.Length; k++)
                                {
    ...
    private void LoadYbnTreeNodes(YbnFile ybn, TreeNode node)
    {
        ...
        if (ybn.Bounds != null) LoadYbnBoundsTreeNode(ybn.Bounds, node);
    }
        ...
        if (b is BoundComposite bc)
        {
            ...
            if (children != null)
                for (var i = 0; i < children.Length; i++)
                {
                    ...
                    if (child != null) LoadYbnBoundsTreeNode(child, boundsnode);
                }
        }
    ...
    private void LoadTrainTrackTreeNodes(TrainTrack track, TreeNode node)
    {
        ...
        {
            var nodesnode = node.Nodes.Add("Nodes (" + track.Nodes.Count + ")");
            nodesnode.Name = "Nodes";
            nodesnode.Tag = track;
    ...
    private void LoadScenarioTreeNodes(YmtFile ymt, TreeNode node)
    {
        ...
        var pointsnode = node.Nodes.Add("Points (" + nodes.Count + ")");
    ...
    private void LoadAudioRelTreeNodes(RelFile rel, TreeNode node)
    {
        ...
        {
            if (reldata is Dat151AmbientZone) zones.Add(reldata as Dat151AmbientZone);
            if (reldata is Dat151AmbientRule) rules.Add(reldata as Dat151AmbientRule);
            if (reldata is Dat151StaticEmitter) emitters.Add(reldata as Dat151StaticEmitter);
            if (reldata is Dat151AmbientZoneList) zonelists.Add(reldata as Dat151AmbientZoneList);
            if (reldata is Dat151StaticEmitterList) emitterlists.Add(reldata as Dat151StaticEmitterList);
            if (reldata is Dat151InteriorSettings) interiors.Add(reldata as Dat151InteriorSettings);
            if (reldata is Dat151InteriorRoom) interiorrooms.Add(reldata as Dat151InteriorRoom);
        }


        if (zones.Count > 0)
        {
            var n = node.Nodes.Add("Ambient Zones (" + zones.Count + ")");
            n.Name = "AmbientZones";
            n.Tag = rel;
        ...
        if (rules.Count > 0)
        {
            var n = node.Nodes.Add("Ambient Rules (" + rules.Count + ")");
            n.Name = "AmbientRules";
            n.Tag = rel;
        ...
        if (emitters.Count > 0)
        {
            var n = node.Nodes.Add("Static Emitters (" + emitters.Count + ")");
        ...
        {
            var zonelistsnode = node.Nodes.Add("Ambient Zone Lists (" + zonelists.Count + ")");
            zonelistsnode.Name = "AmbientZoneLists";
            zonelistsnode.Tag = rel;
            for (var i = 0; i < zonelists.Count; i++)
            {
        ...
        {
            var emitterlistsnode = node.Nodes.Add("Static Emitter Lists (" + emitterlists.Count + ")");
            emitterlistsnode.Name = "StaticEmitterLists";
            emitterlistsnode.Tag = rel;
            for (var i = 0; i < emitterlists.Count; i++)
            {
        ...
        {
            var n = node.Nodes.Add("Interiors (" + interiors.Count + ")");
            n.Name = "Interiors";
            n.Tag = rel;
        ...
        {
            var n = node.Nodes.Add("Interior Rooms (" + interiorrooms.Count + ")");
            n.Name = "InteriorRooms";
            n.Tag = rel;
            for (var i = 0; i < interiorrooms.Count; i++)
            {
    ...
    public void SetYmapHasChanged(YmapFile ymap, bool changed)
    {
        if (ProjectTreeView.Nodes.Count > 0)
        {
            ...
            var changestr = changed ? "*" : "";
            for (var i = 0; i < ymnode.Nodes.Count; i++)
            {
    ...
    public void SetYtypHasChanged(YtypFile ytyp, bool changed)
    {
        if (ProjectTreeView.Nodes.Count > 0)
        {
            ...
            var changestr = changed ? "*" : "";
            for (var i = 0; i < ytnode.Nodes.Count; i++)
            {
    ...
    public void SetYbnHasChanged(YbnFile ybn, bool changed)
    {
        if (ProjectTreeView.Nodes.Count > 0)
        {
            ...
            var changestr = changed ? "*" : "";
            for (var i = 0; i < ynnode.Nodes.Count; i++)
            {
                var ynode = ynnode.Nodes[i];
                if (ynode.Tag == ybn)
                {
                    var name = ybn.Name;
                    if (ybn.RpfFileEntry != null) name = ybn.RpfFileEntry.Name;
                    ynode.Text = changestr + name;
                    break;
                }
            }
    ...
    public void SetYndHasChanged(YndFile ynd, bool changed)
    {
        if (ProjectTreeView.Nodes.Count > 0)
        {
            ...
            for (var i = 0; i < ynnode.Nodes.Count; i++)
            {
                ...
                {
                    var name = ynd.Name;
                    if (ynd.RpfFileEntry != null) name = ynd.RpfFileEntry.Name;
                    ynode.Text = changestr + name;
                    break;
                }
            }
    ...
    public void SetYnvHasChanged(YnvFile ynv, bool changed)
    {
        if (ProjectTreeView.Nodes.Count > 0)
        {
            ...
            for (var i = 0; i < ynnode.Nodes.Count; i++)
            {
                ...
                {
                    var name = ynv.Name;
                    if (ynv.RpfFileEntry != null) name = ynv.RpfFileEntry.Name;
                    ynode.Text = changestr + name;
                    break;
                }
            }
    ...
    public void SetTrainTrackHasChanged(TrainTrack track, bool changed)
    {
        if (ProjectTreeView.Nodes.Count > 0)
        {
            ...
            var changestr = changed ? "*" : "";
            for (var i = 0; i < trnode.Nodes.Count; i++)
            {
                var tnode = trnode.Nodes[i];
                if (tnode.Tag == track)
                {
                    var name = track.Name;
                    if (track.RpfFileEntry != null) name = track.RpfFileEntry.Name;
    ...
    public void SetScenarioHasChanged(YmtFile scenario, bool changed)
    {
        if (ProjectTreeView.Nodes.Count > 0)
        {
            ...
            var changestr = changed ? "*" : "";
            for (var i = 0; i < scnode.Nodes.Count; i++)
            {
                var snode = scnode.Nodes[i];
                if (snode.Tag == scenario)
                {
                    var name = scenario.Name;
                    if (scenario.RpfFileEntry != null) name = scenario.RpfFileEntry.Name;
    ...
    public void SetAudioRelHasChanged(RelFile rel, bool changed)
    {
        if (ProjectTreeView.Nodes.Count > 0)
        {
            ...
            var changestr = changed ? "*" : "";
            for (var i = 0; i < acnode.Nodes.Count; i++)
            {
                var anode = acnode.Nodes[i];
                if (anode.Tag == rel)
                {
                    var name = rel.Name;
                    if (rel.RpfFileEntry != null) name = rel.RpfFileEntry.Name;
    ...
    public TreeNode FindYmapTreeNode(YmapFile ymap)
    {
        ...
        for (var i = 0; i < ymapsnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindEntityTreeNode(YmapEntityDef ent)
    {
        ...
        for (var i = 0; i < entsnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindCarGenTreeNode(YmapCarGen cargen)
    {
        ...
        for (var i = 0; i < cargensnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindLodLightTreeNode(YmapLODLight lodlight)
    {
        ...
        for (var i = 0; i < lodlightsnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindBoxOccluderTreeNode(YmapBoxOccluder box)
    {
        ...
        for (var i = 0; i < boxesnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindOccludeModelTreeNode(YmapOccludeModel model)
    {
        ...
        for (var i = 0; i < modelsnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindGrassTreeNode(YmapGrassInstanceBatch batch)
    {
        ...
        for (var i = 0; i < batchnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindYtypTreeNode(YtypFile ytyp)
    {
        ...
        for (var i = 0; i < ytypsnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindArchetypeTreeNode(Archetype archetype)
    {
        ...
        for (var i = 0; i < archetypenode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindMloRoomTreeNode(MCMloRoomDef room)
    {
        ...
        if (mloarchetypenode != null)
        {
            ...
            for (var j = 0; j < roomsnode.Nodes.Count; j++)
            {
    ...
    public TreeNode FindMloPortalTreeNode(MCMloPortalDef portal)
    {
        ...
        if (mloarchetypenode != null)
        {
            ...
            for (var j = 0; j < portalsnode.Nodes.Count; j++)
            {
    ...
    public TreeNode FindMloEntitySetTreeNode(MCMloEntitySet entset)
    {
        ...
        if (mloarchetypenode != null)
        {
            ...
            for (var j = 0; j < entsetsnode.Nodes.Count; j++)
            {
    ...
    public TreeNode FindYbnTreeNode(YbnFile ybn)
    {
        ...
        for (var i = 0; i < ybnsnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindCollisionBoundsTreeNode(Bounds b)
    {
        ...
        for (var i = 0; i < bnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindYndTreeNode(YndFile ynd)
    {
        ...
        for (var i = 0; i < yndsnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindPathNodeTreeNode(YndNode n)
    {
        ...
        for (var i = 0; i < nodesnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindYnvTreeNode(YnvFile ynv)
    {
        ...
        for (var i = 0; i < ynvsnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindTrainTrackTreeNode(TrainTrack track)
    {
        ...
        for (var i = 0; i < trainsnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindScenarioTreeNode(YmtFile ymt)
    {
        ...
        for (var i = 0; i < scenariosnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindScenarioNodeTreeNode(ScenarioNode p)
    {
        ...
        for (var i = 0; i < pointsnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindAudioRelTreeNode(RelFile rel)
    {
        ...
        for (var i = 0; i < relsnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindAudioAmbientZoneTreeNode(AudioPlacement zone)
    {
        ...
        for (var i = 0; i < zonesnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindAudioAmbientRuleTreeNode(AudioPlacement rule)
    {
        ...
        for (var i = 0; i < rulesnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindAudioAmbientRuleTreeNode(uint hash)
    {
        ...
        for (var i = 0; i < relsnode.Nodes.Count; i++)
        {
            ...
            for (var j = 0; j < rulesnode.Nodes.Count; j++)
            {
    ...
    public TreeNode FindAudioStaticEmitterTreeNode(AudioPlacement emitter)
    {
        ...
        for (var i = 0; i < zonenode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindAudioAmbientZoneListTreeNode(Dat151AmbientZoneList list)
    {
        ...
        for (var i = 0; i < zonelistsnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindAudioStaticEmitterListTreeNode(Dat151StaticEmitterList list)
    {
        ...
        for (var i = 0; i < emitterlistsnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindAudioInteriorTreeNode(Dat151InteriorSettings interior)
    {
        ...
        for (var i = 0; i < interiorsnode.Nodes.Count; i++)
        {
    ...
    public TreeNode FindAudioInteriorRoomTreeNode(Dat151InteriorRoom room)
    {
        ...
        if (roomsnode == null) return null;
        for (var i = 0; i < roomsnode.Nodes.Count; i++)
        {
    ...
    public void TrySelectCollisionBoundsTreeNode(Bounds bounds)
    {
        ...
        if (tnode == null) tnode = FindYbnTreeNode(bounds?.GetRootYbn());
        if (tnode != null)
        {
    ...
    public void TrySelectCollisionPolyTreeNode(BoundPolygon poly)
    {
        ...
        if (tnode == null) tnode = FindCollisionBoundsTreeNode(poly?.Owner);
        if (tnode == null) tnode = FindYbnTreeNode(poly?.Owner?.GetRootYbn());
        if (tnode != null)
        {
            if (ProjectTreeView.SelectedNode == tnode)
    ...
    public void TrySelectCollisionVertexTreeNode(BoundVertex vert)
    {
        ...
        if (tnode == null) tnode = FindCollisionBoundsTreeNode(vert?.Owner);
        if (tnode == null) tnode = FindYbnTreeNode(vert?.Owner?.GetRootYbn());
        if (tnode != null)
        {
            if (ProjectTreeView.SelectedNode == tnode)
    ...
    public void TrySelectPathNodeTreeNode(YndNode node)
    {
        ...
        if (tnode == null) tnode = FindYndTreeNode(node?.Ynd);
        if (tnode != null)
        {
            if (ProjectTreeView.SelectedNode == tnode)
    ...
    public void TrySelectNavPolyTreeNode(YnvPoly poly)
    {
        ...
        if (tnode == null) tnode = FindYnvTreeNode(poly?.Ynv);
        if (tnode != null)
        {
            if (ProjectTreeView.SelectedNode == tnode)
                OnItemSelected?.Invoke(poly);
    ...
    public void TrySelectNavPointTreeNode(YnvPoint point)
    {
        ...
        if (tnode == null) tnode = FindYnvTreeNode(point?.Ynv);
        if (tnode != null)
        {
            if (ProjectTreeView.SelectedNode == tnode)
    ...
    public void TrySelectNavPortalTreeNode(YnvPortal portal)
    {
        ...
        if (tnode == null) tnode = FindYnvTreeNode(portal?.Ynv);
        if (tnode != null)
        {
            if (ProjectTreeView.SelectedNode == tnode)
                OnItemSelected?.Invoke(portal);
    ...
    public void TrySelectTrainNodeTreeNode(TrainTrackNode node)
    {
        ...
        if (tnode == null) tnode = FindTrainTrackTreeNode(node?.Track);
        if (tnode != null)
        {
            if (ProjectTreeView.SelectedNode == tnode)
                OnItemSelected?.Invoke(node);
    ...
    public void TrySelectScenarioNodeTreeNode(ScenarioNode node)
    {
        ...
        if (tnode == null) tnode = FindScenarioTreeNode(node?.Ymt);
        if (tnode != null)
        {
            if (ProjectTreeView.SelectedNode == tnode)
                OnItemSelected?.Invoke(node);
    ...
    public void TrySelectAudioAmbientZoneTreeNode(AudioPlacement zone)
    {
        ...
        if (tnode == null) tnode = FindAudioRelTreeNode(zone?.RelFile);
        if (tnode != null)
        {
            if (ProjectTreeView.SelectedNode == tnode)
    ...
    public void TrySelectAudioAmbientRuleTreeNode(AudioPlacement rule)
    {
        ...
        if (tnode == null) tnode = FindAudioRelTreeNode(rule?.RelFile);
        if (tnode != null)
        {
            if (ProjectTreeView.SelectedNode == tnode)
    ...
    public void TrySelectAudioStaticEmitterTreeNode(AudioPlacement emitter)
    {
        ...
        if (tnode == null) tnode = FindAudioRelTreeNode(emitter?.RelFile);
        if (tnode != null)
        {
            if (ProjectTreeView.SelectedNode == tnode)
    ...
    public void TrySelectAudioAmbientZoneListTreeNode(Dat151AmbientZoneList list)
    {
        ...
        if (tnode == null) tnode = FindAudioRelTreeNode(list?.Rel);
        if (tnode != null)
        {
            if (ProjectTreeView.SelectedNode == tnode)
    ...
    public void TrySelectAudioInteriorTreeNode(Dat151InteriorSettings interior)
    {
        ...
        if (tnode == null) tnode = FindAudioRelTreeNode(interior?.Rel);
        if (tnode != null)
        {
            if (ProjectTreeView.SelectedNode == tnode)
    ...
    public void TrySelectAudioInteriorRoomTreeNode(Dat151InteriorRoom room)
    {
        ...
        if (tnode == null) tnode = FindAudioRelTreeNode(room?.Rel);
        if (tnode != null)
        {
            if (ProjectTreeView.SelectedNode == tnode)
                OnItemSelected?.Invoke(room);
    ...
    public void RemoveEntityTreeNode(YmapEntityDef ent)
    {
        ...
        {
            tn.Parent.Text = "Entities (" + ent.Ymap.AllEntities.Length + ")";
    ...
    public void RemoveCarGenTreeNode(YmapCarGen cargen)
    {
        ...
        if (tn != null && tn.Parent != null)
        {
            tn.Parent.Text = "Car Generators (" + cargen.Ymap.CarGenerators.Length + ")";
            tn.Parent.Nodes.Remove(tn);
        }
    }
    ...
    public void RemoveGrassBatchTreeNode(YmapGrassInstanceBatch batch)
    {
        ...
        if (tn != null && tn.Parent != null)
        {
            tn.Parent.Text = "Grass Batches (" + batch.Ymap.GrassInstanceBatches.Length + ")";
            tn.Parent.Nodes.Remove(tn);
        }
    }
    ...
    public void RemoveArchetypeTreeNode(Archetype archetype)
    {
        ...
        if (tn != null && tn.Parent != null)
        {
            tn.Parent.Text = "Archetypes (" + archetype.Ytyp.AllArchetypes.Length + ")";
            tn.Parent.Nodes.Remove(tn);
        }
    }
    ...
    public void RemoveMloEntityTreeNode(MCEntityDef ent)
    {
        ...
        if (tn != null && tn.Parent != null) tn.Parent.Nodes.Remove(tn);
    }
    ...
    public void RemovePathNodeTreeNode(YndNode node)
    {
        ...
        if (tn != null && tn.Parent != null)
        {
            tn.Parent.Text = "Nodes (" + node.Ynd.Nodes.Length + ")";
            tn.Parent.Nodes.Remove(tn);
        }
    }
    ...
    public void RemoveTrainNodeTreeNode(TrainTrackNode node)
    {
        ...
        if (tn != null && tn.Parent != null)
        {
            tn.Parent.Text = "Nodes (" + node.Track.Nodes.Count + ")";
            tn.Parent.Nodes.Remove(tn);
        }
    }
    ...
    public void RemoveScenarioNodeTreeNode(ScenarioNode node)
    {
        ...
        if (tn != null && tn.Parent != null)
        {
            tn.Parent.Text = "Points (" + (node.Ymt?.ScenarioRegion?.Nodes?.Count ?? 0) + ")";
            tn.Parent.Nodes.Remove(tn);
        }
    }
    ...
    public void RemoveAudioAmbientZoneTreeNode(AudioPlacement zone)
    {
        ...
        if (tn != null && tn.Parent != null)
        {
            ...
            tn.Parent.Text = "Ambient Zones (" + zones.Count + ")";
            tn.Parent.Nodes.Remove(tn);
        }
    }
    ...
    public void RemoveAudioAmbientRuleTreeNode(AudioPlacement rule)
    {
        ...
        if (tn != null && tn.Parent != null)
        {
            ...
            tn.Parent.Text = "Ambient Rules (" + rules.Count + ")";
            tn.Parent.Nodes.Remove(tn);
        }
    }
    ...
    public void RemoveAudioStaticEmitterTreeNode(AudioPlacement emitter)
    {
        ...
        if (tn != null && tn.Parent != null)
        {
            ...
            tn.Parent.Text = "Static Emitters (" + emitters.Count + ")";
            tn.Parent.Nodes.Remove(tn);
        }
    }
    ...
    public void RemoveAudioAmbientZoneListTreeNode(Dat151AmbientZoneList list)
    {
        ...
        if (tn != null && tn.Parent != null)
        {
            ...
            tn.Parent.Text = "Ambient Zone Lists (" + zonelists.Count + ")";
            tn.Parent.Nodes.Remove(tn);
        }
    }
    ...
    public void RemoveAudioStaticEmitterListTreeNode(Dat151StaticEmitterList list)
    {
        ...
        if (tn != null && tn.Parent != null)
        {
            ...
            tn.Parent.Text = "Static Emitter Lists (" + emitterlists.Count + ")";
            tn.Parent.Nodes.Remove(tn);
        }
    }
    ...
    public void RemoveAudioInteriorTreeNode(Dat151InteriorSettings interior)
    {
        ...
        if (tn != null && tn.Parent != null)
        {
            ...
            tn.Parent.Text = "Interiors (" + interiors.Count + ")";
            tn.Parent.Nodes.Remove(tn);
        }
    }
    ...
    public void RemoveAudioInteriorRoomTreeNode(Dat151InteriorRoom room)
    {
        ...
        if (tn != null && tn.Parent != null)
        {
            ...
            tn.Parent.Text = "Interior Rooms (" + interiors.Count + ")";
            tn.Parent.Nodes.Remove(tn);
        }
    }
    ...
    {
        var focused = ProjectForm?.ContainsFocus ?? ContainsFocus;
        var addSelection = focused && (ModifierKeys & Keys.Control) > 0;
        var fillSelection = focused && (ModifierKeys & Keys.Shift) > 0;
        if (addSelection)
        {
        ...
        else if (fillSelection)
        {
            ...
            else
            {
                ...
                {
                    if (start) SelectedNodes.Add(cnode);
                    if (cnode == snode)
                    {
                        if (start) break;
                        start = true;
    ...
    {
        if (ProjectTreeView.SelectedNode != null) OnItemActivated?.Invoke(ProjectTreeView.SelectedNode.Tag);
    }
    ...
    private void ProjectTreeView_BeforeCollapse(object sender, TreeViewCancelEventArgs e)
    {
        ...
        if (inDoubleClick && e.Action == TreeViewAction.Collapse) e.Cancel = true;
    }

    private void ProjectTreeView_BeforeExpand(object sender, TreeViewCancelEventArgs e)
    {
        //if (e.Node.Tag != CurrentProjectFile) return; //disabling doubleclick expand/collapse only for project node
        if (inDoubleClick && e.Action == TreeViewAction.Expand) e.Cancel = true;
    }
