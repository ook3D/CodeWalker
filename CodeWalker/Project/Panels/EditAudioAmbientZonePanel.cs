...
public partial class EditAudioAmbientZonePanel : ProjectPanel
{
    ...
    private void UpdateUI()
    {
        ...
        {
            AddToProjectButton.Enabled = CurrentZone?.RelFile != null
                ? !ProjectForm.AudioFileExistsInProject(CurrentZone.RelFile)
                : false;
            ...
            if (ProjectForm.WorldForm != null) ProjectForm.WorldForm.SelectObject(CurrentZone);
        }
    }
    ...
    private void ProjectItemChanged()
    {
        ...
        if (CurrentZone?.RelFile != null) ProjectForm.SetAudioFileHasChanged(true);
    }
    ...
    private void ShapeComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        var shape = Dat151ZoneShape.Box;
        if (Enum.TryParse(ShapeComboBox.Text, out shape))
