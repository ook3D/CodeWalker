...
public partial class GenerateNavMeshPanel : ProjectPanel
{
    ...
    private void GenerateButton_Click(object sender, EventArgs e)
    {
        ...
        var min = FloatUtil.ParseVector2String(MinTextBox.Text);
        var max = FloatUtil.ParseVector2String(MaxTextBox.Text);

        if (min == max)
        {
            MessageBox.Show(
                "Unable to generate - No valid area was specified!\nMake sure Min and Max form a box around the area you want to generate the nav meshes for.");
            return;
        }

        if (min.X < -6000 || min.Y < -6000 || max.X > 9000 || max.Y > 9000) //it's over 9000
            if (MessageBox.Show(
                    "Warning: min/max goes outside the possible navmesh area - valid range is from -6000 to 9000 (X and Y).\nDo you want to continue anyway?",
                    "Warning - specified area out of range", MessageBoxButtons.YesNo) != DialogResult.Yes)
                return;
        ...
        var density = 0.5f; //distance between vertices for the initial grid
        ...
        var hitTestCount = 0; //statistic for number of hit tests done
        var hitCount = 0; //statistic for total ray hits
        var newCount = 0; //statistic for total new control vertices
        ...
        Task.Run(() =>
        {
            ...
            var vertexCountX = (int)vertexCountXY.X + 1;
            var vertexCountY = (int)vertexCountXY.Y + 1;
            //int vertexCountTot = vertexCountX * vertexCountY;
            vgrid.BeginGrid(vertexCountX, vertexCountY);

            var ray = new Ray(Vector3.Zero, new Vector3(0, 0, -1)); //for casting with
            ...
            {
                var ybn = gameFileCache.GetYbn(boundsitem.Name);
            ...
            for (var vx = 0; vx < vertexCountX; vx++)
            for (var vy = 0; vy < vertexCountY; vy++)
            {
            ...
            var tryGetEdge = new Func<Vector3, Vector3, GenEdge>((v1, v2) =>
            {
                ...
                GenEdge edge = null;
                if (edgeDict.TryGetValue(key1, out edge) || edgeDict.TryGetValue(key2, out edge)) return edge;
                return null;
            });
            ...
            {
                for (var p = 0; p < polys.Count; p++) //build edge dict
                {
                    var poly = polys[p];
                    poly.Index = p;
            ...
            var plthresh = 0.3f; //threshold for plane dist test
            var dpthresh = 0.75f; //threshold for plane normals test
            var dthresh = 6.0f; //absolute distance thresh
            for (var p = 0; p < polys.Count; p++)
            {
                ...
                for (var i = 0; i < poly.Vertices.Length; i++)
                {
                    ...
                    for (var j = 0; j <= eind1; j++) newverts.Add(poly.Vertices[j]);
                    for (var j = l2beg; j <= l2end; j++) newverts.Add(poly2.Vertices[j % p2cnt]);
                    for (var j = l1beg; j < p1cnt; j++) newverts.Add(poly.Vertices[j]);
                    ...
                    while (remredun)
                    {
                        ...
                        for (var j = 0; j < varr.Length; j++)
                        {
                            ...
                            if (v0 == v2)
                            {
                                ...
                                {
                                    if (j == varr.Length - 1) newverts = newverts.GetRange(0, newverts.Count - 1);
                    ...
                    for (var j = 0; j < newpoly.Vertices.Length; j++)
                    {
                        ...
                        if (tedge.Poly1 == poly2)
                        {
                            tedge.Poly1 = newpoly;
                            tedge.EdgeIndex1 = j;
                        }

                        if (tedge.Poly2 == poly2)
                        {
                            tedge.Poly2 = newpoly;
                            tedge.EdgeIndex2 = j;
                        }
            ...
            buildEdgeDict();
            var dpthresh1 = 0.5f;
            var dpthresh2 = 0.7f; //try preserve shape more when not attached
            foreach (var poly in polys)
            {
                ...
                for (var i = 1; i < poly.Vertices.Length; i++)
                {
                    ...
                    if (edge0 == null) continue; //really shouldn't happen
                    ...
                    if (edge1 == null) continue; //really shouldn't happen
                    ...
                    if (poly0 != null && poly0.Vertices.Length < 5) continue;
                    ...
                    edge.Poly2 = poly0;
                    edge.EdgeIndex2 =
                        poly0?.FindVertex(vert0) ?? -1; //(edge0.Poly2 != poly0) ? edge0.EdgeIndex1 : edge0.EdgeIndex2;
                    edgeDict[key] = edge;
            ...
            var findIntersection = new Func<GenPoly, int, int, int>((poly, i0, i1) =>
            {
                ...
                for (var i = 1; i < vcnt; i++)
                {
                    ...
                    var a1 = v1.Y - v0.Y;
                    var b1 = v0.X - v1.X;
                    var c1 = a1 * v0.X + b1 * v0.Y;
                    var a2 = v3.Y - v2.Y;
                    var b2 = v2.X - v3.X;
                    var c2 = a2 * v2.X + b2 * v2.Y;
            ...
            var findConvexSplit = new Func<GenPoly, int, int>((poly, starti) =>
            {
                ...
                if (iok < 0) iok += vcnt;
                var anyok = false;
                for (var i = -2; i >= -vcnt; i--)
                {
                    var i1 = i + starti;
                    if (i1 < 0) i1 += vcnt; //i1 = i1 % vcnt;
                    var angl0 = getAngle(poly, starti, i1);
                    var angldiff0 = getAngleDiff(curangl, angl0);
                    if (angldiff0 < 0) break; //split line would not be convex at starti
                    var i2 = (i1 + 1) % vcnt;
                    var angl1 = getAngle(poly, i1, i2);
                    var angldiff1 = getAngleDiff(angl0, angl1);
                    if (angldiff1 < 0) break; //split line would not be convex at i1
                    var angl2 = getAngle(poly, i1, i2);
                    var angldiff2 = getAngleDiff(angl2, prevangl);
                    if (angldiff2 < 0) break; //this step back is not convex
                    var inti = findIntersection(poly, starti, i1);
                    if (inti >= 0) break; //split line intersects a poly edge!
                    prevangl = angl2;
                    anyok = true;
                    iok = i1;
                }
                ...
                for (var i = 2; i <= vcnt; i++)
                {
                    ...
                    if (angldiff2 < 0) break; //this step forward is not convex..
                    var inti = findIntersection(poly, i1, starti);
                    if (inti >= 0) break; //split line intersects poly edge!
                    prevangl = angl2;
                    anyok = true;
                    iok = i1;
                }

                if (anyok) return iok | 0x40000000; //set this flag to indicate polys got switched
            ...
            foreach (var ynv in ynvs)
            {
                ...
                ProjectForm.Invoke((MethodInvoker)delegate { ProjectForm.AddYnvToProject(nynv); });
            }
    ...
    private class GenPoly
    {
        ...
        public Vector3 GetCenter()
        {
            ...
            {
                for (var i = 0; i < Vertices.Length; i++) c += Vertices[i];
                c /= Vertices.Length;
            }
        ...
        public void RemoveVertex(Vector3 v)
        {
            ...
            if (removed) Vertices = newverts.ToArray();
    ...
    {
        private readonly List<int> CornersB = new();
        private readonly List<int> CornersT = new();
        public int CurVertexCount;
        public int[,] VertexCounts;
        public int VertexCountX;
        public int VertexCountY;
        public readonly List<GenVertex> VertexList = new();
        public int[,] VertexOffsets;
        public GenVertex[] Vertices;
        private readonly List<Vector3> VerticesB = new();
        private readonly List<Vector3> VerticesT = new();
        ...
        public int FindVertex(int x, int y, float z, float thresh)
        {
            ...
            var offset = VertexOffsets[x, y];
            var count = VertexCounts[x, y];
            var lasti = offset + count;

            var minz = float.MaxValue;
            var mini = -1;
            for (var i = offset; i < lasti; i++)
            {
                var vz = Vertices[i].Position.Z;
                var dz = Math.Abs(vz - z);
        ...
        public void ConnectVertices()
        {
            ...
            for (var vx = 0; vx < VertexCountX; vx++)
            {
                ...
                {
                    var py = vy - 1;
                    var imin = VertexOffsets[vx, vy];
                    var imax = VertexCounts[vx, vy] + imin;
        ...
        public List<GenPoly> GenPolys()
        {
            ...
            for (var vy = 0; vy < VertexCountY; vy++)
            {
                ...
                for (var i = imin; i < imax; i++)
                {
                    ...
                    if (Vertices[i].PrevIDX < 0 && Vertices[i].PrevIDY < 0 && Vertices[i].NextIDX < 0 &&
                        Vertices[i].NextIDY < 0) continue; //(not connected to anything)
                    ...
                    var poly = new GenPoly(); //start a new poly
                    ...
                    var vplane = new Plane(Vertices[i].Position, Vertices[i].Normal);
                    var plthresh = 0.25f; //threshold for plane dist test

                    var dpx = FindPolyEdgeDist(ref vplane, plthresh, i, 0);
                    var dpy = FindPolyEdgeDist(ref vplane, plthresh, i, 1);
                    var dnx = FindPolyEdgeDist(ref vplane, plthresh, i, 2);
                    var dny = FindPolyEdgeDist(ref vplane, plthresh, i, 3);
                    ...
                    if (addpoly)
                    {
                        ...
                        if (poly.CornerIndices?.Length > 2) polys.Add(poly);
                    }
            ...
            foreach (var poly in polys)
            {
                ...
                for (var i = 0; i < poly.CornerIndices.Length; i++)
                {
                    var id = poly.CornerIndices[i];
        ...
        private void AssignVertices(ref Plane vpl, float plt, int i, int dnx, int dny, int dpy, int dir, GenPoly poly)
        {
            ...
            for (var qx = 0; qx <= maxdnx; qx++) //go along the row until the next poly is hit...
            {
                lastqi = qi;
                var qipy = qi; //bottom vertex id for this column
                var qiny = qi; //top vertex id for this column
                for (var qy = 0; qy <= cdpy; qy++) //assign this row of verts to the poly
                {
                ...
                var ndpy = FindPolyEdgeDist(ref vpl, plt, qi, dirpy); //height for the next col..
                var ndny = FindPolyEdgeDist(ref vpl, plt, qi, dirny);
                var ddpy = ndpy - cdpy;
                var ddny = ndny - cdny;
            ...
            CornersB.Add(lastqi);
            var cc = CornersB.Count + CornersT.Count - 1;
            var corners = new int[cc];
            var ci = 0;
            for (var c = 0; c < CornersB.Count; c++)
            {
                corners[ci] = CornersB[c];
                ci++;
            }

            for (var c = CornersT.Count - 1; c > 0; c--)
            {
                corners[ci] = CornersT[c];
                ci++;
            }
        ...
        private void AssignVertices2(ref Plane vpl, float plt, int i, int dnx, int dny, int dpy, int dir, GenPoly poly)
        {
            ...
            var ti = i;
            while (CanPolyIncludeNext(ref vpl, plt, ti, dirpx,
                       out ti)) qi = ti; //make sure to start at the leftmost point...


            //loop until top and bottom lines intersect, or moved more than max dist

            var slopeb = FindSlope(ref vpl, plt, qi, dir, dirpy, dirny, 100);
            var slopet = FindSlope(ref vpl, plt, qi, dir, dirny, dirpy, 100);
            var syb = MaxOffsetFromSlope(slopeb);
            var syt = MaxOffsetFromSlope(slopet);
            var ony = 0;
            var ldyb = 0;
            var ldyt = 0;
            //int corndxb = 0;
            ...
            for (var x = 0; x < 50; x++)
            {
                ...
                var nxi = qi;
                var cgx = CanPolyIncludeNext(ref vpl, plt, qi, dir, out nxi);
                if (!cgx)
                {
                    var ybi = qi;
                    for (var yb = 0; yb <= dyb; yb++)
                    {
                ...
                if (!cgx)
                    ...
                    break;
                if (nxi < 0) break; //shouldn't happen?


                int nextyb;
                int nextyt;
                var nextib = FindPolyEdgeID(ref vpl, plt, nxi, dirpy, out nextyb);
                var nextit = FindPolyEdgeID(ref vpl, plt, nxi, dirny, out nextyt);
                ...
                var nextsyb = nextyb - ony - dyb;
                var nextsyt = nextyt + ony - dyt;
        ...
        private void AssignVertices3(ref Plane vpl, float plt, int i, int dir, GenPoly poly)
        {
            ...
            var dy = dyb + dyt; //total distance between bottom and top
            ...
            var slopeb = FindSlope(ref vpl, plt, qib, dir, dirpy, dirny, dyb > 0 ? dyb : 100);
            var slopet = FindSlope(ref vpl, plt, qit, dir, dirny, dirpy, dyt > 0 ? dyt : 100);
            var syob = MaxOffsetFromSlope(slopeb);
            var syot = MaxOffsetFromSlope(slopet);
        ...
        public List<GenPoly> GenPolys2()
        {
            ...
            for (var vy = 0; vy < VertexCountY; vy++)
            {
                ...
                for (var i = imin; i < imax; i++)
                {
                    ...
                    if (nidxy == -1)
                    {
                        if (nidyx == -1) continue; //can't form a square! could use the 3?

                        nidxy = nidyx;
                    }


                    var f0 = CompareVertexTypes(i, nidx);
                    var f1 = CompareVertexTypes(nidx, nidxy);
                    var f2 = CompareVertexTypes(nidy, nidxy);
                    var f3 = CompareVertexTypes(i, nidy);
        ...
        private int FindNextID(ref Plane vpl, float plt, int i, int dirnx, int dirny, int dirpy, float slope,
            out int dx, out int dy)
        {
            ...
            var cgx = CanPolyIncludeNext(ref vpl, plt, i, dirnx, out ti);
        ...
        private bool CanPolyIncludeNext(ref Plane vplane, float plthresh, int i, int dir, out int ni)
        {
            ...
            switch (dir)
            {
                default:
                case 0:
                    ni = Vertices[i].PrevIDX;
                    ct = Vertices[i].CompPrevX;
                    break;
                case 1:
                    ni = Vertices[i].PrevIDY;
                    ct = Vertices[i].CompPrevY;
                    break;
                case 2:
                    ni = Vertices[i].NextIDX;
                    ct = Vertices[i].CompNextX;
                    break;
                case 3:
                    ni = Vertices[i].NextIDY;
                    ct = Vertices[i].CompNextY;
                    break;
            }
            ...
            if (Vertices[ni].PolyID >= 0) return false; //already assigned a poly..
        ...
        private float FindSlope(ref Plane vpl, float plt, int i, int dirnx, int dirny, int dirpy, float maxslope)
        {
            ...
            {
                var dy0 = FindPolyEdgeDist(ref vpl, plt, qi, dirny);
                var dy1 = FindPolyEdgeDist(ref vpl, plt, ti, dirny);
                var dy = dy1 - dy0;
            ...
            else //new slope must be <0
            {
                ...
                var dx0 = FindPolyEdgeDist(ref vpl, plt, qi, dirnx);
                var dx1 = FindPolyEdgeDist(ref vpl, plt, ti, dirnx);
                var dx = dx1 - dx0;
        ...
        private int FindNextCornerID(ref Plane vpl, float plt, int i, int dirnx, int dirny, int dirpy, float slope,
            out int dx, out int dy)
        {
            dx = 0;
            dy = 0;
            ...
            var mx = 0;
            var my = 0;
            var diry = slope > 0 ? dirny : dirpy;
            var incy = slope > 0 ? 1 : -1;
            if (slope >= 1 || slope <= -1)
            {
                var sy = (int)Math.Abs(slope);
                while (my < sy)
            ...
            if (slope != 0)
            {
                var sx = (int)Math.Abs(1.0f / slope);
                while (mx < sx)
                    if (CanPolyIncludeNext(ref vpl, plt, qi, dirnx, out ti))
                    {
                        qi = ti;
            ...
            //slope==0
            for (var x = 0; x < 50; x++) //just try go +X until there's a hit.
                if (CanPolyIncludeNext(ref vpl, plt, qi, dirnx, out ti))
                {
                    qi = ti;
                    dx++;
