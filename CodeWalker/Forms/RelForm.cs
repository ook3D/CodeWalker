...
public partial class RelForm : Form
{
    ...
    private Dat10Synth currentSynth;
    private bool DelayHighlight;
    private readonly Style ErrorStyle = new TextStyle(Brushes.Red, null, FontStyle.Underline);

    private readonly ExploreForm exploreForm;


    private string fileName;
    private readonly Style GreenStyle = new TextStyle(Brushes.Green, null, FontStyle.Regular);

    private bool loadingSynth;
    private bool LoadingXml;
    ...
    private readonly Style OrangeRedStyle = new TextStyle(Brushes.OrangeRed, null, FontStyle.Regular);
    private readonly Style RedStyle = new TextStyle(Brushes.Red, null, FontStyle.Regular);

    private Synthesizer synthesizer; // TODO(alexguirre): dispose synthesizer
    private string xml;
    ...
    {
        fileName = rel?.Name;
        if (string.IsNullOrEmpty(fileName)) fileName = rel?.RpfFileEntry?.Name;
        ...
        if (rel != null)
        {
            ...
            {
                sb.AppendLine("NameTable - " + rel.NameTable.Length + " entries");
                foreach (var name in rel.NameTable) sb.AppendLine(name);
                sb.AppendLine();
            }

            if (rel.IndexStrings != null)
            {
                sb.AppendLine("IndexStrings - " + rel.IndexStrings.Length + " entries");
                foreach (var rstr in rel.IndexStrings) sb.AppendLine(rstr.Name);
                sb.AppendLine();
            }

            if (rel.IndexHashes != null)
            {
                sb.AppendLine("IndexHashes - " + rel.IndexHashes.Length + " entries");
    ...
    private bool SaveRel(XmlDocument doc)
    {
        ...
        if (rpfFileEntry?.Parent != null)
        {
            ...
            try
            {
                ...
                StatusLabel.Text = metaFormat + " file saved successfully at " + DateTime.Now;
    ...
    private bool CloseDocument()
    {
        if (modified)
        {
            var res = MessageBox.Show("Do you want to save the current document before closing it?",
    ...
    private byte[] ParseSynthOutputs()
    {
        ...
        for (var i = 0; i < outputsIndices.Length; i++)
        {
            if (outputs.Length <= i)
                break;

            var output = "0";
            if (outputs[i].Length > 1) output = outputs[i].Substring(1);
            if (byte.TryParse(output, out var ind)) outputsIndices[i] = ind;
        }
    ...
    private Dat10SynthVariable[] ParseSynthVariables()
    {
        ...
        for (var i = 0; i < variablesLines.Length; i++)
        {
    ...
    private void Search()
    {
        ...
        var textsearch = SearchTextRadio.Checked;
        ...
        foreach (var rd in CurrentFile.RelDatasSorted)
            if (textsearch)
            {
                if ((rd.Name?.ToLowerInvariant().Contains(textl) ?? false) || rd.NameHash == hash ||
                    rd.NameHash == hashl ||
    ...
    private void XmlTextBox_VisibleRangeChangedDelayed(object sender, EventArgs e)
    {
        ...
        if (DelayHighlight) HTMLSyntaxHighlight(XmlTextBox.VisibleRange);
    }
    ...
    {
        var synth = (Dat10Synth)SynthsComboBox.SelectedItem;
        ...
        SynthVariablesTextBox.Text = string.Join(Environment.NewLine,
            synth.Variables.Select(v => $"{MetaXmlBase.HashString(v.Name)} {FloatUtil.ToString(v.Value)}"));

        SynthOutputsTextBox.Clear();
        SynthOutputsTextBox.Text = string.Join(" ",
            synth.OutputsIndices.Take(synth.OutputsCount).Select(bufferIdx => $"B{bufferIdx}"));
        ...
        if (newSynth == null)
            ...
            try
            {
                ...
                if (synthesizer == null)
                {
                    ...
                    {
                        var buffersCopy = new float[synthesizer.Buffers.Length][];
                        for (var i = 0; i < buffersCopy.Length; i++)
                        {
                            ...
                            try
                            {
                                ...
                                if (series != null)
                                {
                                    ...
                                    foreach (var v in buffersCopy[i])
                                        series.Points.AddY(Math.Max(Math.Min(v, 2.0f),
                                            -2.0f)); //make sure crazy accidental values don't crash it later
                                }
                            }
                            catch
                            {
                ...
                for (var i = 0; i < newSynth.BuffersCount; i++)
                {
