...
public partial class AwcForm : Form
{
    ...
    public void LoadAwc(AwcFile awc)
    {
        ...
        fileName = awc?.Name;
        if (string.IsNullOrEmpty(fileName)) fileName = awc?.FileEntry?.Name;
        ...
        LabelInfo.Text = awc.Streams.Length + " track(s), Length: " +
                         TimeSpan.FromSeconds(totalLength).ToString("h\\:mm\\:ss");
        UpdateFormTitle();
    }
    ...
    private void UpdateUI()
    {
        ...
        {
            var playedMs = Player.PlayTimeMS;
            var totalMs = Player.TotalTimeMS;
    ...
    private void ExportAsWav_Click(object sender, EventArgs e)
    {
        if (PlayListView.SelectedItems.Count == 1)
        {
            ...
            var ext = ".wav";
            if (audio?.MidiChunk != null) ext = ".midi";
    ...
    private void PlayListView_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (PlayListView.SelectedItems.Count == 1)
        {
            var item = PlayListView.SelectedItems[0];
            var audio = item.Tag as AwcStream;
            if (audio?.MidiChunk != null) ExportAsWav.Text = "Export as .midi";
        }
    }
