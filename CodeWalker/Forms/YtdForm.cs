...
public partial class YtdForm : Form
{
    ...
    private float CurrentZoom; //1.0 = 100%, 0.0 = stretch
    ...
    public void LoadYtd(YtdFile ytd)
    {
        ...
        FileName = ytd?.Name;
        if (string.IsNullOrEmpty(FileName)) FileName = ytd?.RpfFileEntry?.Name;
    ...
    public void LoadTexDict(TextureDictionary texdict, string filename)
    {
        ...
        var texlist = new List<Texture>(texs);
        texlist.Sort((a, b) => { return a.Name?.CompareTo(b.Name) ?? 0; });

        foreach (var tex in texlist)
        {
            var lvi = TexturesListView.Items.Add(tex.Name);
            lvi.ToolTipText = tex.Name;
            lvi.Tag = tex;
            lvi.SubItems.Add(tex.Width + " x " + tex.Height);
        }

        if (TexturesListView.Items.Count > 0) TexturesListView.Items[0].Selected = true;
    ...
    private string GetTexCountStr()
    {
        var texs = TexDict?.Textures?.data_items;
        if (texs == null) return "";
        return texs.Length + " texture" + (texs.Length != 1 ? "s" : "");
    }
    ...
    private void ShowTextureMip(Texture tex, int mip, bool mipchange)
    {
        ...
        {
            UpdateStatus("Error reading texture mip: " + ex);
            SelTexturePictureBox.Image = null;
        }
    ...
    private void UpdateSaveTextureAs()
    {
        ...
        {
            var fname = CurrentTexture.Name + ".dds";
            var sas = "Save " + fname + " As...";
    ...
    {
        if (Ytd == null) return;
        if (!(ExploreForm?.EditMode ?? false)) saveas = true;
        ...
        if (saveas) //save direct to filesystem in external location
        {
            ...
            var fpath = SaveYTDFileDialog.FileName;
        ...
        Modified = false;
        UpdateStatus("YTD file saved successfully at " + DateTime.Now);
        UpdateFormTitle();
    }
    ...
    private void SaveTextureAs()
    {
        ...
        var fpath = SaveDDSFileDialog.FileName;
        var dds = DDSIO.GetDDSFile(CurrentTexture);
    ...
    {
        Texture tex = null;
        if (TexturesListView.SelectedItems.Count == 1) tex = TexturesListView.SelectedItems[0].Tag as Texture;
    ...
    {
        var s = SelTextureZoomCombo.Text;
