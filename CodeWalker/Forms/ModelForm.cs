...
{
    private ClipMapEntry AnimClip;


    private bool animsInited;
    ...
    private readonly Dictionary<DrawableBase, bool> DrawableDrawFlags = new();
    private bool editingLights;
    ...
    private int gridCount = 40;
    private float gridSize = 1.0f;
    private readonly object gridSyncRoot = new();
    private readonly List<VertexTypePC> gridVerts = new();
    ...
    private ModelLightForm lightForm;


    private ModelMatForm materialForm;
    private Archetype ModelArchetype;

    private MetaHash ModelHash;
    private bool modelModified;
    private Point MouseDownPoint;
    private readonly bool MouseInvert = Settings.Default.MouseInvert;
    ...
    private volatile bool pauserendering = false;


    private readonly Vector3 prevworldpos = new(0, 0, 0); //also the start pos
    ...
    private bool updateArchetypeStatus = true;
    ...
    private void InitAnimation()
    {
        ...
        else
        {
            ...
            foreach (var ycde in ycds) ycdlist.Add(ycde.GetShortName());
    ...
    private Archetype TryGetArchetype(uint hash)
    {
        ...
        {
            UpdateStatus("Archetype: " + arch.Name);
    ...
    private void UpdateGridVerts()
    {
        lock (gridSyncRoot)
        {
            ...
            var s = gridSize * gridCount * 0.5f;
            var cblack = (uint)Color.Black.ToRgba();
            var cgray = (uint)Color.DimGray.ToRgba();
            var cred = (uint)Color.DarkRed.ToRgba();
            var cgrn = (uint)Color.DarkGreen.ToRgba();
            var interval = 10;
            ...
            for (var i = 0; i <= gridCount; i++) //draw main lines last, so they are on top
            {
                var o = gridSize * i - s;
                if (i % interval == 0)
                {
    ...
    private void RenderGrid(DeviceContext context)
    {
        ...
        lock (gridSyncRoot)
        {
            if (gridVerts.Count > 0) Renderer.RenderLines(gridVerts);
        }
    }
    ...
    {
        if (Widget.Visible) Renderer.RenderTransformWidget(Widget);
    }
    ...
    {
        if (AnimClip != null) AnimClip.EnableRootMotion = EnableRootMotion;
            ...
            if (Ydd.Loaded)
                foreach (var kvp in Ydd.Dict)
                    if (!DrawableDrawFlags.ContainsKey(kvp.Value)) //only render if it's checked...
                    {
                        ...
                        Renderer.RenderDrawable(kvp.Value, arch, null, Ydd.RpfFileEntry.ShortNameHash, null, null,
                            AnimClip);
                    }
        }
        else if (Ypt != null)
        ...
        {
            if (Ybn.Loaded) Renderer.RenderCollisionMesh(Ybn.Bounds, null);
        }
        else if (Ynv != null)
        {
            if (Ynv.Loaded) Renderer.RenderNavMesh(Ynv);
        }
    }
    ...
    public void LoadModel(YdrFile ydr)
    {
        ...
        if (ModelHash != 0) ModelArchetype = TryGetArchetype(ModelHash);
        ...
        if (ydr.Drawable?.LightAttributes.data_items.Length > 0) DeferredShadingCheckBox.Checked = true;
    ...
    public void LoadModels(YddFile ydd)
    {
        ...
        if (Ydd.Drawables != null)
        {
            ...
            foreach (var d in Ydd.Drawables)
            {
                ...
                if (d.Skeleton != null) Skeleton = d.Skeleton;
            }
    ...
    public void LoadModel(YftFile yft)
    {
        ...
        if (yft.Fragment?.LightAttributes.data_items.Length > 0) DeferredShadingCheckBox.Checked = true;
    ...
    public void LoadModel(YbnFile ybn)
    {
        ...
        if (Ybn.Bounds != null) MoveCameraToView(Ybn.Bounds.SphereCenter, Ybn.Bounds.SphereRadius);
    ...
    public void LoadParticles(YptFile ypt)
    {
        ...
        {
            var maxrad = 0.01f;
            foreach (var d in ypt.DrawableDict.Values) maxrad = Math.Max(maxrad, d.BoundingSphereRadius);
            MoveCameraToView(Vector3.Zero, maxrad);
        }
    ...
    private void LoadClipDict(string name)
    {
        ...
        items.Sort();
        foreach (var item in items) ClipComboBox.Items.Add(item);
    }
    ...
    private void UpdateControlInputs(float elapsed)
    {
        ...
        var movevec = Input.KeyboardMoveVec();
    ...
    private void UpdateModelsUI(Dictionary<uint, Drawable> dict)
    {
        ...
        if (dict != null)
        {
            var items = new List<KeyValuePair<uint, Drawable>>();
            foreach (var kvp in dict) items.Add(kvp);
    ...
    private void UpdateModelsUI(Dictionary<uint, DrawableBase> dict)
    {
        ...
        if (dict != null)
        {
            var items = new List<KeyValuePair<uint, DrawableBase>>();
            foreach (var kvp in dict) items.Add(kvp);
            items.Sort((a, b) =>
            {
                return ((MetaHash)a.Key).ToCleanString().CompareTo(((MetaHash)b.Key).ToCleanString());
            });
            foreach (var kvp in items)
            {
                AddDrawableTreeNode(kvp.Value, kvp.Key, check);
    ...
    private void AddDrawableModelsTreeNodes(DrawableModel[] models, string prefix, bool check,
        TreeNode parentDrawableNode = null)
    {
        ...
        for (var mi = 0; mi < models.Length; mi++)
        {
            ...
            var mnode = tnc.Add(mprefix + " " + model);
            mnode.Tag = model;
            mnode.Checked = check;

            var tmnode = TexturesTreeView.Nodes.Add(mprefix + " " + model);
            tmnode.Tag = model;
    ...
    private void UpdateSelectionDrawFlags(TreeNode node)
    {
        ...
        lock (Renderer.RenderSyncRoot)
        {
            if (drwbl != null)
            {
                if (rem)
                {
                    if (DrawableDrawFlags.ContainsKey(drwbl)) DrawableDrawFlags.Remove(drwbl);
                }
                else
                {
    ...
    private void UpdateEmbeddedTextures(DrawableBase dwbl)
    {
        ...
        {
            if (model?.Geometries == null) continue;
            foreach (var geom in model.Geometries) geom.UpdateRenderableParameters = true;
        }
    }
    ...
    public void UpdateEmbeddedTextures()
    {
        if (Ydr != null)
        {
            if (Ydr.Loaded) UpdateEmbeddedTextures(Ydr.Drawable);
        }
        else if (Ydd != null)
        {
            if (Ydd.Loaded)
    ...
    private void ShowMaterialEditor()
    {
        ...
        if (materialForm == null)
        {
            ...
            else if (dict != null) materialForm.LoadModels(dict);
    ...
    private void ShowTextureEditor()
    {
        ...
        else if (Ypt != null && Ypt.Loaded) td = Ypt?.PtfxList?.TextureDictionary;

        if (td != null)
        {
            var f = new YtdForm(null, this);
            f.Show(this);
    ...
    private void ShowLightEditor()
    {
        ...
        if (lightForm == null)
        {
            ...
            else if (dict != null) lightForm.LoadModels(dict);
        ...
        {
            if (lightForm.WindowState == FormWindowState.Minimized) lightForm.WindowState = FormWindowState.Normal;
            lightForm.Focus();
        }

        DeferredShadingCheckBox.Checked =
            true; //make sure we can see the lights we're editing (maybe this is bad for potatoes but meh)
    }
    ...
    private void Save(bool saveAs = false)
    {
        ...
        if (saveAs)
        {
            ...
            if (fileExt.Length > 1 && fileExt.StartsWith(".")) fileExt = fileExt.Substring(1);
        ...
        try
        {
            ...
            if (Ydr != null)
                ...
                fileBytes = Ypt.Save();
            else if (Ynv != null) fileBytes = Ynv.Save();
#if !DEBUG
        }
        catch (Exception ex)
        {
        ...
        if (rpfSave)
        {
            if (!rpfFileEntry.Path.ToLowerInvariant().StartsWith("mods"))
                if (MessageBox.Show(
                        "This file is NOT located in the mods folder - Are you SURE you want to save this file?\r\nWARNING: This could cause permanent damage to your game!!!",
                        "WARNING: Are you sure about this?", MessageBoxButtons.YesNo) != DialogResult.Yes)
            ...
            try
            {
                ...
                StatusLabel.Text = rpfFileEntry.Name + " saved successfully at " + DateTime.Now;
        ...
        {
            if (string.IsNullOrEmpty(fn)) fn = rpfFileEntry?.Path;
    ...
    private void SaveAllTextures(bool includeEmbedded)
    {
        ...
        var tryGetTextureFromYtd = new Func<uint, YtdFile, Texture>((texHash, ytd) =>
        {
            ...
            if (ytd.Loaded) return ytd.TextureDict?.Lookup(texHash);
            return null;
        });
        ...
        var collectTextures = new Action<DrawableBase>(d =>
        {
            if (includeEmbedded)
            {
                ...
                {
                    foreach (var tex in ypt.PtfxList.TextureDictionary.Textures.data_items) textures.Add(tex);
            ...
            if (arch == null) arch = currentArchetype;
            ...
            foreach (var s in d.ShaderGroup.Shaders.data_items)
            {
                ...
                foreach (var p in s.ParametersList.Parameters)
                {
                    ...
                    if (tex != null)
                    {
                        if (includeEmbedded) textures.Add(tex); //probably redundant
                    }
                    else
                    {
                        ...
                        if (tex == null)
                        {
                            ...
                            while (ptxdhash != 0 && tex == null)
                            {
                                ...
                                if (tex == null) ptxdhash = gameFileCache.TryGetParentYtdHash(ptxdhash);
                            }
                            ...
                            if (tex == null) texturesMissing.Add(t.Name);
                        }
        ...
        if (Ydr != null) collectTextures(Ydr.Drawable);
        if (Ydd?.Drawables != null)
            foreach (var d in Ydd.Drawables)
                collectTextures(d);
        if (Yft?.Fragment != null)
        ...
        if (successcount > 0) sb.AppendLine(successcount + " textures successfully exported.");
        if (texturesMissing.Count > 0) sb.AppendLine(texturesMissing.Count + " textures weren't found!");
        if (errordds.Count > 0) sb.AppendLine(errordds.Count + " textures couldn't be converted to .dds!");
        if (sb.Length > 0)
    ...
    {
        if (ActiveControl is NumericUpDown) ActiveControl = null;
        switch (e.Button)
        {
    ...
    private void ModelForm_KeyDown(object sender, KeyEventArgs e)
    {
        ...
        var enablemove =
            true; // (!iseditmode) || (MouseLButtonDown && (GrabbedMarker == null) && (GrabbedWidget == null));
    ...
    private void ToolsDragPanel_MouseMove(object sender, MouseEventArgs e)
    {
        ...
        {
            var rx = e.X + ToolsPanel.Left + ToolsDragPanel.Left;
    ...
    {
        if (e.Node != null) UpdateSelectionDrawFlags(e.Node);
    }
