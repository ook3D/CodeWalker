...
public partial class ModelMatForm : Form
{
    ...
    private void SelectGeometry(DrawableGeometry geom)
    {
        ...
        tmpPanel.Location = new Point(0, 0);
    ...
    private void ParamTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        else //Vector4 array
        {
            ...
            for (var i = 0; i < parm.DataType; i++)
            {
                var vec = Vector4.Zero;
                if (i < strs.Length) vec = FloatUtil.ParseVector4String(strs[i].Trim());
                vecs[i] = vec;
            }
        ...
        if (geom != null)
        {
            if (Drawable != null) UpdateRenderableParams(Drawable, geom.Shader);
