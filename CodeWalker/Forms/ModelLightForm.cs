...
public partial class ModelLightForm : Form
{
    ...
    private LightAttributes selectedLight;
    ...
    public void LoadModel(DrawableBase drawable)
    {
        ...
        if (dr != null)
        {
            ...
            if (lights != null) AddLightsTreeNodes(lights.data_items);
        }
        else if (fr != null)
        {
            FragDrawable = fr;
    ...
    public void LoadModels(Dictionary<uint, Drawable> dict)
    {
        ...
        if (dict != null)
            ...
            {
                var mhash = new MetaHash(kvp.Key);
                ...
                if (lights != null) AddLightsTreeNodes(lights, dnode);
    ...
    private void AddLightsTreeNodes(LightAttributes[] lights, TreeNode parent = null)
    {
        ...
        for (var mi = 0; mi < lights.Length; mi++)
        {
            var tnc = LightsTreeView.Nodes;
            var light = lights[mi];
            var mprefix = "Light" + (mi + 1) + " : " + light.Type;
    ...
    public void UpdateUI()
    {
        ...
        else
        {
            ...
            ColourLabel.BackColor = Color.FromArgb(light.ColorR, light.ColorG, light.ColorB);
            ...
            VolumeColorLabel.BackColor =
                Color.FromArgb(light.VolumeOuterColorR, light.VolumeOuterColorG, light.VolumeOuterColorB);
    ...
    public void UpdateWidgetTransform()
    {
        ...
        if (sl.Type == LightType.Capsule)
            ...
            ModelForm.SetWidgetTransform(pos, sl.Orientation,
                new Vector3(sl.ConeOuterAngle, sl.ConeInnerAngle, sl.Falloff));
        else
    ...
    private void CreateLight()
    {
        ...
        if (Drawable != null)
        {
            ...
            var lights = Drawable.LightAttributes.data_items?.ToList() ?? new List<LightAttributes>();
        ...
        else if (FragDrawable != null)
        {
            ...
            var lights = FragDrawable.OwnerFragment.LightAttributes.data_items?.ToList() ?? new List<LightAttributes>();
            lights.Add(selectedLight);
        ...
        else
        {
            ...
            if (n != null)
            {
                ...
                if (dr != null)
                {
                    ...
                    var lights = dr.LightAttributes.data_items?.ToList() ?? new List<LightAttributes>();
                    lights.Add(selectedLight);
    ...
    private void DeleteLight()
    {
        ...
        if (Drawable != null)
        {
            var lights = Drawable.LightAttributes.data_items.ToList();
        ...
        {
            var lights = FragDrawable.OwnerFragment.LightAttributes.data_items.ToList();
            lights.Remove(selectedLight);
            FragDrawable.OwnerFragment.LightAttributes.data_items = lights.ToArray();
            UpdateLightParams();
            LoadModel(FragDrawable);
        ...
        else
        {
            ...
            if (dr != null)
            {
                var lights = dr.LightAttributes.data_items.ToList();
                lights.Remove(selectedLight);
                dr.LightAttributes.data_items = lights.ToArray();
                UpdateLightParams();
                LoadModels(DrawableDict);
    ...
    private void UpdateVolumeColour()
    {
        ...
        VolumeColorLabel.BackColor = Color.FromArgb(r, g, b);
    ...
    public void SetWidgetModeUI(WidgetMode mode)
    {
        ...
        MoveMenuItem.BackColor =
            mode == WidgetMode.Position ? SystemColors.GradientActiveCaption : SystemColors.Control;
        RotateMenuItem.Checked = mode == WidgetMode.Rotation;
        RotateMenuItem.BackColor =
            mode == WidgetMode.Rotation ? SystemColors.GradientActiveCaption : SystemColors.Control;
    ...
    private void UpdateLightTreeNodeText()
    {
        ...
        if (lnode.Index >= 0) lnode.Text = "Light" + (lnode.Index + 1) + " : " + selectedLight.Type;
    }
    ...
    private void IntensityTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        var v = FloatUtil.Parse(IntensityTextBox.Text);
    ...
    private void InnerAngleTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        var v = FloatUtil.Parse(InnerAngleTextBox.Text);
    ...
    private void OuterAngleTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        var v = FloatUtil.Parse(OuterAngleTextBox.Text);
    ...
    private void CoronaIntensityTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        var v = FloatUtil.Parse(CoronaIntensityTextBox.Text);
    ...
    private void CoronaSizeTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        var v = FloatUtil.Parse(CoronaSizeTextBox.Text);
    ...
    private void TangentTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        var v = FloatUtil.ParseVector3String(TangentTextBox.Text);
    ...
    {
        var t = Vector3.Normalize(FloatUtil.ParseVector3String(TangentTextBox.Text));
        TangentTextBox.Text = FloatUtil.GetVector3String(t);
    }
    ...
    private void CalculateTangentButton_Click(object sender, EventArgs e)
    {
        var d = Vector3.Normalize(FloatUtil.ParseVector3String(DirectionTextBox.Text));
        var v = Vector3.Down;
        var t = Vector3.Normalize(Vector3.Cross(d, v));
        if (t == Vector3.Zero)
        {
            v = Vector3.Left;
    ...
    private void ExtentTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        var v = FloatUtil.ParseVector3String(ExtentTextBox.Text);
    ...
    private void CoronaZBiasTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        var v = FloatUtil.Parse(CoronaZBiasTextBox.Text);
    ...
    private void VolumeIntensityTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        var v = FloatUtil.Parse(VolumeIntensityTextBox.Text);
    ...
    private void VolumeSizeScaleTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        var v = FloatUtil.Parse(VolumeSizeScaleTextBox.Text);
    ...
    private void VolumeOuterExponentTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        var v = FloatUtil.Parse(VolumeOuterExponentTextBox.Text);
    ...
    private void ShadowNearClipTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        var v = FloatUtil.Parse(ShadowNearClipTextBox.Text);
    ...
    private void TimeFlagsTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        uint.TryParse(TimeFlagsTextBox.Text, out var v);
        if (selectedLight.TimeFlags != v) selectedLight.TimeFlags = v;
        populatingui = true;
        UpdateTimeFlagsCheckBoxes();
        populatingui = false;
    }
    ...
    private void FlagsTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        uint.TryParse(FlagsTextBox.Text, out var v);
        if (selectedLight.Flags != v) selectedLight.Flags = v;
        populatingui = true;
        UpdateFlagsCheckBoxes();
        populatingui = false;
    }
    ...
    private void TextureHashTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        if (!uint.TryParse(TextureHashTextBox.Text, out var v)) v = JenkHash.GenHash(TextureHashTextBox.Text);
        if (selectedLight.ProjectedTextureHash != v) selectedLight.ProjectedTextureHash = v;
    }
    ...
    private void CullingPlaneNormalTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        var v = FloatUtil.ParseVector3String(CullingPlaneNormalTextBox.Text);
    ...
    private void CullingPlaneOffsetTextBox_TextChanged(object sender, EventArgs e)
    {
        if (populatingui) return;
        if (selectedLight == null) return;
        var v = FloatUtil.Parse(CullingPlaneOffsetTextBox.Text);
