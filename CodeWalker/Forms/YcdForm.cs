...
public partial class YcdForm : Form
{
    ...
    public YcdForm()
    {
        ...
        MainListView.ContextMenuStrip.Items.Add(new ToolStripMenuItem("Export to openFormats (.onim)...", null,
            ExportOnim_Click));
    }
    ...
    private void ExportOnim_Click(object sender, EventArgs e)
    {
        if (MainListView.SelectedItems[0].Tag is Animation anim)
        {
            ...
            var newfn = $"{Path.GetFileNameWithoutExtension(Ycd.Name)}_{MainListView.SelectedItems[0].Text}.onim";
    ...
    public void LoadYcd(YcdFile ycd)
    {
        ...
        fileName = ycd?.Name;
        if (string.IsNullOrEmpty(fileName)) fileName = ycd?.RpfFileEntry?.Name;
