...
public partial class SettingsForm : Form
{
    ...
    public SettingsForm(WorldForm owner)
    {
        ...
        foreach (var keyval in allkeys) KeyBindingComboBox.Items.Add(keyval);
    ...
    private void LoadAdvancedSettings()
    {
        ...
        CacheSizeUpDown.Value = Math.Min(Math.Max(Settings.Default.CacheSize / 1048576, CacheSizeUpDown.Minimum),
            CacheSizeUpDown.Maximum);
        CacheTimeUpDown.Value = Math.Min(Math.Max((decimal)Settings.Default.CacheTime, CacheTimeUpDown.Minimum),
            CacheTimeUpDown.Maximum);
        GPUCacheTimeUpDown.Value =
            Math.Min(Math.Max((decimal)Settings.Default.GPUCacheTime, GPUCacheTimeUpDown.Minimum),
                GPUCacheTimeUpDown.Maximum);
        GPUFlushTimeUpDown.Value =
            Math.Min(Math.Max((decimal)Settings.Default.GPUCacheFlushTime, GPUFlushTimeUpDown.Minimum),
                GPUFlushTimeUpDown.Maximum);
        GeometryCacheSizeUpDown.Value =
            Math.Min(Math.Max(Settings.Default.GPUGeometryCacheSize / 1048576, GeometryCacheSizeUpDown.Minimum),
                GeometryCacheSizeUpDown.Maximum);
        TextureCacheSizeUpDown.Value =
            Math.Min(Math.Max(Settings.Default.GPUTextureCacheSize / 1048576, TextureCacheSizeUpDown.Minimum),
                TextureCacheSizeUpDown.Maximum);
        CollisionCacheSizeUpDown.Value =
            Math.Min(Math.Max(Settings.Default.GPUBoundCompCacheSize / 1048576, CollisionCacheSizeUpDown.Minimum),
                CollisionCacheSizeUpDown.Maximum);
    }
    ...
    private void KeyBindingComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            var val = (Keys)KeyBindingComboBox.SelectedItem;
            if (val != selKeyBindingVal)
            {
                ...
                if (worldForm != null) worldForm.SetKeyBindings(keyBindings);
            }
        }
    }
    ...
    private void CameraSensitivityUpDown_ValueChanged(object sender, EventArgs e)
    {
        ...
        if (worldForm != null) worldForm.SetCameraSensitivity(camSensitivity, camSmoothing);
    }
    ...
    private void MouseInvertCheckBox_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (worldForm != null) worldForm.SetMouseInverted(mouseinvert);
    }
    ...
    {
        if (MessageBox.Show("Are you sure you want to reset all CodeWalker settings to their default values?",
                "Confirm reset settings to defaults", MessageBoxButtons.YesNo) == DialogResult.Yes)
        {
    ...
    {
        if (worldForm != null) worldForm.OnSettingsFormClosed();
    }
