...
{
    private Texture currentTex; // Used by save button
    private bool MouseSelectEnable;
    ...
    public void SetSelection(MapSelection item)
    {
        ...
        if (item.EntityDef != null) AddSelectionEntityHierarchyNodes(item.EntityDef);
    ...
    private void AddSelectionEntityHierarchyNodes(YmapEntityDef entity)
    {
        ...
        if (seltn != null) HierarchyTreeView.SelectedNode = seltn;
    }
    ...
    private void SelectTexture(TextureBase texbase, bool mipchange)
    {
        ...
        if (tex == null && texbase != null) tex = TryGetTexture(texbase, out ytd, ref errstr);
        if (tex != null)
        {
            currentTex = tex;
            var mip = 0;
            ...
            object owner = null;
            if (Selection.Drawable != null) owner = Selection.Drawable.Owner;
            var ydr = owner as YdrFile;
            var ydd = owner as YddFile;
            var yft = owner as YftFile;

            SelTextureNameTextBox.Text = tex.Name;
            SelTextureDictionaryTextBox.Text = ytd != null ? ytd.Name :
                ydr != null ? ydr.Name :
                ydd != null ? ydd.Name :
                yft != null ? yft.Name : string.Empty;
    ...
    private Texture TryGetTexture(TextureBase texbase, out YtdFile ytd, ref string errstr)
    {
        ...
        if (tex == null)
        {
            ...
            while (ptxdhash != 0 && tex == null)
            {
                ...
                if (tex == null) ptxdhash = WorldForm.GameFileCache.TryGetParentYtdHash(ptxdhash);
            }
            ...
            if (tex == null)
            {
                ...
                if (ytd != null)
                {
                    ...
                    if (ytd.Loaded) tex = ytd.TextureDict.Lookup(texhash);
                }
    ...
    private void SaveAllTexturesButton_Click(object sender, EventArgs e)
    {
        ...
        foreach (TreeNode texnode in geomnode.Nodes)
        {
            ...
            var tex = texbase as Texture;
            var errstr = "";
            if (tex == null && texbase != null) tex = TryGetTexture(texbase, out _, ref errstr);
            if (tex != null)
