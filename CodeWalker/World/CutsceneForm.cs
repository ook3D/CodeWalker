...
public partial class CutsceneForm : Form
{
    ...
    private readonly AudioDatabase AudioDatabase;
            ...
            if (Playing)
            {
                ...
                if (Cutscene.PlaybackTime != newt)
                    if (EnableAudio) // handle looping cutscenes, loop the audio also
                        BeginInvoke(() => PlayAudio(Cutscene.PlaybackTime));
            }
    ...
    private void SelectCutscene(CutsceneDropdownItem dditem)
    {
        ...
        Task.Run(() =>
        {
            ...
            {
                if (!AudioDatabase.IsInited) AudioDatabase.Init(GameFileCache);
    ...
    private void CutsceneLoaded(Cutscene cs)
    {
        ...
        if (cs != null)
        {
            ...
            if (Playing) PlayAudio(cs.PlaybackTime);
        }
    ...
    private void AnimateCameraCheckBox_CheckedChanged(object sender, EventArgs e)
    {
        ...
        {
            if (Cutscene != null) Cutscene.CameraClipUpdate = true;
        }
    }
    ...
    private void SubtitlesCheckBox_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (Cutscene != null) Cutscene.EnableSubtitles = EnableSubtitles;
    }
    ...
    private void AudioCheckBox_CheckedChanged(object sender, EventArgs e)
    {
        ...
        {
            if (Playing && Cutscene != null) PlayAudio(Cutscene.PlaybackTime);
        }
    }
    ...
    private void VolumeTrackBar_Scroll(object sender, EventArgs e)
    {
        ...
        var sp = Cutscene?.SoundPlayer;
        if (sp != null) sp.SetVolume(Volume);
    }

    private class CutsceneDropdownItem
    {
...
public class Cutscene
{
    ...
    public bool CameraClipUpdate; //signal to the form to update the camera clip planes


    public CutsceneObject CameraObject;

    public Quaternion CameraRotationOffset = Quaternion.RotationAxis(Vector3.UnitX, -1.57079632679f) *
                                             Quaternion.RotationAxis(Vector3.UnitZ, 3.141592653f);
    ...
    public Dictionary<int, CutObject> Objects { get; set; }
    public Dictionary<int, CutsceneObject> SceneObjects { get; set; }
    public CutEvent[] LoadEvents { get; set; }
    public CutEvent[] PlayEvents { get; set; }
    public CutConcatData[] ConcatDatas { get; set; }

    public int NextLoadEvent { get; set; }
    public int NextPlayEvent { get; set; }
    public int NextCameraCut { get; set; }
    public int NextConcatData { get; set; }

    public Gxt2File Gxt2File { get; set; }
    ...
    {
        var cutListCount = (CameraCutList?.Length ?? 0) + 1;
        var shortName = CutFile.FileEntry?.GetShortNameLower() ?? "";
        Ycds = new YcdFile[cutListCount];
        if (!string.IsNullOrEmpty(shortName))
            for (var i = 0; i < cutListCount; i++)
            {
                var ycdname = shortName + "-" + i;
                ...
                if (ycd != null) ycd.BuildCutsceneMap(i);
                Ycds[i] = ycd;
            }
    }
    ...
    {
        if (newTime > Duration) newTime = 0.0f; //stop or loop?
        ...
        var cutOffset = newTime - cutStart; //offset into the current cut
    ...
    public void Render(Renderer renderer)
    {
        if (SceneObjects != null)
        {
            foreach (var obj in SceneObjects.Values)
            {
                ...
                if (obj.Ped != null) renderer.RenderPed(obj.Ped);
                if (obj.Prop != null) renderer.RenderArchetype(obj.Prop.Archetype, obj.Prop, null, true, obj.AnimClip);
                if (obj.Vehicle != null) renderer.RenderVehicle(obj.Vehicle, obj.AnimClip);
                if (obj.Weapon != null) renderer.RenderWeapon(obj.Weapon, obj.AnimClip);
            }
            ...
            foreach (var obj in SceneObjects.Values)
            {
                ...
                if (obj.HideEntity != null) renderer.RenderHideEntity(obj.HideEntity);
            }
        }
    }
    ...
    private void RaiseEvent(CutEvent e)
    {
        switch (e.iEventId)
        {
            case CutEventType.LoadScene:
                LoadScene(e);
                break;
            case CutEventType.LoadAnimation:
                LoadAnimation(e);
                break;
            case CutEventType.LoadAudio:
                LoadAudio(e);
                break;
            case CutEventType.LoadModels:
                LoadModels(e);
                break;
            case CutEventType.LoadRayfireDes:
                LoadRayfireDes(e);
                break;
            case CutEventType.LoadParticles:
                LoadParticles(e);
                break;
            case CutEventType.LoadOverlays:
                LoadOverlays(e);
                break;
            case CutEventType.LoadGxt2:
                LoadGxt2(e);
                break;
            case CutEventType.UnloadModels:
                UnloadModels(e);
                break;
            case CutEventType.UnloadRayfireDes:
                UnloadRayfireDes(e);
                break;
            case CutEventType.EnableScreenFade:
                EnableScreenFade(e);
                break;
            case CutEventType.EnableHideObject:
                EnableHideObject(e);
                break;
            case CutEventType.EnableFixupModel:
                EnableFixupModel(e);
                break;
            case CutEventType.EnableBlockBounds:
                EnableBlockBounds(e);
                break;
            case CutEventType.EnableAnimation:
                EnableAnimation(e);
                break;
            case CutEventType.EnableParticleEffect:
                EnableParticleEffect(e);
                break;
            case CutEventType.EnableOverlay:
                EnableOverlay(e);
                break;
            case CutEventType.EnableAudio:
                EnableAudio(e);
                break;
            case CutEventType.EnableCamera:
                EnableCamera(e);
                break;
            case CutEventType.EnableLight:
                EnableLight(e);
                break;
            case CutEventType.DisableScreenFade:
                DisableScreenFade(e);
                break;
            case CutEventType.DisableHideObject:
                DisableHideObject(e);
                break;
            case CutEventType.DisableBlockBounds:
                DisableBlockBounds(e);
                break;
            case CutEventType.DisableAnimation:
                DisableAnimation(e);
                break;
            case CutEventType.DisableParticleEffect:
                DisableParticleEffect(e);
                break;
            case CutEventType.DisableOverlay:
                DisableOverlay(e);
                break;
            case CutEventType.DisableAudio:
                DisableAudio(e);
                break;
            case CutEventType.DisableCamera:
                DisableCamera(e);
                break;
            case CutEventType.DisableLight:
                DisableLight(e);
                break;
            case CutEventType.Subtitle:
                Subtitle(e);
                break;
            case CutEventType.PedVariation:
                PedVariation(e);
                break;
            case CutEventType.CameraCut:
                CameraCut(e);
                break;
            case CutEventType.CameraShadowCascade:
                CameraShadowCascade(e);
                break;
            case CutEventType.CameraUnk1:
                CameraUnk1(e);
                break;
            case CutEventType.CameraUnk2:
                CameraUnk2(e);
                break;
            case CutEventType.CameraUnk3:
                CameraUnk3(e);
                break;
            case CutEventType.CameraUnk4:
                CameraUnk4(e);
                break;
            case CutEventType.CameraUnk5:
                CameraUnk5(e);
                break;
            case CutEventType.CameraUnk6:
                CameraUnk6(e);
                break;
            case CutEventType.CameraUnk7:
                CameraUnk7(e);
                break;
            case CutEventType.CameraUnk8:
                CameraUnk8(e);
                break;
            case CutEventType.DecalUnk1:
                DecalUnk1(e);
                break;
            case CutEventType.DecalUnk2:
                DecalUnk2(e);
                break;
            case CutEventType.PropUnk1:
                PropUnk1(e);
                break;
            case CutEventType.Unk1:
                Unk1(e);
                break;
            case CutEventType.Unk2:
                Unk2(e);
                break;
            case CutEventType.VehicleUnk1:
                VehicleUnk1(e);
                break;
            case CutEventType.PedUnk1:
                PedUnk1(e);
                break;
        }
    }
    ...
    private void LoadAudio(CutEvent e)
    {
        ...
        if (SceneObjects.TryGetValue(obje.iObjectId, out var audobj))
    ...
    private void LoadGxt2(CutEvent e)
    {
        ...
        if (gxt2entry != null) //probably should do this load async
        {
            ...
            if (Gxt2File != null)
                for (var i = 0; i < Gxt2File.TextEntries.Length; i++)
                {
    ...
    private void PedVariation(CutEvent e)
    {
        ...
        if (cso?.Ped != null)
        {
            var comp = args.iComponent;
            var drbl = args.iDrawable;
            var texx = args.iTexture;

            Task.Run(() => { cso.Ped.SetComponentDrawable(comp, drbl, 0, texx, GameFileCache); });
        }
    }
    ...
    private T[] RecastArray<T>(object[] arr) where T : class
    {
        ...
        for (var i = 0; i < arr.Length; i++) r[i] = arr[i] as T;
        return r;
    }
...
public class CutsceneObject
{
    ...
    public void Init(CutObject obj, GameFileCache gfc, AudioDatabase adb)
    {
        ...
        if (obj is CutNamedObject nobj) Name = nobj.cName;
    ...
    private void InitAudio(CutAudioObject aud, GameFileCache gfc, AudioDatabase adb)
    {
        ...
        var soundhash = JenkHash.GenHash(soundname);
        var soundhash2 = JenkHash.GenHash(soundname2);

        if (adb?.SoundsDB != null)
        {
            if (adb.SoundsDB.TryGetValue(soundhash, out var snd))
                SoundInfo = snd;
            else if (adb.SoundsDB.TryGetValue(soundhash2, out var snd2)) SoundInfo = snd2;
        }


        if (SoundInfo is Dat54StreamingSound strsnd)
        {
            var dur = strsnd.Duration;
            MetaHash awchash = 0;
            AwcFile awc = null;
            ...
            foreach (var chan in strsnd.ChildSounds)
                if (chan is Dat54SimpleSound chansnd)
                {
                    ...
                    {
                        awchash = chanawchash;
                        if (adb.ContainerDB.TryGetValue(awchash, out var awcentry))
                        {
                            awc = new AwcFile();
                    ...
                    if (awc?.StreamDict != null)
                    {
                        ...
                        if (awc.StreamDict.TryGetValue(chanhash, out var chanstream))
                        {
            ...
            for (var i = 0; i < streaminfs.Count; i++)
            {
    ...
    public override string ToString()
    {
        return CutObject?.ToString() ?? ObjectID + ": " + Name;
    }
}